...
- `nesting_engine`：sparrow 求解。`constraints.py`（重合/旋转**全局**上限 `MAX_OVERLAP_MM=10` / `MAX_ROTATION_TOL_DEG=45`（2026-08-17 起不再按片型钳制，版师按片型的工艺参考值在 `.docs/business/排料规则_详细版.md`）+ 位图腐蚀 + 合法性校验（US-002 起成对齐套校验与 `PAIR_TYPES` 已删））、`sparrow_baseline.py`（基线 + **共享层**：`SIZE_PALETTE`/`size_color`（2026-08-20 起**尺码** → 16 色循环表**单一真相源**，锚点 `SIZE_ANCHOR=DEFAULT_SIZES[0]=28` 稳定绝对映射、同码同色跨片型；solver manifest / PNG / DXF ACI / CLI SVG 四处同源取色；此前 US-002~2026-08-19 为 g 码键 `label_color`）/_clean_polygon/solve_with_progress，被 solver/export/sparrow_experiments 复用）、`sparrow_experiments.py`（旋转/重合公差实验；US-002 起内片集合改 `--internal g04,g07` 命令行参数）、`labeling.py`（**g01+ 编号单一真相源**：`assign_codes` 统一赋码（顺序模式 = `sequential_sort_key`（group_key 前置保跨码同号 + 几何稳定序）；母版 block 名自带 `g/G/#`+数字编号且每码内唯一时全量复用），被 parse 赋号/label 代表裁片两处共用；导出 PNG/DXF 逐片叠印 g 码，PLT 永不加文字）、`waist_band.py`（US-009 腰头成带核心，**v2 2026-08-21 构造性链构造重写**替换 v1 spyrrow 带内子求解：`build_band_plan` 按版师真实构造「N 条单副本异码链」—— 链内 size 降序滑移贴靠（`_slide_touch` 粗扫+二分，无 RNG 毫秒级）+ 整链点对称翻转 ⇒ **开口朝左+最大码在最右**（建议1 定稿形态），贴触判据 `_chain_gap`≤1mm + 开口/码序程序自检 → 原始轮廓 union→erode(d_g)→归一化组合片（`WB_*` pid 永不出现在 manifest/frame/导出）+ `expand_placements` 展开权威式（offset 减号）；fill<45% 降为灾难形态守卫、带高守卫 ≤min(gate,1910)；实测 6 码×1 链内缝隙 0.00mm、端到端 A/B **+2.27pt**（88.35% vs OFF 86.08%）；**US-015（2026-08-21）v1.1 填料混带**：`fillers` 任意 g 码多选（版师确认无白名单，仅数量 ≤3 + ≠主码护栏，`routes_ws._BAND_MAX_FILLERS`）全部副本在链堆叠后经 `_fill_gaps` 贪心塞带内空隙（成员棱边 y 候选 × rot{0,180} × 双滑向，`prep()` bbox 预筛 + `FILLER_BISECT_ITERS=24` 二分，同确定性无 RNG），带内碰撞多边形补腐蚀到 `BAND_INNER_D_MM=2.0`（版师口径 2~4mm 保守端 —— 肋间切口端部开到小件可入宽度；组合片外轮廓 erode 仍 d_g、纯腰路径逐字节不变）；5336 实测混带 fill 79.5%→**86.5%**、A/B 3 seed 均值 **−0.534pt**（两 seed 改善）verdict=accept；**禁 import web**，AST 守卫在 tests/test_waist_band.py）、`waist_band_gate.py`（US-010 go/no-go 试点闸门：`python -m materialsorting.nesting_engine.waist_band_gate` 三组决策实验 —— 密度 A/B（≤1.0pt 线）/NFP 吞吐微基准（≤30% 线）/fill-预算曲线，结论 go/go-with-chunks/no-go；**2026-08-21 实测 no-go**（A/B 劣化 1.204pt + NFP 劣化 35%）→ 纯腰 v1 不合入，直接转 US-015 v1.1 混填料路线；禁 import web/cli、只读 intermediate、产物只落 `out/config_runs/_probes/`）。
