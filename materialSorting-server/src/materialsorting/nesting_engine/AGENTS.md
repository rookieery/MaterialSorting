...
| 文件 | 角色 |
| --- | --- |
| `constraints.py` | 重合/旋转**全局**上限（2026-08-17 起：`MAX_OVERLAP_MM=10.0` / `MAX_ROTATION_TOL_DEG=45.0`，每片型钳制表已删，版师 per-ptype 参考值在 `.docs/business/排料规则_详细版.md` §3.2/§4）+ 位图腐蚀 + 合法性校验（US-002 起 `PAIR_TYPES` 与成对齐套校验已删 —— 服务于已删除的合成镜像模型） |
| `sparrow_baseline.py` | 基线求解 CLI + ★共享层（`LABEL_PALETTE`/`label_color`（US-002 起 g 码 → 16 色循环表单一真相源，取代 `PTYPE_COLORS`）/ `_clean_polygon` / `solve_with_progress`，被 experiments/export/solver 复用；SVG 图例按 g 码数值序、标题「裁片」） |
