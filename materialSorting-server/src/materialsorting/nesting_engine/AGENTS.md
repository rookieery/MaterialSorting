...
```bash
python -c "from materialsorting.nesting_engine import labeling, constraints, sparrow_baseline, waist_band, prefix"
```
...
| `prefix.py` | **US-001 起始端成套前后幅核心构造**（依据 `.docs/business/起始端成套前后幅_版师确认清单.md` v3 + P0 探针 `out/tmp_probe/prefix_lib.py` 移植收敛）：`eligible_sizes(quantities, front, back, sizes=None)`（P2 资格规则：该码两 g 码 demand 均 ==2，0/1/3/缺码不合格；sizes 过滤必须给 —— 资格码须真实进主解实例，5336 → {32,33,34,35,38}）/ `pick_prefix_size(eligible, *, seed, front, back)`（seeded 随机取一：`random.Random(zlib.crc32(f'{seed}\|{front}\|{back}'))` 派生 + 排序归一，勿用全局 random/hash()；空集合 `PrefixError` 文案指路数量矩阵）/ `build_prefix_plan(pid_meta, pieces_by_id, *, front_pid, back_pid, d_g, gate_nest, order='interleave')` → `(chunk, gaps, holes)`：4 片（前×2+后×2 同码）interleave 竖排贴靠（`_slide_touch_y` Y 向粗扫+二分，无 RNG）→ 原始轮廓 union → 焊接 → erode(d_g) → clean → 归一化，产 `PS_*` BandChunk（**直接喂 waist_band.expand_placements**）；守卫：order 非法 ValueError、副本不齐 2+2/不同码/竖排高>min(gate,1910)/贴触>1mm `PrefixError`。**rot180 负坐标坑**：成员先归一原点再做候选对齐+滑触，记账 `tr=(xoff−b0, yoff−b1)`（P0 曾因缺补偿片侧移并排，单测锁死）。`main` 冒烟（`python -m materialsorting.nesting_engine.prefix --intermediate <commit 产物>`）对拍 P0：size34 bbox 1155×1458 / fill 83.3% / gaps 0.00 / holes 0。**禁 import web/cli**（AST 守卫 tests/test_prefix.py）；复用 waist_band 私有算子（`_solid_region`/`_exterior_coords`/`_valid_geometry`）与 `erode_polygon`/`_transform_polygon`/`_clean_polygon` |
