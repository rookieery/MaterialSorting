...
- **测试**：`tests/test_waist_band_ws.py` 25 项（band 关闭四变体直收 manifest / 开启全链路 stage→manifest→末帧副本守恒+工件校验（`seed==zlib.crc32(b'1|g05')`）/ on-off manifest 逐字段一致 / 校验五参数 error 早退 / ack 放行 / DegenerateBand worker error / band 阶段 stop 无孤儿（**v2 成带毫秒级，stop 用例换 7 码×8 副本弧形腰片撑 ~2s 构造窗口**，弧片长宽比>6 须 `ack:true`）/ `exclude_labels` Item 层单元 / `time_budget` 旋钮 deprecated 兼容）。
