...
| `routes_ws.py` | **2026-08-20 自 server.py 拆出**。APIRouter：`WS /ws/solve` + `_terminate_solve_process` + `_SENTINEL`；从 runtime 取共享 executor/state；**US-011** `_parse_band` band 服务端校验（label `^g\d+$` / 存在母版 / quantities>0 / 硬警告形态需 ack；非法=结构化 error 早退 + **显式 ws.close()**——TestClient 下早退 return 不关 WS 则 client receive 永不抛 disconnect）+ `on_stage` 回调投 `{type:stage,stage:band}`；**US-015** fillers 护栏并入 `_parse_band`（数组项 `^g\d+$` / ≠主码 / 去重后 ≤`_BAND_MAX_FILLERS`=3 / 各自存在母版 + quantities>0；填料**不做**硬警告 ack 判定 —— 填料本就是小片；返回 dict 恒含 `fillers` 键，缺省 `[]` 旧 payload 行为不变） |
| `routes_band.py` | **US-013 新增**。APIRouter：`POST /api/band/preview`（FR-7 成带预演回显 —— executor 线程 5s 预算 `build_band_plan`，`_resolve_d_tol`/`build_pid_meta` 与 solve_worker._build_band 同口径）→ 200 `{ok,fill_pct,bbox,elapsed,break_even:[62.4,63.6]}`（几何失败也 200 `ok:false,error` = 结果数据不阻塞确认；400/409/422 `{error}`，硬警告 422 附 `hard_warning:true`）；不落 band_runs 工件；**US-015** body `band.fillers` 随预演同口径混带（fill 分子 = 腰 + 填料面积和，逐 filler `_resolve_d_tol` 喂 `filler_ds`） |
...
| `solve_worker.py` | **US-025 新增**。顶层 `solve_worker(pieces_snapshot, gate_mm, solve_params, result_queue)` —— Windows spawn 可 pickle（无闭包、参数全 JSON）。子进程内 `build_instance(...) → 投 {kind:manifest}` → `instance.solve(config, progress=ProgressQueue)` → drain 出中间解投 `{kind:frame,report}` → 末尾投 `{kind:final,final}` 或 `{kind:error,message}`。所有投递纯 JSON，spyrrow 对象绝不跨进程；**US-011** `band` 参数 → `_build_band`（**进程内**同步跑 `waist_band.build_band_plan`，不 spawn 孙进程；失败=「成带失败」error 不投 manifest）+ `_write_band_artifact`（`OUT_DIR/band_runs/*.json`，写失败仅 warn）+ `_emit_placed` 展开单点（WB_ 组合片条目展开回成员 placement，三处帧/final 发射共享）；**US-015** `_build_band` 透传 `fillers`/`filler_ds`（逐 filler `_resolve_d_tol`），主实例 `exclude_labels={label, *fillers}`（填料副本同扣减路径，manifest on/off 一致性不变） |
| `band_accept.py` | **US-014 新增**。A/B 验收闭环 CLI（`python -m materialsorting.web.band_accept [--quick]`，独立模块不进 server 路由零副作用）：同源同构终验 —— 判据①密度 A/B（off/on × seeds，均值劣化 ≤`DENSITY_ACCEPT_PT`=1.0pt）+ ②形态（`pair_adjacency` 同码成对相邻率 =100%（**边距口径**：同 pid 最近邻放置足迹最近边距 ≤`PAIR_EPS_MM`（=waist_band.`PAIR_ADJ_EPS_MM` 单一真相源）；PRD 中心距公式为同朝向并排特例，FR-8 头尾翻转 180° 成对中心距可达 L 量级而物理边距 ~0，中心距字段仅对照输出）+ `band_span` ≤ stage bbox+`SPAN_SLACK_MM`=2 刚性展开不变量；off 臂 g05 成对率对照参考）+ ③确定性（`frame_signature`/`final_signature` 忽略 wall-clock elapsed 逐帧对拍 + `artifact_replay_equal` band_runs 工件回放对拍（排除 `band_elapsed`））+ ④导出（`export_verify` PNG/R12-DXF/PLT + root logger「导出跳过」WB_ 泄漏哨兵捕获 + DXF AC1009/POLYLINE 头 + dxf/plt 字节 grep `b'WB_'`，on/off 双臂同管线）→ 报告 `out/config_runs/_probes/band_accept_report.json`（探针惯例）。跑批口径与生产 0.9063 基线**不同构**（uploads 源、web 默认 params/per_type ⇒ g05 d=0.0、无 kill），只与同配置 off 对照；P0 需求表镜像 `accept_quantities`（31→1/36→3/余 2 ⇒ g05 14 副本） |
| `export.py` | `apply_transform` / `placed_to_world`（用**原始**非 eroded 轮廓，pid 直查 intermediate 零重放；US-024 起 5 层一并变换，notch 点按点变换 + 法线按向量旋转；US-002 起输出无 ptype 键、`color=size_color(size)`（2026-08-20 尺码键））/ `render_png`（matplotlib Agg；US-024 起 5 层叠加 net 绿虚线 / internal 橙 / notch 黄短线段 / grain 红虚线；图例条目 = placed 的尺码并集数值序、标题「尺码」）/ `write_marker_dxf`（R12 POLYLINE + ACI 色；US-024 起多 layer outline1/14/8/4/7 各自独立 entity；ACI = `size_aci(size)` = `((size-28)%24)+1`（2026-08-20 尺码键））/ `write_marker_plt`（US-033 HPGL/HP-GL 文本 `IN;PS;SP1-5;PU/PD/PG`，无 VS/LB；坐标=mm×40 round；5 层笔号与 DXF layer 同语义；空层跳过；纯 ASCII bytes，无临时文件、无新依赖）。**2026-08-20 拆分**（600 行硬限制，658→47 门面 + 四模块）：`export_geometry.py`（placed_to_world/apply_transform/size_aci/LAYER5 常量共享层）/ `export_png.py`（render_png，Agg+CJK rcParams 副作用保留）/ `export_dxf.py`（write_marker_dxf，ezdxf 警告抑制保留）/ `export_plt.py`（write_marker_plt + 全部 PLT 算子常量）；export.py 门面 re-export 全部旧公共符号（含 `_plt_frame_stats`/`_PLT_PD_MAX_PTS`），消费方 import 路径零改动 |
...
- **StartPayload 新增可缺省 `band` 键**：`{enabled, label, ack?, fillers?, time_budget?}`；缺省/null/{}/非 dict/enabled falsy = 关闭（旧行为逐字段不变，`test_solve_proc.py`/`test_ws_stop.py` 零改动全绿）。`fillers` = US-015 填料 g 码数组（缺省/空数组 = 纯腰 v2 路径，旧 payload 形状不变）。`time_budget` 是**内部旋钮**（测试/US-013 预演缩短成带预算，缺省 15s=`DEFAULT_BAND_TIME_BUDGET_S`；非法值静默回退 None），非 FR-1 前端契约键。
- **`_parse_band` 服务端校验（routes_ws，单一校验点）**：enabled 时 label 须匹配 `^g\d+$` + 存在于当前母版 + 该 g 码 quantities>0（demand 口径镜像 `build_pid_meta`：missing→1、显 0→0、sizeKey=`str(size)`/null→`'null'`）；成员最小边 <60mm 或长宽比 >6（硬警告形态，阈值 `_BAND_ACK_MIN_EDGE_MM`/`_BAND_ACK_MAX_ASPECT`，US-013 试用后可调）须 payload 显式 `ack:true`。**US-015 fillers 护栏**：须为 g 码数组、逐项 `^g\d+$`、≠ 主 g 码、去重后 ≤`_BAND_MAX_FILLERS`=3、各项存在母版 + quantities>0；填料**不做**硬警告 ack 判定（填料本就是小片，ack 语义只约束主腰形态）。非法抛 `ValueError` → ws_solve 发 `{type:error}` 早退（**不发 manifest**，与 build 失败同契约）+ **显式 `await ws.close()`**（TestClient 下早退 return 不关 WS 则后续 client receive_json 永不抛 disconnect → 测试挂死）。
...
- **manifest 一致性不变量（AC#1）**：band on/off 的 manifest `total_area_mm2` + `pieces` 列表**逐字段一致** —— `exclude_labels={label}` 只跳 spyrrow Item 层，`pid_meta`/`total_area` 原样（禁用「quantities=0 式移除」——那会连 manifest 一起抹掉）；US-015 起排除集扩为 `{label, *fillers}`（填料副本同扣减路径，一致性口径不变）。
...
- **band_runs 工件**：`_write_band_artifact` 落 `paths.OUT_DIR/band_runs/band_{label}_seed{seed}_%Y%m%d_%H%M%S.json`（= `BandChunk.to_dict()` + `main_seed`/`band_elapsed`；US-014 回放对拍数据源）。写失败仅 stderr warn 不阻塞求解；`MS_OUT_DIR` 环境变量随 spawn 传递（测试隔离靠它，monkeypatch 父进程 `paths` 无效）。
- **测试**：`tests/test_waist_band_ws.py` 38 项（band 关闭四变体直收 manifest / 开启全链路 stage→manifest→末帧副本守恒+工件校验（`seed==zlib.crc32(b'1|g05')`）/ on-off manifest 逐字段一致 / 校验五参数 error 早退 / ack 放行 / DegenerateBand worker error / band 阶段 stop 无孤儿（**v2 成带毫秒级，stop 用例换 7 码×8 副本弧形腰片撑 ~2s 构造窗口**，弧片长宽比>6 须 `ack:true`）/ `exclude_labels` Item 层单元 / `time_budget` 旋钮 deprecated 兼容 / **US-015** fillers 校验八参数矩阵 + 去重放行 + 混带 e2e（stage→manifest→末帧 g05+g06 守恒 + 工件 fillers 记录）+ 混带 on/off manifest 一致性 + 预演带 fillers 200 ok + 预演 fillers 422）。

## US-014 关键约定（A/B 验收闭环 band_accept + 形态判据口径）

- **跑批口径三差异**（与生产 0.9063 基线**不同构**，只与同配置 off 臂对照）：uploads 源 intermediate / web 默认口径（params 全 0 + per_type=None ⇒ g05 d=0.0，= 版师开 band 不配 per_type 的实际形态）/ 无 kill（单 seed 全预算跑完）。
- **成对判据边距口径**：PRD 公式「中心距 ≤ (w_i+w_j)/2+ε」是同朝向并排的对轴特例（该情形 中心距−w == 边距，两口径等价）；FR-8 头尾翻转 180° 成对（g05 L/w≈6 长片）中心连线沿长边、中心距可达 L 量级而物理边距 ~0（US-014 实测 g05_34 对：边距 1.5mm / 中心距 713mm）—— 中心距口径会把物理相邻的成对误判散落。权威判据 = 放置足迹最近边距（shapely `distance`，与朝向无关），`PAIR_EPS_MM` 引 `waist_band.PAIR_ADJ_EPS_MM`（=10，单一真相源：吸收 spyrrow 紧排缝隙实测 0.01~10mm）。
- **成带机制 v2（2026-08-21 构造性链构造，替换 US-014 成对形态重试）**：`nesting_engine/waist_band.py` 重写 —— 根因：v1 spyrrow StripPacking 带内求解目标是最短用布 X 而非贴触，产出 48.2% 对角阶梯坏带形，刚性组合片反噬主解 -3.81pt（US-014 成对重试在每 pid 单副本配置下 `_pairs_complete` 空真无牙口）。v2 = 版师真实构造「N 条单副本异码链」：链内 size 降序构造 + 整链点对称翻转 ⇒ **开口朝左+最大码在最右**（用户建议1），链内缝隙 0.00mm（贴触判据 ≤1mm，非 bbox fill）。实测 5336：6 码×1 fill 71.8%（537×1326）/ P0 14 副本 79.5%；端到端 A/B（30s seed0 生产 WS 链路）**ON 88.35% vs OFF 86.08%（+2.27pt）**、无 WB_ 泄漏、守恒 6/6。带高守卫 `min(gate,1910)`（组合片须进主解条带）。`time_budget` 旋钮 deprecated no-op（构造毫秒级）；成对判据（本文件边距口径）仍服务 band_accept 回放对拍。曾试「分码分块拼接」形态：成对 100% 但均值密度劣化 3.7pt（宽 1818×1271 组合片主解难包），已否决 —— 形态与密度需同时达标，构造性链两者兼得。
- **确定性对拍三件套**：`frame_signature`/`final_signature`（忽略 wall-clock elapsed）逐帧相等 + `artifact_replay_equal`（band_runs 工件 JSON 相等，排除 `band_elapsed`）。**quick 档（秒级预算）帧列可因 wall-clock 截断漂移**（主解按时长截断、迭代数随机器负载波动；确定性验收线在产品预算档 —— 120s×2 对拍 2015 帧全等）。
- **verdict**：`{density, form, determinism, export, conclusion: accept|reject}` 四判据与门；CLI 退出码 0/1；报告/导出落 `out/config_runs/_probes/`（探针惯例，只读 web 事实源）。

## US-015 关键约定（v1.1 填料混带：任意 g 码多选 + 同守恒/泄漏/扣减口径）

- **协议**：`band.fillers: [g 码]`（版师确认无白名单约束，仅数量上限 + 不可选主 g 码）；前端镜像常量 `BAND_MAX_FILLERS=3`（`lib/params.ts`）与后端 `_BAND_MAX_FILLERS` 同值。缺省/空数组 = 纯腰 v2 路径（payload 形状与旧版逐键一致，`collectBand` 清洗后非空才附键）。
- **web 编排（solve_worker._build_band）**：填料成员与腰成员同一展开/守恒/泄漏口径 —— `fillers` 全部副本进带（`_fill_gaps` 塞隙）、`BandChunk.fillers` 进 to_dict/band_runs 工件、帧前展开单点不变；主实例同步扣减 `exclude_labels={band.label, *band.fillers}`（同一 exclude_labels 路径，manifest on/off 一致性口径不变）。
- **d 口径（`BAND_INNER_D_MM=2.0`）**：混带下带内碰撞多边形在成员基准 d（主码 `d_g` / 填料 `filler_ds[label]` 逐个 `_resolve_d_tol` 裁定）之上补腐蚀到 2.0（版师口径 2~4mm 保守端）—— 肋间切口端部开到小件可入宽度；**组合片外轮廓 erode 仍 `d_g`**（包络断言不变）、**纯腰路径碰撞腐蚀不放大（逐字节不变）**；erode 失败自动回退原轮廓（碰撞超集安全方向）。
- **A/B 验收（scripts/us015_band_ab.py，复用 band_accept 探针套件）**：`py -3.11 scripts/us015_band_ab.py --fillers g07 [--seeds 0,1,2] [--time 120]` —— off vs on(fillers) 双臂 × seeds，判据：带 fill ≥ break-even 62.4% + seed 均值劣化 ≤1.0pt + 守恒（placed 计数 vs P0 demand + 无 WB_）；报告 `out/config_runs/_probes/us015_ab_report.json`。2026-08-21 实测（g05+g07、3 seed×120s）：带 fill 79.5%→**86.5%**（bbox 1153→1097 收窄）、均值劣化 **−0.534pt**（seed0 +2.06 / seed1 +0.46 / seed2 −0.92）、verdict=accept；浏览器终值对拍 87.77%==87.766%（scripts/us015_verify.mjs 17 项全过）。填料选择依据：g06（711×30 长条）fill 44.4% 触发 BandQualityError 灾难守卫、g07（216×67）实测最优。
- **浏览器目检证据**：`.docs/business/us015_filler_multiselect.png`（多选交互 + 预演回显 86.5%）/ `us015_mixed_band_stage.png` / `us015_mixed_band_final_seed0.png`（终值 87.77% 对拍）。
