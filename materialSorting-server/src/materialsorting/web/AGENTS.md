...
| 文件 | 角色 |
| --- | --- |
| `server.py` | FastAPI app；路由 `GET /`、`mount /static`、`POST /export`、`POST /api/parse-dxf`（US-004）、`POST /api/commit-to-nesting`（US-010 + **US-002 多会话绑定**：读 `X-Session-Id`，带 sid → per-doc 快照挂会话不碰 default；无 sid → default reload）、`POST /api/session`（多会话 US-001：Header 解析 + `resolve(create=True)` + `SessionError`→JSONResponse 一行映射）、`GET /api/ptypes`（US-020）、`WS /ws/solve`；US-020 可 reload `_PIECES_STATE`（threading.Lock 保护）；`ThreadPoolExecutor(max_workers=6)` 求解桥 + 上传解析/commit 复用；**文件尾** `from .strategy import register_strategy_routes` 注册策略四路由（US-004，见下）+ `session_registry.start_scanner()`（US-001 会话过期 daemon 扫描）；commit 落盘 doc dict 带 `doc_id` 键（策略 start 定位母版原件）；**US-006（2026-08-27）**：commit 路由尾 `run_in_executor(diskclean.trigger_cleanup, str(UPLOADS_DIR))` best-effort TTL 清理 + `main()` 起 `diskclean.start_startup_cleaner()`（daemon 线程，只在真 server 进程触发——TestClient 导入 app 不起线程不碰真实 out/）。**2026-08-20 拆分**（600 行硬限制，783→329）：state/线程池搬 `runtime.py`、parse 响应构造搬 `parse_payload.py`、WS solve 搬 `routes_ws.py`、`/`·ptypes·`/export` 搬 `routes_views.py`（APIRouter include，注册顺序不变）；**`_commit_to_nesting_sync` + 上传路由留守**（测试 monkeypatch `server_mod.UPLOADS_DIR` 依赖 server 命名空间 `__globals__`）；拆出符号全部 re-export（`_DOC_ID_RE` 2026-08-27 起改自 `sessions.SID_RE` re-export，同一编译对象）；**US-002（2026-08-27）**：`PIECES_INTERMEDIATE_NAME` + `_per_doc_intermediate(doc_id)` 路径约定 + commit 双写（详见下「多会话 US-002 关键约定」） |
...
| `export_plt.py` | **2026-08-20 自 export.py 拆出**。`write_marker_plt` / `_plt_frame_stats` / `_plt_pt` / `_plt_polyline` / `_clip_closed_y` / `_clip_open_y` / `_y_clip_point` + 全部 PLT 常量（2026-08 撞机修正注释块整段搬移）。**2026-08-28 版师定案**：门幅框/裁剪界 = 输入 `gate_mm` 原样（`PLOT_SAFE_MAX_Y_MM` 1910 钳制已删，框顶 gate−5） |
...
- **worker 形态与构造**：`solve_worker(..., prefix={'front','back'})` → `_build_prefix` **进程内同步**构造（不 spawn 孙进程，同 band 进程模型）：`eligible_sizes` → `pick_prefix_size(seed=solve_params['seed'], front, back)`（zlib.crc32 派生 Random，跨进程可重放）→ `build_prefix_plan`（d_g=max(d_front,d_back) 经 `_resolve_d_tol`，gate_nest=gate_mm 原样，2026-08-28 起单一口径）。失败（PrefixError/ValueError/几何异常）只投 `{kind:error,'前缀构造失败:…'}` 不投 manifest（同 build 失败契约）。
- **manifest 一致性不变量**：prefix on/off 的 manifest `total_area_mm2`+`pieces` 逐字段一致 —— 主实例 `exclude_pids={f'{front}_{size}', f'{back}_{size}'}`（**pid 级** Item 层扣减 {(front,size),(back,size)}），与 band 的 `exclude_labels` **并存互不干扰**（双开时两个都传）；**禁 quantities=0 式移除**（会连 pid_meta/total_area 一起抹掉）。
...
- **消息序（双开 band+prefix）**：`stage(band) → stage(prefix) → manifest → frame* → final`。prefix stage = `{type:'stage',stage:'prefix',size,fill_pct,bbox,holes,elapsed}`（`on_stage` 已泛化为键白名单转发，band/prefix 共用回调）；旧前端 default:break 静默忽略。
- **final 置换挂钩（仅 final，帧不置换）**：`_finalize_prefix` 在 final 发射前调 `pin_prefix_layout`（US-002）—— 组合片在主解 min_x≤6mm 已锚定 → 跳过（P0 常态）；>6mm → permute+reinsert+validate 复检，失败回退置换前布局（`rolled_back=True`）。final 消息带 `prefix: {size, pid, pin: pin_stats, band_pos}`；`width_mm` 口径：skip/回退 = solver 原值，置换成功 = 原始轮廓世界几何重算（`max(bounds[2]) − min(min(bounds[0]), 0)`）。
...
- **前缀工件 `_write_prefix_artifact`**：`paths.PREFIX_RUNS_DIR/<yyyymmddTHHMMSS>_<pid>.json`（ts 去冒号连字符），内容 = 资格码构造（size/fill_pct/bbox/holes/gaps/d_g/stage_elapsed）+ `chunk.to_dict()` 完整回放 + pin stats + band_pos + width_mm —— US-005 A/B 回放对拍数据源；写失败仅 `_log.warning` 不影响求解交付。测试隔离：`MS_OUT_DIR` 环境变量（spawn 子进程经 env 继承 re-import paths）。
...
- **测试**：`tests/test_prefix_ws.py` 23 项（关闭四变体零回归 / 全链路 stage→manifest→副本守恒+final.prefix / on-off manifest 逐字段一致 / 校验 7 参数化 error 早退 / `_parse_prefix` 纯单元 / sizes 过滤尊重资格码 / worker 构造失败只投 error / 双开 stage 序+band_pos / band 单开 / prefix 单开 band_pos None / exclude_pids Item 层单元 / proc 级 pin 守卫+工件）。
...
- **生命周期**：容量 4 / 空闲 10 分钟过期（墓碑 1h，401 `session_expired` 阻断弹窗，前端弃 sid 刷新重来）/ 429 `session_limit`；**求解中（`ws_open>0`）与策略轮询中（status 即 touch）不被扫描误杀**。
- **环境变量**（import 时解析，非法 warn 回退缺省）：`MS_SESSION_MAX=4`（并发会话上限，default 不占额）、`MS_SESSION_TTL_SEC=600`（空闲过期秒数）、`MS_UPLOAD_TTL_DAYS=14`（uploads 磁盘 TTL 天数，US-006）。
...
- **惰性过期 + 扫描兜底**：请求时惰性检查（超时当场逐出为墓碑 → 401）+ 30s daemon 扫描线程（server.py 文件尾 `session_registry.start_scanner()` 启动；daemon=True）。**扫描必须跳过 `ws_open>0`**（WS 连接钉住，求解中不误杀）；已死会话不再发请求，容量名额只能靠扫描回收。扫描线程异常只 warn 不崩。
...
- **测试**：`tests/test_web_strategy.py` 29 存量例零改动 + 新增 6 例（`dual_env` fixture：`strat_env` 之上隔离 registry + `_STRATEGY_STATES`）：双会话 start/status 互不串台（B 目录 mtime 更新不串台——不依赖 mtime）/ 清理按 sid 前缀（B start 不删 A 的 run 目录；default 保护并行会话）/ stop 只树杀本会话 pid + result 只读本会话 run_dir / 同 sid 二次 409 跨会话不 409 + 闸门 fail-fast（未知 401、非法 400 不 spawn）/ orphan 按本 sid marker / 过期 401 后墓碑过龄同 sid 重建仍能续读清理。
