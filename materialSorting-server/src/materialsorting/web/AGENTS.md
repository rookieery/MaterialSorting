...
| `routes_ws.py` | **2026-08-20 自 server.py 拆出**。APIRouter：`WS /ws/solve` + `_terminate_solve_process` + `_SENTINEL`；从 runtime 取共享 executor/state；**US-011** `_parse_band` band 服务端校验（label `^g\d+$` / 存在母版 / quantities>0 / 硬警告形态需 ack；非法=结构化 error 早退 + **显式 ws.close()**——TestClient 下早退 return 不关 WS 则 client receive 永不抛 disconnect）+ `on_stage` 回调投 `{type:stage,stage:band}` |
...
| `solve_worker.py` | **US-025 新增**。顶层 `solve_worker(pieces_snapshot, gate_mm, solve_params, result_queue)` —— Windows spawn 可 pickle（无闭包、参数全 JSON）。子进程内 `build_instance(...) → 投 {kind:manifest}` → `instance.solve(config, progress=ProgressQueue)` → drain 出中间解投 `{kind:frame,report}` → 末尾投 `{kind:final,final}` 或 `{kind:error,message}`。所有投递纯 JSON，spyrrow 对象绝不跨进程；**US-011** `band` 参数 → `_build_band`（**进程内**同步跑 `waist_band.build_band_plan`，不 spawn 孙进程；失败=「成带失败」error 不投 manifest）+ `_write_band_artifact`（`OUT_DIR/band_runs/*.json`，写失败仅 warn）+ `_emit_placed` 展开单点（WB_ 组合片条目展开回成员 placement，三处帧/final 发射共享） |
...
- **StartPayload 新增可缺省 `band` 键**：`{enabled, label, ack?, time_budget?}`；缺省/null/{}/非 dict/enabled falsy = 关闭（旧行为逐字段不变，`test_solve_proc.py`/`test_ws_stop.py` 零改动全绿）。`time_budget` 是**内部旋钮**（测试/US-013 预演缩短成带预算，缺省 15s=`DEFAULT_BAND_TIME_BUDGET_S`；非法值静默回退 None），非 FR-1 前端契约键。
- **`_parse_band` 服务端校验（routes_ws，单一校验点）**：enabled 时 label 须匹配 `^g\d+$` + 存在于当前母版 + 该 g 码 quantities>0（demand 口径镜像 `build_pid_meta`：missing→1、显 0→0、sizeKey=`str(size)`/null→`'null'`）；成员最小边 <60mm 或长宽比 >6（硬警告形态，阈值 `_BAND_ACK_MIN_EDGE_MM`/`_BAND_ACK_MAX_ASPECT`，US-013 试用后可调）须 payload 显式 `ack:true`。非法抛 `ValueError` → ws_solve 发 `{type:error}` 早退（**不发 manifest**，与 build 失败同契约）+ **显式 `await ws.close()`**（TestClient 下早退 return 不关 WS 则后续 client receive_json 永不抛 disconnect → 测试挂死）。
...
- **进程模型（AC#4）**：band 子求解跑在 `solve_worker` 进程内（`_build_band` 同步调用 `build_band_plan`，**不 spawn 孙进程**——`_terminate_solve_process` 的 terminate 不级联孙进程）；stop/断开随 worker 进程 OS 级整体回收，无存活 python 子进程。band 失败（`BandError`/`ValueError`/几何异常）= 只投 `{kind:error,'成带失败:…'}` 不投 manifest（test_solve_proc.py:155 契约）。
...
- **测试**：`tests/test_waist_band_ws.py` 16 项（band 关闭四变体直收 manifest / 开启全链路 stage→manifest→末帧副本守恒+工件校验（`seed==zlib.crc32(b'1|g05')`）/ on-off manifest 逐字段一致 / 校验五参数 error 早退 / ack 放行 / DegenerateBand worker error / band 阶段 stop 无孤儿 / `exclude_labels` Item 层单元 / `time_budget` 旋钮）。
...
- **成对形态重试成带**（US-014 修订 `nesting_engine/waist_band.py`，根因：spyrrow 不按喂入序聚排，「输入 size-major 序 ⇒ 同码相邻」假设被 3 seed 实测否定 —— 2 个同码对散落 400mm+；同码全成对紧排解在解空间常见 ~50%/次）：固定派生 seed 序列重试取首个 `_pairs_complete` 解（确定性重放），6 次全败走 `_slot_fallback` 确定性槽位兜底（矩形类 fill ~100% 即最优、弧形腰片 ~40% 由 fill 下限拦截）；带内幅宽钳 `min(gate,1910)`（组合片须进主解条带）。实测 5336 3 seed 成对率 100%、fill 64~67%、bbox ~1180×1500。曾试「分码分块拼接」形态：成对 100% 但均值密度劣化 3.7pt（宽 1818×1271 组合片主解难包），已否决 —— 形态与密度需同时达标，有机紧排解两者兼得。
