...
| 文件 | 角色 |
| --- | --- |
| `server.py` | FastAPI app；路由 `GET /`、`mount /static`、`POST /export`、`POST /api/parse-dxf`（US-004）、`POST /api/commit-to-nesting`（US-010 + **US-002 多会话绑定**：读 `X-Session-Id`，带 sid → per-doc 快照挂会话不碰 default；无 sid → default reload）、`POST /api/session`（多会话 US-001：Header 解析 + `resolve(create=True)` + `SessionError`→JSONResponse 一行映射）、`GET /api/ptypes`（US-020）、`WS /ws/solve`；US-020 可 reload `_PIECES_STATE`（threading.Lock 保护）；`ThreadPoolExecutor(max_workers=6)` 求解桥 + 上传解析/commit 复用；**文件尾** `from .strategy import register_strategy_routes` 注册策略四路由（US-004，见下）+ `session_registry.start_scanner()`（US-001 会话过期 daemon 扫描）；commit 落盘 doc dict 带 `doc_id` 键（策略 start 定位母版原件）。**2026-08-20 拆分**（600 行硬限制，783→329）：state/线程池搬 `runtime.py`、parse 响应构造搬 `parse_payload.py`、WS solve 搬 `routes_ws.py`、`/`·ptypes·`/export` 搬 `routes_views.py`（APIRouter include，注册顺序不变）；**`_commit_to_nesting_sync` + 上传路由留守**（测试 monkeypatch `server_mod.UPLOADS_DIR` 依赖 server 命名空间 `__globals__`）；拆出符号全部 re-export（`_DOC_ID_RE` 2026-08-27 起改自 `sessions.SID_RE` re-export，同一编译对象）；**US-002（2026-08-27）**：`PIECES_INTERMEDIATE_NAME` + `_per_doc_intermediate(doc_id)` 路径约定 + commit 双写（详见下「多会话 US-002 关键约定」） |
...
| `routes_views.py` | **2026-08-20 自 server.py 拆出**。APIRouter：`GET /`、`GET /api/ptypes`、`POST /export`；`POST /api/band-preview`（2026-08-24 成带形态预览：`_parse_band` 单一校验点 + 主进程同步 `build_band_plan`，失败 200 `ok:false` 包络，响应无 `WB_`）+ `POST /api/prefix-preview`（2026-08-25 前缀组合形态预览同套路：`_parse_prefix` 校验 + `eligible_sizes→pick_prefix_size(seed 缺省 0)→build_prefix_plan`，成员带 `tag`=g 码，响应无 `PS_`）；从 runtime 取共享 state |
...
- **start**（202）：校验序 = 409（内存态非终态 **或** marker 在）→ 422（state 空 / doc 缺 `doc_id`——旧 intermediate 须重新 commit / `uploads/<doc_id>.dxf` 丢失）→ 400（mode ∉ {se,race}、minutes ∉ {10,20,30,60}、seed 非整数、band/prefix 非法）。config 写 `out/uploads/strategy_cfg_<stamp>.json`（9 键；可选键 `sizes`/`per_type`/`quantities` **仅 truthy 才写**——None 会被 `cli.config.load_config` 按类型错误拒；`band`（2026-08-22）/`prefix`（2026-08-25）经 `routes_ws._parse_band`/`_parse_prefix` 单一校验点（prefix 含 2+2 资格码，非法 → 400 早退防长跑空跑），合法开启才写 `{'enabled':true,...}`，null/enabled falsy 不写键）；`master_dxf` 必为 `resolve()` 绝对路径；`gate_mm` 请求值优先回退 state。
