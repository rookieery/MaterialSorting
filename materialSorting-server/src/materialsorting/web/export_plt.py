...
#   - Y 绘制平移（2026-08-31 用户定案）：旧导出 Y 贴 0 起画、门幅框线压在图纸
#     原点上。→ 整张图纸（门幅框/裁片/工艺线/表格）全体 Y + PLOT_LEAD_Y_MM
#     一起平移，框线左侧留出空纸边（首版 5mm 对齐生产件观感，用户反馈太短，
#     定案 = 唛架信息表格宽度 TABLE_W_MM=36mm）。纯绘制层位移：框在世界坐标
#     仍满幅 [0, gate]，框内相对几何/裁剪界/求解带/利用率全部不变；切割软件以
#     框线为布料范围、只裁框内内容，框外的纸边空白与表格不参与裁剪（用户口径：
#     这里只是绘制图纸）。
#
...
PLOT_LEAD_Y_MM = TABLE_W_MM    # Y 幅宽向整体绘制平移 = 表格宽 36mm（2026-08-31 用户定案）
...
# PLOT_LEAD_Y_MM 与旧内缩的区别：内缩改的是框的位置（框 [5,gate−5] vs 内容 [0,gate]
# → 裁片穿框）；平移是框+内容+表格整体位移（相对几何零变化，只是整张图离图纸
# 原点 36mm=表格宽，首版 5mm 被用户反馈太短），世界坐标层（求解/裁剪/密度）不动。
...
def write_marker_plt(world_pieces, *, width_mm: float, gate_mm: float, title: str,
    ...
    门幅框与裁剪界（2026-08-28 版师定案：输入幅宽即实际幅宽，见模块注释）：
      ...
      - 全体 Y + ``PLOT_LEAD_Y_MM`` 绘制平移（2026-08-31 用户定案：整张图纸一起
        离图纸原点 36mm=表格宽 TABLE_W_MM，首版 5mm 被反馈太短）。纯绘制层位移——
        世界坐标层（求解带/裁剪界/密度）与框内相对几何零变化，切割软件仍以框线
        为布料范围、只裁框内内容（PLT 只是绘制图纸）。
    ...
    **无需临时文件**），DXF 走 ezdxf 写盘读字节。空层跳过（``net_polygon`` 空则该层
    ...
    # 合一）。世界坐标给 [0, gate]，经 _plt_pt 全体 +PLOT_LEAD_Y_MM 平移后画在
    # [36, gate+36]——框线离图纸原点 36mm=表格宽，与裁片相对几何不变（同日定案
    # 的绘制平移）。
    # 恒为 width_mm：信息表格在排料图外围（2026-08-30 v2：不占排料区、
    # 不计入用料，切割时布上无此内容）。
