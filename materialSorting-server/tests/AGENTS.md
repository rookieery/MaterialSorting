...
| `test_cli_portfolio.py` | PC-002 `cli/portfolio` 控制器 + `run_config` portfolio 旗标：fake solve（`run_serial_portfolio(solve=...)` / monkeypatch `rc_mod.solve_pieces`，帧布局编码 seed/idx 验来源）—— incumbent banking（3 seed 交错轨迹取全局最大帧 + 来源字段 / 等值不覆盖 / 被 kill seed 峰值入账 + 回落不覆盖 / Ctrl-C 中断 seed 已收帧入账）；R0（seed2 中途达标 → killed=True + R0 kill_reason + seed3 不启动 + per_seed 无未启动记录 + 触发帧先入账 / 首帧即达标不空手 / 无 --target 跑满全队列）；portfolio 段与 best 语义（engaged 字段齐全 + best==incumbent；单 seed 无 --target 空段 + 旧语义 final 口径）；`load_controller_params` 矩阵（BOM 合法/不存在/非法 JSON/顶层非对象）；进度行（per-seed 新最优保留旧格式 / 跨 seed 反超恰一条 incumbent 行 / 首 seed 自我刷新不打 / echo=None 静默不影响 banking）；main() 端到端（R0 退出码 0 + R0 终局行 --quiet 也打 + result.json 落盘结构 / 单 seed 无旗标空段 + best==solve[0] / 多 seed best=incumbent 高于全部终值 / --target 越界与 --params 坏文件退出 1 不建 run_dir）；分层（portfolio 全模块不 import web）+ `python -m materialsorting.cli.portfolio` 零输出冒烟 |
