...
**US-002 段置换钉位（P0 灾难 −17.72pt 三因修复后的守卫路径）**：组合片在主解未自然锚定布头时，以 c1=a（组合片左缘）硬切线 + c2∈[b0, b0+flex] 软切线（候选 = {b0} ∪ 片棱边，取**跨线片最少**者）把布头段刚性重组 —— **A 组**（x-span ⊆ [a−eps, c2+eps]，组合片必入）平移 −a 钉到 x=0、**C 组**（max_x ≤ a+eps）右贴 A 右缘、**B 组**（min_x ≥ c2−eps）左贴 C 右缘；三组 x 区间两两不交 ⇒ 无新重叠，纯 x 平移不碰 y。跨线片（bd[0]<c2−eps 且 bd[2]>c2+eps）驱逐后 `reinsert_evicted` 三优先序回填：①组移 home 位 + `{0,1,5..300}mm` +x 微调梯 ②`_slide_touch` 右起滑贴（y 候选 = 已占棱边 4 类对齐 ∪ {0, 门幅−h}，x<0 钳壁）③尾端 +片宽+60mm 追加兜底（width_growth 记 stats，超 50mm warn）。**三守卫参数**（P0 教训单测锁死）：`skip_at_head=6.0`（组合片 min_x ≤ 6 已锚定即整体跳过 —— P0 实测 4/4 seed a=0.145~0.333mm ⇒ 钉位路径零成本）、`eps=5.0`（≥ d 包络外凸 ≈4mm，防贴壁片 bd[0]=−2 误判跨线）、`flex=400.0`（软切线弹性带）。终检 `constraints.validate` 全图复检 + y≤1910 复检，失败回滚置换前布局（LNS 纪律）；`pin_prefix_layout` 纯函数（入参快照不被改写，回滚=返回备份）。探针已知 bug 修复：slide 负 x 钳壁、cur_max 只算已置位片。
...
| `pin_prefix_layout` | `(placements, pid_meta, pieces_by_id, chunk, comp_rotation, comp_translation, prefix_idx, *, gate_nest, flex, eps, skip_at_head) → (placements, stats)` | US-002 编排纯函数：入参快照（副本操作）→ `expand_placements` 定位组合片成员 → `permute_pin` → `reinsert_evicted` → `constraints.validate` 全图复检 + y≤1910 复检，失败回滚返回置换前布局（stats['rolled_back']=True、'issues' 存因）；skip 路径原样返回；引擎层不 import cli（`_recheck_layout` 镜像 lns_bands.recheck_layout） |
