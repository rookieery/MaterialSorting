...
└── src/materialsorting/
    ...
    │   └── prefix.py                  US-001 起始端成套前后幅核心构造（eligible_sizes 2+2 资格码 / pick_prefix_size seeded 随机 / build_prefix_plan 4 片竖排贴靠 PS_* 组合片（BandChunk 同构，直接喂 expand_placements）；interleave 定稿 0 封闭腔；rot180 负坐标记账 tr=(xoff−b0,yoff−b1) 单测锁死；`python -m` 冒烟对拍 P0；禁 import web/cli，AST 守卫在 tests/test_prefix.py）+ US-002 段置换钉位/驱逐重插（permute_pin A/C/B 刚性重组三守卫 skip_at_head=6/eps=5/flex=400、reinsert_evicted 三优先序 home→nudge→slide→尾追加、pin_prefix_layout 编排含 validate 复检+回滚；P0 回归 4/4 seed 跳过 0.00pt）
    ├── web/                           FastAPI + WS 工作台（详见 agent-api-reference.md）
        ...
        ├── export_plt.py              HPGL/PLT 文本导出（自 export.py 拆出）：write_marker_plt + 全部 PLT 常量与裁剪/分块算子；内容按 Y≤输入门幅裁剪（2026-08-28 起单一口径）+ PD 分块 ≤10点/≤110B + 走纸引导；PLT 永不加文字
...
### `prefix.py` — US-001 起始端成套前后幅核心构造（资格码 + PS 组合片）+ US-002 段置换钉位 / 驱逐重插
...
**模块级常量：** `PREFIX_PID_PREFIX='PS_'`（泄漏哨兵：manifest/frame/final/前端/导出永不允许）/ `PREFIX_ORIENTATIONS=(0.,180.)` / `PREFIX_ORDERS=('interleave','grouped')` / `PREFIX_MEMBER_COUNT=4` / `SLIDE_STEP_MM=20.0` + `BISECT_ITERS=40` + `GAP_EPS_MM=1.0`（贴触判据，镜像 CHAIN_GAP_EPS_MM）/ US-002：`PIN_SKIP_AT_HEAD_MM=6.0` + `PIN_STRADDLER_EPS_MM=5.0` + `PIN_CUT_FLEX_MM=400.0`（三守卫，inspect.signature 单测锁死）+ `PIN_NUDGE_LADDER_MM=(0,1,5,10,20,50,100,200,300)`（回填微调梯）+ `PIN_WIDTH_GROWTH_WARN_MM=50.0`（尾追加警戒）+ `PIN_X_FLOOR_TOL_MM=0.5`（x≥0 地板容差）。
...
| `export_plt.py` | 329 | HPGL/PLT 文本（2026-08-20 自 export.py 拆出）：`write_marker_plt` / `_plt_frame_stats` / `_plt_pt` / `_plt_polyline` / `_clip_closed_y` / `_clip_open_y` / `_y_clip_point` + 全部 PLT 常量（撞机修正注释块整段搬移）；门幅框/裁剪均按入参 `gate_mm`（2026-08-28 起单一口径，不再 import 幅宽常量） |
| `prefix_accept.py` | 782 | **US-005 新增**。A/B 验收闭环 CLI（`python -m materialsorting.web.prefix_accept [--quick]`，结构镜像已删的 US-014 band_accept）：`load_accept_pieces`（5336 fail-fast）+ `run_arm`（solve_with_callback_proc 真实产品管线）+ `prefix_form` 形态四子判据（同码 2+2 / min_x≤6 / 相邻 y 交集>0 且缝隙≤1mm / rot 差≈180°±5°；成员识别按 pid —— exclude_pids 使 final 该 4 条只能来自 PS_ 展开）+ `p0_per_type`（P0 口径 per_type 逐码 d/tol，fail-fast）+ `frame_series_equal`（核心前缀逐帧硬判 + 帧数差 12% 相对护栏 —— 内容确定、截断帧位随机器速率漂移）/ `final_best_equal`（best-so-far 密度重现 ≤0.1pt；final 快照为信息字段）/ `artifact_replay_equal`（构造回放，排除 ts/stage_elapsed/pin/band_pos/width_mm）+ `export_verify`（PNG/R12-DXF/PLT + 「导出跳过」warning 哨兵 + b'PS_' 字节 grep）+ `run_all` 编排（off/on × seed、双开档 band_only vs band+prefix 带位另报列、确定性重跑四对拍、导出 on/off）；报告落 `out/config_runs/_probes/prefix_accept_report.json`；工件 diff 按 `*_PS_{front}+{back}@*.json` mtime 快照（`MS_OUT_DIR` 环境变量优先测试隔离）；A/B 报告文档 `.docs/business/起始端成套前后幅_AB验收报告_US005.md` |
