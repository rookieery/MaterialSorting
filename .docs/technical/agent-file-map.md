...
└── src/materialsorting/
    ...
    ├── web/                           FastAPI + WS 工作台（详见 agent-api-reference.md）
        ...
        ├── routes_ws.py               APIRouter（2026-08-20 自 server.py 拆出）：WS /ws/solve + _terminate_solve_process + _SENTINEL；US-011 band 服务端校验 _parse_band（label ^g\d+$ / 存在母版 / quantities>0 / 硬警告形态（min 边<60mm 或长宽比>6）需显式 ack:true；非法=结构化 error 早退+显式 close；US-013 缺 ack 抛 BandAckRequired（ValueError 子类，WS 路径 str(e) 行为不变，预演路由据此回 hard_warning 标记））+ on_stage 回调（{type:stage,stage:band,fill_pct,bbox,fallback,elapsed}，manifest 前唯一一次）→ run_solve 传 band=band_cfg；US-015 fillers 护栏并入 _parse_band（数组项 ^g\d+$ / ≠主码 / 去重后 ≤_BAND_MAX_FILLERS=3 / 各自存在母版 + quantities>0；填料不做硬警告 ack —— 填料本就是小片；返回 dict 恒含 fillers 键，缺省 [] 旧 payload 行为不变）
        ├── routes_band.py             US-013 APIRouter：POST /api/band/preview（FR-7 成带预演回显 —— executor 线程 5s 预算 build_band_plan（_resolve_d_tol/build_pid_meta 与 solve_worker._build_band 同口径）→ 200 {ok,fill_pct,bbox,elapsed,break_even:[62.4,63.6]}；几何失败 200 {ok:false,error}=结果数据不阻塞确认；400/409/422 {error}，硬警告 422 附 hard_warning:true；不落 band_runs 工件；常量 BAND_PREVIEW_TIME_BUDGET_S/BAND_BREAK_EVEN_PCT；US-015 body 可带 band.fillers —— 与 WS _parse_band 同校验（含主码 → 422「不可包含主 g 码」不设 hard_warning），fill 分子 = 腰 + 填料面积）
        ...
        ├── solve_worker.py             US-025 子进程入口（顶层 solve_worker，spawn 可 pickle；子进程内 build_instance + solve，仅 JSON 数据跨进程；US-006 solve_params 原样含 solver_opts；US-011 band 参数 → _build_band 进程内同步跑 waist_band.build_band_plan（不 spawn 孙进程：terminate 不级联孙进程，同步调用随本进程 OS 级回收，stop 后无存活 python 子进程；失败=成带失败 error 只投 error 不投 manifest）+ _write_band_artifact（OUT_DIR/band_runs/*.json，写失败仅 warn）+ _emit_placed 展开单点（三处帧/final 发射共享：WB_ 组合片条目 expand_placements 展开回成员 placement，WB_ 永不跨进程）；US-015 _build_band 透传 fillers/filler_ds（逐 filler `_resolve_d_tol`），主实例 exclude_labels={label,*fillers} —— 填料副本同扣减路径，manifest on/off 一致性不变）
        ├── band_accept.py              US-014 A/B 验收闭环 CLI（python -m materialsorting.web.band_accept [--quick]；不进 server 路由零副作用）：密度 A/B（off/on × seeds，劣化 ≤1.0pt）+ 形态（pair_adjacency 边距口径成对率 100% + band_span 刚性不变量）+ 确定性（frame/final 签名 + band_runs 工件回放对拍）+ 导出（三格式 + 「导出跳过」WB_ 泄漏哨兵 + b'WB_' 字节 grep）→ 报告 out/config_runs/_probes/band_accept_report.json；EXIT 0/1
...
### `waist_band.py` — US-009 腰头成带核心（v2 构造性链构造 + 组合片 + 展开；US-015 填料混带）

依据 `business/腰头成带_落地方案.md` §2（2026-08-21 定稿）。**v2（2026-08-21）求解路径 = 构造性链构造**（替换 v1 spyrrow 带内子求解 + US-014 成对重试，均已删；根因 = strip 目标是最短用布 X 而非贴触，v1 产出 48.2% 对角阶梯坏带形反噬主解 −3.81pt）：版师形态「N 条单副本异码链」（`chain_k` = 每码第 k 副本一条链，链内 size **降序**）—— `_chain_nest` 构造性滑移贴靠贪心（首片锚定原点，后续 rot{0,180}×5 y 对齐 × `_slide_touch` 右起 20mm 粗扫 + 40 次二分到贴触，取 union bbox 增长最小；shapely-only、无 RNG、毫秒级）→ `_flip_chain` 整链点对称翻转（成员 rot+180/tr 取负 —— 合法布纹旋转无镜像）⇒ **开口朝左 + 最大码在最右** → 形态自检（`_chain_gap` ≤ `CHAIN_GAP_EPS_MM`=1.0 贴触口径、`_opening_side` ∈ {left,flat}、最大码质心 x 最大）→ `_stack_chains` 多链堆叠（不翻链保开口方向）+ 带高守卫 ≤ `min(gate_nest, PLOT_SAFE_MAX_Y_MM)` → **US-015 `_fill_gaps`** 填料塞隙（见下）→ 成员**原始轮廓**@带内位 `unary_union` → closing 焊接连通 → `erode`（纯腰 `d_g` 逐字节不变；混带带内碰撞补腐蚀到 `BAND_INNER_D_MM`，组合片外轮廓仍 `d_g`）→ `_clean_polygon` → 平移归一化（记录 offset），整带单组合片（`WB_*` pid）投主解；帧发射前 `expand_placements` 展开回成员 placement。**禁 import web**（单一真相源，web US-011 接线 / 未来 CLI 共用；AST 守卫在 `tests/test_waist_band.py`）。

**US-015 填料塞隙（`_fill_gaps`，v1.1 混带）**：`fillers`（任意 g 码 ≤3、≠主码；护栏在 `routes_ws._parse_band`，本层兜底 0 副本/同主码 ValueError）全部副本在链堆叠后贪心塞带内空隙 —— 每副本 y 候选 = 已排 bbox 棱边 ∪ 各已放几何 bbox 棱边（对齐填料底/顶边）× rot{0,180} × 双滑向（`from_left` 到达朝左凹口），x 走右起粗扫 + `FILLER_BISECT_ITERS=24` 二分；代价 = `_bbox_area`（bbox 并集解析式，免 unary_union）；`prep()` 预处理几何 bbox 预筛。**混带内距口径**：带内碰撞多边形在成员基准 d（主码 `d_g` / 填料 `filler_ds[label]` 逐个裁定）之上补腐蚀到 `BAND_INNER_D_MM=2.0`（版师口径 2~4mm 保守端 —— 肋间切口端部开到小件可入宽度）；erode(e) 贴触 ⇒ **原轮廓允许交叠 ≤2e**（erosion = 允许交叠深度，非分离）；erode 失败回退原轮廓（碰撞超集安全方向）。`BandChunk.fillers` 进 to_dict（band_runs 工件回放）。5336 实测：g05+g07 混带 fill 79.5%→86.5%、A/B 3 seed 均值劣化 −0.534pt verdict=accept。

**模块级常量：** `COMPOSITE_PID_PREFIX='WB_'`（泄漏哨兵）/ `COMPOSITE_ORIENTATIONS=(0.,180.)`（FR-8 成带后整块不再旋转，不带 tol 抖动）/ `MAX_COMPOSITE_VERTICES=600` + `SIMPLIFY_TOLERANCE_MM=0.05` / `DEFAULT_BAND_TIME_BUDGET_S=15`（deprecated 保留）/ `FILL_FLOOR_PCT=45.0`（低于即 fail-fast，禁止无声兜底）/ `WELD_RADIUS_MM=1.0` + `WELD_RADIUS_MAX_MM=512.0` / `PAIR_ADJ_EPS_MM=10.0`（成对相邻边距判据，band_accept 验收同口径单一真相源）/ `CHAIN_GAP_EPS_MM=1.0`（贴触口径）/ `CHAIN_SLIDE_STEP_MM=20.0` + `CHAIN_BISECT_ITERS=40` + `CHAIN_Y_ALIGNS`（链构造滑移参数）/ `FILLER_BISECT_ITERS=24`（填料二分降精度，g07+g08 填充 28s→4.9s）/ `BAND_INNER_D_MM=2.0`（混带带内碰撞腐蚀下限）。

| 函数 / 类 | 签名 | 说明 |
|------|------|------|
| `build_band_plan` | `(pid_meta, pieces_by_id, *, label, seed, gate_nest=NEST_GATE_MM, d_g=0.4, tol_g=3.0, fillers=(), filler_ds=None, time_budget=15, fill_floor=45.0) → BandChunk` | 成员 = pid_meta 中该 label 且 demand>0 全部副本（链构造碰撞用已腐蚀 polygon 不二次腐蚀、orientations=`discretize_orientations(tol_g)`）；**US-015**：`fillers` 各码 demand>0 副本进 `_fill_gaps` 塞隙（filler 碰撞 d 取 `filler_ds.get(label, d_g)`），0 副本/同主码 ValueError 兜底；填料空 = 纯腰 v2 路径逐字节不变；0 副本 ValueError、总副本 1 `DegenerateBand`、守恒失败/解散落/低 fill `BandQualityError`；union 用 **pieces_by_id 原始轮廓**（union bbox minx/miny = offset，减号进展开式）；真实 DXF 自交轮廓经 `buffer(0)` 修复（`_valid_geometry`，实测 M1787 g05 必需）；`time_budget`/`fill_floor` deprecated 保留（兼容 import） |
...
| `BandChunk` | dataclass | `pid/label/polygon(归一化 erode 后)/offset/members(逐副本带内位 size-major 序)/fill_pct/bbox/seed/d_g/tol_g` + **US-015 `fillers`**（进带填料码元组，进 to_dict —— band_runs 工件回放）+ `to_dict()`（纯几何 JSON，同 seed 两跑 `json.dumps` 相等 —— 不含 wall-clock） |
...
| `band_accept.py` | 589 | US-014 **新增**。A/B 验收闭环 CLI（`python -m materialsorting.web.band_accept [--quick]`，不进 server 路由零副作用）：判据①密度 A/B（off/on × seeds，均值劣化 ≤`DENSITY_ACCEPT_PT`=1.0pt）+ ②形态（`pair_adjacency` 同码成对相邻率 =100% —— **边距口径**（shapely distance ≤ `PAIR_EPS_MM`=waist_band.`PAIR_ADJ_EPS_MM` 单一真相源；PRD 中心距公式为同朝向并排特例，FR-8 头尾翻转 180° 成对中心距可达 L 量级而物理边距 ~0，实测 g05_34 对 1.5mm/713mm）+ `band_span` ≤ stage bbox+2 刚性展开不变量；off 臂 g05 成对率对照）+ ③确定性（`frame_signature`/`final_signature` 忽略 wall-clock 逐帧对拍 + `frame_series_equal` **尾帧感知比较**：核心前缀（min(n)−1 帧）逐帧相等 + 帧数差 ≤`FRAME_TAIL_TOLERANCE`=8（wall-clock 预算截断使截止快照可漂 1~3 帧，实测 2005/2008 形态 final 仍相等；核心帧分歧或大面积漂移即 FAIL）+ `artifact_replay_equal` band_runs 工件回放（排除 `band_elapsed`））+ ④导出（`export_verify` PNG/R12-DXF/PLT + root logger「导出跳过」哨兵 + DXF AC1009/POLYLINE + `b'WB_'` 字节 grep，on/off 双臂）；verdict `{density,form,determinism,export,conclusion}` → 报告 `out/config_runs/_probes/band_accept_report.json`；跑批口径三差异（uploads 源/web 默认 ⇒ g05 d=0.0/无 kill）只与同配置 off 对照 |
