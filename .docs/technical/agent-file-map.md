...
## cli/ — 配置驱动求解 CLI（PRD config-driven-solve-cli，US-001 起；串行 Seed Portfolio PC-001 起进程化求解）
...
| `pipeline.py` | US-002 `commit_from_config(cfg, run_dir)` 独立 commit 管线：镜像 `web/server._commit_to_nesting_sync` 同一编排链（`collect_pieces_with_details` → `assign_codes` → `write_piece_dxf` 切 `{label}_{size}.dxf` + `pieces_manifest.json` 到 `run_dir/pieces/` → `load_nest_pieces` → intermediate 落 `run_dir/pieces_intermediate.json`，schema v2 与 web `web.solver.load_pieces` 互读）。**与 web commit 的刻意差异**：不写 `.bak`（时间戳 run_dir 天然全新）、顶层省略 `label_representatives`（web 专属缩略图）、`gate_mm` 写 `cfg.gate_mm`（配置驱动 = 该 run 密度分母；web 固定 `GATE_MM=1980`）；piece 条目经 `_piece_record` 与 server 逐字段一致（含 rounding 位数：点 3 位/法线 4 位/bbox 2 位/area 1 位）—— server schema 变更须同步镜像（web 零改动约束下无法抽共享函数）。`new_run_dir(run_name)` 创建 `CONFIG_RUNS_DIR/<name>_<YYYYMMDD-HHMMSS>` 时间戳目录（保留历史互不覆盖；run_name 清洗归 `run_config`）。commit 只消费 `cfg.master_dxf`/`cfg.gate_mm`（sizes/quantities/per_type 是求解期参数，commit 不过滤全量切片）。**US-003 `solve_pieces(cfg, run_dir, *, seed, time_budget=None, on_progress=None, should_stop=None)`（PC-001 起进程化）**：读 run_dir intermediate → 主进程 `web.solver.build_instance` 取 meta（`demand_sum` 校验 / `total_area` / `n_items` / `n_eroded`，单一真相源不在 CLI 复制 demand 查询）→ `solve_with_callback_proc` **多进程版**求解（spyrrow 对象不可 pickle，子进程内重建 instance 是固有秒级成本；换来 terminate 能力）；密度双口径换算在 `solve_with_callback_proc` 内经同一 `_apply_density_dual` 完成（CLI 不复制公式）：`density`=原面积口径 `total_area/(width*gate)`、`density_sparrow`=sparrow 自报。**逐帧落盘（run_dir）**：`curve_s{seed}.json` = `[{elapsed, phase, density, density_sparrow, width_mm}]`（**增量 append**，exploring 期 ~5ms 一帧、整文件重写是 O(N²) 磁盘写；seed 结束/中断 finally 补 `]` 收口成合法 JSON）+ `best_frame_s{seed}.json` = 该 seed 最优帧完整 `placed_items`（`density` 严格大于才覆盖写；含 `frame_index` 供 portfolio incumbent 标来源）。**`should_stop(frame) -> bool|str`**：每帧评估，真值 → 帧回调内就地 `terminate()` 杀子进程、以该 seed **best-so-far 帧**交付（记录追加 `killed=True` + `kill_reason`=返回字符串或 `'should_stop'`；此路径不做 `placed==Σdemand` 校验 —— 中间帧允许未放满）；恒 False/不传 = 跑满预算。正常路径末解校验 `placed==Σdemand` 不齐抛 RuntimeError，返回 solve 指标 dict（seed/n_items/n_eroded/total_area_mm2/width_mm/real_density/density_sparrow/placed_items/elapsed —— **字段与旧 threading 版完全一致，零回归**）。web.solver 走**函数内延迟 import**（模块级不 import web，保 `python -m materialsorting.cli.pipeline` 导入冒烟零副作用） |
| `run_config.py` | US-003 `ms-run-config` 入口（console_script 与 `python -m materialsorting.cli.run_config` 等价）：argparse（`config` 位置参数 + `--name RUN_NAME` 覆盖 run_name + `--time N` 覆盖单轮时长（冒烟用） + `--quiet`）；run_dir = `new_run_dir(清洗后的 run_name)`（缺省 = 配置文件 stem，Windows 非法字符与控制字符 → `_`，空回退 `run`）；逐 `cfg.seeds` **串行** `solve_pieces`（**US-004 多 seed 语义**：commit 仅一次、每轮重建 build_instance 复用同一份 commit 产物；只串行不并行；种子不要求连续，`[0, 42]` 合法供复现历史 seed 对比；多 seed 启动即打印预计总时长 `len(seeds)×time`（不含解析/切片）+ 每轮「第 i/N 轮（seed=s）」标记（`--quiet` 抑制轮标记））；进度仅打「原面积口径新最优」帧 + 30s 心跳（`--quiet` 全抑制，最终汇总保留）；落 `run_dir/result.json`（config 回显 —— `time` 为 `--time` 覆盖后**生效值** + commit 摘要 + solve 指标数组 + **`best`** = per-seed 解中 `real_density`（原面积口径）最大者的完整指标 dict，并列取先执行者 —— 消除单 seed 随机性对配置评估的干扰）；多 seed 末段先打「各 seed real_density」一行 + 「best = seed N」一行；stdout 末行人类可读汇总取 **best**（real_density（原面积口径）/用布长度/片数/耗时/run_dir 完整路径 —— 单 seed 时 best=唯一解，输出与 US-003 逐字一致）；`main()` 首行 `sys.stdout/stderr.reconfigure(encoding='utf-8')` 防 Windows GBK 乱码（非常规流无 reconfigure 则跳过）；退出码 **0 成功 / 1 配置或管线失败 / 2 求解失败 / 130 Ctrl-C 中断**。**PC-001 逐 seed 落盘**：每轮 `solve_pieces` 完成即重写当前 `result.json`（结构同终态，solve 数组为已完成轮），Ctrl-C 时 run_dir 已持有已完成轮的 curve_s*/best_frame_s*/result.json（`KeyboardInterrupt` 捕获 → stderr 中断说明 + 退出码 130，不打 traceback） |
...
| `ms-run-config` | `cli.run_config:main` | 配置驱动排料一条命令（commit → 求解 → result.json，US-003；`out/config_runs/<name>_<时间戳>/`） |
