...
└── src/materialsorting/
    ...
    ├── web/                           FastAPI + WS 工作台（详见 agent-api-reference.md）
        ...
        ├── routes_views.py            APIRouter（2026-08-20 自 server.py 拆出）：GET / + GET /api/ptypes + POST /api/band-preview（2026-08-24 成带形态预览）+ POST /api/prefix-preview（2026-08-25 前缀组合形态预览）+ POST /export（经 export 门面）
        ├── routes_ws.py               APIRouter（2026-08-20 自 server.py 拆出）：WS /ws/solve + _terminate_solve_process + _SENTINEL；US-011 band 服务端校验 _parse_band（label ^g\d+$ / 存在母版 / quantities>0；非法=结构化 error 早退+显式 close；返回 {'label': str}；**2026-08-22 简化**：硬警告 ack（BandAckRequired）与 US-015 fillers 护栏已删，校验收敛为三项，本函数成为 WS 与 /api/strategy/start 两处共用的**单一校验点**）+ on_stage 回调（{type:stage,stage:band,fill_pct,bbox,fallback,elapsed}，manifest 前唯一一次）→ run_solve 传 band=band_cfg；**US-003 _parse_prefix**（_parse_band 同模式单一校验点：front/back 匹配 ^g\d+$ 且存在于母版且 front≠back；须存在 ≥1 资格码 eligible_sizes —— 无资格码=结构化 error 早退+显式 close，文案指路「请在数量矩阵把所选码前后幅配成 2+2」；返回 {'front','back'}，载荷多余键如 size 静默忽略）+ on_stage 泛化键白名单转发（fill_pct/bbox/fallback/elapsed/size/holes，prefix stage 复用）+ run_solve final 消息透传 final.prefix 段
...
| `routes_views.py` | 350 | APIRouter（2026-08-20 自 server.py 拆出）：`GET /`、`GET /api/ptypes`（含 `_LABEL_REPRESENTATIVE_FIELDS`）、`POST /api/band-preview`（2026-08-24 成带形态预览：`_parse_band` 单一校验点 + 主进程同步 `build_band_plan`，失败 200 `ok:false` 包络，成员 polygon 带内归一坐标，响应无 `WB_`）、`POST /api/prefix-preview`（2026-08-25 前缀组合形态预览同套路：`_parse_prefix` 校验 + `eligible_sizes→pick_prefix_size(seed 缺省 0)→build_prefix_plan`，成员带 `tag`=g 码，响应无 `PS_`）、`POST /export`（PNG/DXF/PLT，经 `.export` 门面）；从 runtime 取共享 state；契约详见 agent-api-reference.md 预览专节 |
| `routes_ws.py` | 329 | APIRouter（2026-08-20 自 server.py 拆出）：`/ws/solve` WS 端点 + `_terminate_solve_process` + `_SENTINEL`；US-011 band 服务端校验 `_parse_band`（label ^g\d+$ / 存在母版 / quantities>0 → `{'label': str}`；**2026-08-22 起 WS 与 /api/strategy/start 共用的单一校验点**，ack/fillers 护栏已删）；**US-003 `_parse_prefix`**（band 同模式：front/back ^g\d+$ / 存在母版 / front≠back / eligible_sizes ≥1 资格码，无资格码=error 早退+显式 close 文案指路数量矩阵；返回 `{'front','back'}`）；on_stage 泛化键白名单转发（+size/holes 供 prefix stage）；final 消息透传 final.prefix；从 runtime 取共享 executor/state |
