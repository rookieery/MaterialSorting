...
└── src/materialsorting/
    ...
    │   ├── waist_band.py              US-009 腰头成带核心（build_band_plan/expand_placements/BandChunk；US-014 修订成对形态重试；禁 import web）
    ...
    ├── web/                           FastAPI + WS 工作台（详见 agent-api-reference.md）
        ...
        ├── routes_ws.py               APIRouter（2026-08-20 自 server.py 拆出）：WS /ws/solve + _terminate_solve_process + _SENTINEL；US-011 band 服务端校验 _parse_band（label ^g\d+$ / 存在母版 / quantities>0 / 硬警告形态（min 边<60mm 或长宽比>6）需显式 ack:true；非法=结构化 error 早退+显式 close；US-013 缺 ack 抛 BandAckRequired（ValueError 子类，WS 路径 str(e) 行为不变，预演路由据此回 hard_warning 标记））+ on_stage 回调（{type:stage,stage:band,fill_pct,bbox,fallback,elapsed}，manifest 前唯一一次）→ run_solve 传 band=band_cfg
        ├── routes_band.py             US-013 APIRouter：POST /api/band/preview（FR-7 成带预演回显 —— executor 线程 5s 预算 build_band_plan（_resolve_d_tol/build_pid_meta 与 solve_worker._build_band 同口径）→ 200 {ok,fill_pct,bbox,elapsed,break_even:[62.4,63.6]}；几何失败 200 {ok:false,error}=结果数据不阻塞确认；400/409/422 {error}，硬警告 422 附 hard_warning:true；不落 band_runs 工件；常量 BAND_PREVIEW_TIME_BUDGET_S/BAND_BREAK_EVEN_PCT）
...
### `waist_band.py` — US-009 腰头成带核心（带内聚排 + 组合片 + 展开；US-014 修订成对形态重试）

依据 `business/腰头成带_落地方案.md` §2（2026-08-21 定稿）。腰头 g 码裁片在全幅（钳 `min(gate, PLOT_SAFE_MAX_Y_MM)`=1910 —— 组合片须进主解条带）带内独立小求解聚排，**成对形态重试选解**（US-014：固定派生 seed 序列取首个 `_pairs_complete` 解；根因 = spyrrow 不按喂入序聚排，原「单次全量求解 + 输入 size-major 序 ⇒ 同码相邻」假设被 3 seed 实测否定、2 个同码对散落 400mm+；成对形态出现率实测 ~50%/次）→ 成员**原始轮廓**@带内位 `unary_union` → closing 焊接连通（恒 ⊇ 原 union）→ `erode(d_g)` → `_clean_polygon` → 平移归一化（记录 offset），整带单组合片（`WB_*` pid）投主解；帧发射前 `expand_placements` 展开回成员 placement。**禁 import web**（单一真相源，web US-011 接线 / 未来 CLI 共用；AST 守卫在 `tests/test_waist_band.py`）。

**模块级常量：** `COMPOSITE_PID_PREFIX='WB_'`（泄漏哨兵）/ `COMPOSITE_ORIENTATIONS=(0.,180.)`（FR-8 成带后整块不再旋转，不带 tol 抖动）/ `MAX_COMPOSITE_VERTICES=600` + `SIMPLIFY_TOLERANCE_MM=0.05` / `DEFAULT_BAND_TIME_BUDGET_S=15` / `FILL_FLOOR_PCT=45.0`（低于即 fail-fast，禁止无声兜底）/ `BAND_NUM_WORKERS=1`（**确定性**：实测同 seed num_workers=1 与 =4 结果不同）/ `WELD_RADIUS_MM=1.0` + `WELD_RADIUS_MAX_MM=512.0` / `PAIR_ADJ_EPS_MM=10.0`（成对相邻边距判据，band_accept 验收同口径单一真相源）/ `BAND_MAX_TRIES=6`（成对形态重试次数，预算均摊 `max(1, total//6)`）。

| 函数 / 类 | 签名 | 说明 |
|------|------|------|
| `build_band_plan` | `(pid_meta, pieces_by_id, *, label, seed, gate_nest=NEST_GATE_MM, d_g=0.4, tol_g=3.0, time_budget=15, fill_floor=45.0) → BandChunk` | 成员 = pid_meta 中该 label 且 demand>0 全部副本（Item 用已腐蚀 polygon 不二次腐蚀、orientations=`discretize_orientations(tol_g)`）；**US-014 成对重试**：`BAND_MAX_TRIES` 次独立解（seed=`band_seed_for(band_seed,'try{k}')`）取首个 `_pairs_complete`（同 pid 最近邻边距 ≤10mm）解，全败走 `_slot_fallback`（确定性 bbox 槽位：构造性成对，矩形类 fill ~100%、弧形腰片 ~40% 由 fill 下限拦截）；0 副本 ValueError、总副本 1 `DegenerateBand`、守恒失败/解散落/低 fill `BandQualityError`；union 用 **pieces_by_id 原始轮廓**（union bbox minx/miny = offset，减号进展开式）；真实 DXF 自交轮廓经 `buffer(0)` 修复（`_valid_geometry`，实测 M1787 g05 必需） |
