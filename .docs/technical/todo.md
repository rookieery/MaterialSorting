...
- [x] 图形预览区拆除（ParsedPiecesView 组件+测试删除：卡片点击放大与矩阵行头缩略图弹同一 PieceZoomModal，交互冗余且挤占矩阵一屏看全；PreviewPage 右侧主体收敛为 QtyMatrix + 空态文案改缩略图放大指引；QtyMatrix repPiece 返回 {piece,size}、缩略图 openZoom 改传 rep.size 所见即所放大（顺修 label 缺 activeSize 片时点击静默无反应）；style.css 删 .parsed-pieces-*/.piece-grid/.piece-card 系列（保留 PieceZoomModal 复用的 .piece-card-label）；previewTour parsed 步文案改矩阵+缩略图描述 + TOUR_VERSION '2'→'3'；测试同步 PreviewPage 改写 / QtyMatrix 缩略图拆 2 项 / previewTour 版本断言；文档同步 agent-component-map + business-overview） — completed on 2026-08-16 16:55
