...
| POST | `/api/extreme/start` | extreme US-002：spawn `ms-run-config --extreme` 子进程启动极限长跑（202）；载荷 `{time_total_s, seed?, gate_mm?, sizes?, per_type?, quantities?}`（**无 band/prefix** —— 在场即 400）；**与 /api/strategy/start 同会话状态槽单飞互斥（409 双向）、跨会话独立**；恢复会话 intermediate 数据源分支同策略（状态文件 US-005，2026-09-12） | `strategy.extreme_start` |
...
| POST | `/api/machine/solve` | **机器对接 US-002（2026-09-21）任务提交**：multipart `file`（母版 DXF 二进制 ≤20MB，同 /api/parse-dxf 上限）+ `config`（JSON 字符串：`gate_mm` 必填 int>0 / `run_mode` ∈ normal·advanced·extreme 缺省 normal / `sizes` int[] / `per_type` {g码:{d,tol}} / `quantities` {g码:{码号:int≥0}} / `client_ref` str≤128 幂等键；**time/seeds/band/prefix 在场 400 —— 运行参数 MS 侧烘焙**）→ 铸任务 sid（'m'+时间戳+rand，即 task_id）→ server parse+commit 管线复用（延迟 import；快照只挂本任务 sid **不碰 default `_PIECES_STATE`**；坏 DXF → 422 中文）→ spawn 三档映射（normal=plain `--time 180` 单 seed 0 / advanced=`--strategy race --time 1200` / extreme=`--extreme --time 7200`，单一真相源 `machine.RUN_MODE_SPECS`）→ marker `.web_strategy_active_<sid>.json` 恰 5 键 mode='machine' → `202 {task_id, run_name, started_at}`（run_name=`machine_<task6>_<rand6>`）；同 `client_ref` 在飞 → 409 返回既有 task_id | `machine.machine_solve`（server.py 文件尾 strategy 之后 `register_machine_routes`） |
| GET | `/api/machine/solve/{task_id}/status` | **机器对接 US-003（2026-09-21）状态轮询**（2s 级；`machine._machine_status`）：→ `{state, mode:'machine', run_mode, total_budget_sec, elapsed_sec, incumbent, current, per_seed, error, exit_code}` 恰 10 键，**无 placed_items 控载荷**，进度源只读 result.json / best_frame_s*.json（**绝不读运行中 curve_s*.json**）；`incumbent` 四键 `{density,width_mm,seed,frame_index}` = **物理毛版包络口径**（`solver._apply_density_dual` 单一权威，与 result.json 一致，不可与历史 erode 口径混比），portfolio.incumbent 优先（race/se 档）→ 运行中 / plain（normal 档，portfolio 段恒空）回落各 best_frame 边车 density 最大帧；状态机 starting→running→done\|stopped\|error（复用 `_status_common(sid=,gate=False)` 状态推进写回内存态；error 带 stderr 尾 2000 字符）；**不走会话闸门** —— 会话被 TTL 逐出后仍 200（`_STRATEGY_STATES` 内存态路径），轮询即活性（会话在则 `touch` 刷 last_active）；orphan（MS 重启后 marker 遗留、内存态空）附加 alive/pid 键：pid 存活可 stop，pid 死超 30s 宽限 → error，run_mode/total_budget 经 machine_cfg time 反查恢复 | `machine.machine_solve_status` |
...
> 机器对接 `/api/machine/*`（YL 排料对接二期 2026-09-21 立项 + 三期 2026-09-22）：US-001 骨架 + US-002 `POST /api/machine/solve` + US-003 `GET/POST /api/machine/solve/{task_id}/status|stop|result` + US-004 `POST /api/machine/export` + US-005 `DELETE /api/machine/solve/{task_id}` + 三期 US-001 `GET /api/machine/solve/{task_id}/state-file` **六端点全部注册**（逐端点契约见上表行；**完整对接契约专节见下「机器对接 /api/machine/* — YL 后端对接契约」—— YL 侧开发者可仅凭该节完成对接**）。机器任务 = strategy 家族第三成员（`mode='machine'`，每任务独立 'm' 前缀 sid —— 复用 `_STRATEGY_STATES` 状态槽 / marker / run_dir 发现 / 树杀 / run 存活钉住骨架 + US-003 起 `_status_common`/`_stop_common`/`_result_common` 三公共实现参数化（keyword-only `sid`/`gate`/`drift`，缺省 = 策略/极限旧语义零变化），浏览器工作台会话不受扰）。六端点公共约定：X-Machine-Token 认证先行（`MS_MACHINE_TOKEN` 设置时缺失/错误 → 401）；task_id 格式非法（不满足 sessions.SID_RE）→ 400（marker 路径拼接安全闸）、未知任务 / 外族（非 machine）状态槽 → 404。US-005 另含 machine_* run_dir mtime>7 天机会式清理（每次 solve start 触发，非 machine 前缀不动）。
...
- 成功：文件字节流，`Content-Disposition: attachment; filename="<ascii>"; filename*=UTF-8''<quoted-cn>`
  ...
  - PLT（毛版）：`fmt='plt-clean'`（2026-08-31；当日由「净版」更名，协议值不变）→ `write_marker_plt(..., clean=True)`，形态对齐生产参考件 `data/PC-20250508NJIF_5028-1#_29223513.plt`：① 带表格时唛架**左右各一份同内容表格**（右表原位不动；左表 = 同一 `info_table_polylines` 以 `table_x0=-(TABLE_GAP_MM+TABLE_W_MM)` 再画一份，key 带落小 x 侧与右表阅读方向一致、无镜像文字，value 带外缘与门幅左边框共线），X 走纸引导随之扩为 `PLOT_LEAD_X_MM+TABLE_GAP_MM+TABLE_W_MM=56mm`（左表世界 x∈[−36,0] → 绘制 x≥20），PS 纸长按扩后 lead 计算；② 正文每片**只画毛版轮廓（polygon）+ 尺码\*数量标注 + 最外层刀口**（2026-09-11 起按用户需求加入，画法与全量版同款 `_notch_lines` 单一真相源但 `edge_only` 过滤——仅 notch 点距毛版 polygon 边界 ≤`_NOTCH_EDGE_MAX_MM`=2mm 的**贴边裁剪对位刀口**，净版边定位剪口（距边=缝份 5~20mm）与片内剪口不画，五母版实测双峰 ≤0.062mm vs ≥3.17mm、分离带取 2；经 `_LAYER_NOTCH` 桶输出层序仍在轮廓后标注前；参考件正文无任何工艺线属其自身口径；标注几何/字高 10mm/字库与全量版全同，仅去杆+箭羽），net_polygon/internal_lines/布纹杆羽不画。文件名后缀 `_clean`（ascii）/`_毛版`（中文，更名前一日为 `_净版`）；`clean=False` 输出与旧版逐字节一致（零回归红线）。
...
| 429 | 活跃会话数满且 sid 未注册（US-002） | `{"code":"session_limit","error":"当前使用用户过多（最多 6 人同时在线），请稍后尝试"}` |
...
| 活跃会话数已满（`MS_SESSION_MAX`，缺省 6） | 429 | `{"code": "session_limit", "error": "当前使用用户过多（最多 6 人同时在线），请稍后尝试"}` |
...
1. **容量闸门**：`MS_SESSION_MAX`（env，缺省 6）仅计活跃会话（default 不占额）；已有会话重复 POST 幂等、不受闸门影响。
2. **空闲过期**：`MS_SESSION_TTL_SEC`（env，缺省 600）双路径检查 —— 请求时惰性检查 + 30s daemon 扫描线程（`ws_open>0` 的会话跳过：WS 连接钉住不误杀；扫描是惰性检查的兜底，已死会话不再发请求，名额只能由扫描回收）。**alive hook 豁免（2026-08-30）**：`_is_expired` 三段短路（WS 钉住 → TTL 未超 → `_pinned_by_hook`），hook 只在本来就要过期时才被问（新鲜会话零开销）；strategy.py 注册 `_run_alive_hook` —— 策略/极限 run 存活期间（滚动钉住 `clock()+90s`，须 > 2× 扫描周期）与终态后宽限窗 `MS_RESULT_GRACE_SEC`（env，缺省 600，2026-09-13 由 7200 收敛对齐 TTL）内不逐出（睡眠唤醒/关页/跑完挂机等轮询中断场景不丢结果；宽限窗内任何操作恢复正常空闲语义；被钉住的会话仍占容量名额）；hook 在持 registry 锁上下文内执行 —— 只许纯内存 dict 读写 + 非阻塞 `poll()`，异常吞掉当 None，宽限窗外逐出照旧走墓碑。
...
| 429 | `session_limit` | `当前使用用户过多（最多 6 人同时在线），请稍后尝试`（随 `MS_SESSION_MAX` 插值） | 活跃会话满且 sid 未注册（create 路径） |
...
工作台**运行状态**（非导出产物）的可传递快照：后端出 doc 块（会话 `state['doc']` 原样，含 5 层渲染字段 —— 与 `/export`、`/api/edit-polish`、求解同一真相源），前端回传 form/quantities/quantities_base/run/pending_strategy_result 五块（`lib/stateFile.buildSavePayload`，US-003；quantities_base = 整列设值基准，2026-09-12 additive；pending_strategy_result = 待确认策略/极限 done 结果槽，前端产 US-002 2026-09-13），`web/statefile.py` 聚合并 gzip 序列化。恢复端 `/api/state-restore`（US-002）与本端复用同一守恒校验函数 `check_placed_conservation`。设计全文 `.docs/business/状态文件保存恢复_落地方案.md`（§四 schema / §五 后端改动点）。
...
```jsonc
{
  ...
  "quantities_base": { "g02": 2 },                 // 可选（2026-09-12）：{label:整数} 整列设值基准
                                                   // （qtyStore baseValue）；省键式 = 只送 ≠1 的行，
                                                   // 全 1 / 不送 → 文件无此键；纯 UI 基准（弹层初值/
                                                   // 特例高亮），不参与守恒/manifest
...
- `pending_strategy_result` **前端产方（US-002，2026-09-13）**：`lib/stateFile.buildPendingBlock` 轮询两族 store（useStrategyStore → useExtremeStore）—— `result` 非空且未 `resultApplied` 且 `state==='done'` 且 mode 三枚举才入槽；应用落定（`markResultApplied`）后整键缺席（已应用结果由 run 块承载，无双份数据）；`best.placed_items` 经 `deepCopyPlaced` 深拷贝（translation 拷断 + mirror omit-when-false，run.placed 同口径）。
...
- `file`：状态文件字节（扩展名 `.msn` / `.json`；纯 JSON 也接受 —— gzip 魔数 `1f 8b` 嗅探，开发期手改友好）；裸上传 >20MB → 413。
...
```jsonc
{
  "doc_id": "<铸新 uuid hex>",          // 每次恢复铸新文档身份（链式传递无原文件依赖）
  "filename": "5336#老六订单14%7%围加9.dxf",   // = doc.source（缺省回退上传文件名）
  "parse": { /* _build_parse_payload 同构：{doc_id, filename, sizes:[{size, pieces:[{label, polygon, 5 层}]}]}，
                经 _DocPieceView 属性视图复用（label 与 doc 同源）—— PreviewPage 零解析改动 */ },
  "manifest": { /* routes_ws.on_manifest / strategy result 同形：{gate_mm, total_area_mm2,
                   n_eroded, pieces:[{id, size, color, area_mm2, polygon, raw_polygon,
                   d_mm, label, demand, 5 层}]} = build_pid_meta(doc.pieces, form.sizes,
                   per_type, quantities) 确定性重算（params 缺省全 0 = web 口径） */ },
  ...
  "pending_strategy_result": { /* 2026-09-13 additive 回传：待确认策略/极限 done 结果槽
                                   ...
                                   缺席 → null → 前端 no-op；前端消费 = applyRestorePayload
                                   第 6 步（US-002）：按 mode 路由写回族 store（extreme→极限 /
                                   se·race→策略；合成 result={state:'done', mode, run_dir:null,
                                   manifest:本响应重算值, best, summary} + resultApplied:false）+
                                   openModal 自动开弹窗在结果态（refresh idle 采纳守卫防新 sid
                                   空槽打回）；2026-09-14 起第 6 步同第 5 步显式解锁 + 切超排
                                   Tab —— 弹窗 Portal 到 body 不随 Tab 隐藏，快照无 run 块时
                                   不切页则悬浮在上传预览页上（弹窗属于超排工作流） */ },
  "form": { }, "quantities": { },
...
响应内几何三份（parse + manifest + placed，~2-3MB，本机/局域部署可接受）—— 文件本身仍单份几何，manifest 不入文件故无漂移。

### 错误响应（`parse_state_document` 校验链，全 fail-fast 结构化 JSON）

| 场景 | 状态 | 说明 |
|------|------|------|
| 扩展名非 .msn/.json | 400 | `{"error":"仅支持 .msn / .json 状态文件"}` |
| 裸上传 >20MB / 解压后 >20MB | 413 | gzip 炸弹防线（前者未解压先拦，同 parse-dxf 口径） |
| 坏 gzip / 坏 JSON / 顶层非对象 / 深嵌套 RecursionError | 400 | `状态文件损坏…`（json.loads 容错，不炸 500） |
| schema_version 缺失/非 int/v<1 | 400 | `状态文件损坏（schema_version 缺失或非法…）` |
...
8. 测试：`tests/test_web_statefile.py` 共 104 例（US-001 23 + US-002 47 + 2026-09-11 E2E 冒烟回归 2 + 2026-09-12 quantities_base 7：save 入档/省键对拍、restore 透传回传、两端非法形态 400 参数化 + 2026-09-13 pending_strategy_result 槽 22：save 带槽往返/省键对拍、槽守恒 400 两款（改数量+母版外 pid）、槽形态 400 参数化 6、restore 槽透传回传/缺席 null、mode 枚举必败+三枚举通过、守恒两款内部不一致、全量形态 400 参数化 9 + 2026-09-14 run.stale 展示级降级 3：stale:true 恢复跳守恒 200（placed 原样重建）/ stale 仍拒母版外 pid 400（硬门保留）/ run.stale 非布尔 400 参数化新形态）。
...
body = `/api/state-save` 同形 `{form, quantities, quantities_base?, run?, save_as?, pending_strategy_result?}`（**save_as 键容忍忽略** —— 仅影响 save 的响应 CD，快照不含；`pending_strategy_result` 槽省键式随载荷自然携带，前端 done 结果拉取落定/应用落定即立即 flush（US-002 触发面 #4：两族 store `result null→非空` 或 `resultApplied false→true`），应用后载荷发送时刻现取 → 槽退场由 run 块承载）；`X-Session-Id` 同其余端点（无 sid → default 会话，存储键 `'default'`）。
...
| 成功入库但**陈旧 run 块打标保留**（2026-09-14：仅 run 守恒失败而 pending 槽在场且守恒通过 —— .msn 恢复的已求解 run 在用户切码/改数量后与 form 失配的常态；run 块 `stale:true` 展示级降级标记照常入库，恢复端按标记跳过守恒终检 —— 旧布局作弹窗背景，取消保留/确认置换） | 200 | `{"stored": true, "stale_run": true}`（additive 观测键，前端无感） |
| 会话空（无 doc/pieces，未 commit） | 200 | `{"stored": false, "reason": "empty"}` |
| 守恒校验失败（`check_placed_conservation` 复用：改数量未重解中间态；= 无 pending 在场、或 pending 槽自身亦失配） | 200 | `{"stored": false, "reason": "conservation"}`（**先前好快照字节不变 = last-good**） |
...
### 请求（POST /api/state-recover，US-002）

`X-Session-Id` = **新 sid**（前端铸新后携带；缺省 → default 会话，state-restore 同语义）；body `{"from_sid": "<旧sid>"}`（须过 `SID_RE`，非法/缺失/非字符串 → 400 `{"error": "from_sid 非法"}`）。
...
| 场景 | 状态 | 响应 |
| --- | --- | --- |
| checkpoint 不在 / 超 TTL / 已被恢复消费 | 404 | `{"code": "checkpoint_not_found", "error": "未找到可恢复的工作状态（快照不存在或已过期）"}` |
| from_sid 非法（非法字符/缺失/非字符串/body 非 JSON） | 400 | `{"error": "from_sid 非法"}` |
| 快照校验链失败（坏 gzip/JSON/schema/形态，state-restore 同契约） | 400/413 | `{"error": ...}`（StateFileError 同文案） |
| 新 sid 命中墓碑（误带旧 sid） | 401 | `{"code": "session_expired", "error": ...}` |
| 活跃会话满员 | 429 | `{"code": "session_limit", "error": ...}（**checkpoint 不删**，名额腾出后重试仍可恢复） |
...
2. **last-good**（FR-2）：守恒失败不覆盖上一份好快照（put 未发生，字节级断言）、返回 `stored:false` 而非报错 —— 自动后台任务不打扰用户。**唯一例外（2026-09-14 修复，真实事故回归锁；同日二改 = 初版「丢弃 run 块」让恢复后背景空置、取消一无所有，改打标保留）**：仅 run 块守恒失败而 pending 槽在场且守恒通过 → run 块 `{**run, stale:true}` 展示级降级打标照常入库 `{stored:true, stale_run:true}` —— 陈旧 run 不连坐待确认 done 结果；恢复端 `_validate_run_block` 见 `stale:true` 跳过守恒终检（`_validate_placed_list(skip_conservation=True)`，pid 命中 doc.pieces 硬门与逐条形态校验保留；`.msn` 手动 save 路径不认该键、严格 400 契约不变），旧布局照常合成进画布作弹窗背景（确认被 pending 置换、取消保留，与活界面同口径；前端 `run.stale` 消费忽略、`buildRunBlock` 不回写）；无 pending 时维持整载荷 last-good，改数量未重解 → 恢复改前完整快照的既有定案不变。
...
7. 分层：`web/checkpoint.py` 仅 import `sessions`/`statefile`，禁 import server（AST 守卫 `tests/test_web_checkpoint.py`，含 fastapi.concurrency/asyncio 白名单）；`register_checkpoint_routes(app)` server.py 文件尾注册（statefile 同模式）；`python -m materialsorting.web.checkpoint` 冒烟 37 项（store 假时钟生命周期 + 写入内核全路径 + 2026-09-13 pending 槽入档/守恒 last-good/mode 非法 + 2026-09-14 陈旧 run 打标独立裁决/打标快照自洽（run.stale 保留非丢块）/pending 续败回落 last-good + recover 全链：_FakeRequest 驱动真实 async handler，带槽快照恢复响应回传对拍）。
8. 测试：`tests/test_web_checkpoint.py` 共 50 例（store 单元 6 + peek 口径 3 + 写入往返 5 + empty 1 + conservation last-good 3 + 闸门/载荷 2 + DELETE 4 + 路由/env/AST 守卫 3 + `rebuild_session_from_document` 直接单元 1 + US-002 recover 11：真实 commit 全链 / 404×2 / 双次 404 / 429 条目保留 / 坏 from_sid 400 / 坏快照不占名额 / 墓碑 401 / threadpool spy / default 会话 / 路由白盒 + 2026-09-13 pending 槽 8：写入往返对拍（mode=extreme+无槽省键）/ 槽守恒 last-good（无 run 独立触发）/ 槽形态 400 参数化 5 / recover 响应回传 + 2026-09-14 分块独立守恒 3：陈旧 run + 守恒通过 pending → stale 打标入库（resp `stale_run:true` + 快照 run 块 `stale:true` 保留非丢块）/ run 打标后 pending 续败回落 last-good / recover 恢复陈旧 run 背景保留（响应 run.stale 透传 + placed 原样 + pending 并存））。
9. **前端消费链（US-003/US-004，2026-09-13；US-002 同日补触发面）**：写入 = `lib/sessionCheckpoint` 自动调度（form/qty 去抖 3s + run done/编辑保存/策略·极限结果落定·应用立即（US-002 触发面 #4 两族 store result null→非空 / resultApplied false→true）+ hidden flush + pagehide keepalive，一律 apiFetch 统一出口）；恢复 = `lib/sessionRecovery` 启动期编排（探测 401 → peek 旧 sid → 铸新 → 裸 fetch recover → 200 `applyRestorePayload` 复用 state-restore 前端 / 404·网络错静默新会话 toast / 429 session_limit 阻断弹窗）；启动清理 = 会话存活刷新 DELETE（F5 干净重置）、探测 401/阻断不清（快照留给恢复消费）。
10. **端到端冒烟（US-005 五路径 + US-003 pending 槽扩展，2026-09-13；bg 段 2026-09-14）**：`materialSorting-web/scripts/smoke_session_recovery.mjs` 自举起服（:8010，`MS_SESSION_TTL_SEC=60 MS_SESSION_MAX=1 MS_EDIT_HOLD_SEC=5 MS_RESULT_GRACE_SEC=3`—— 末项收敛终态宽限窗，冒烟「跑完不确认 + 宽限窗外过期」相位不必等生产 600s）。四相位（命令行逗号子集分段复跑，缺省全量、验收口径 = 全量一跑全绿）：**pending 段（US-003 主路径 + 续段②③）** race 10min 不确认 → 槽落 checkpoint（无 run 块）→ 过期刷新恢复：响应回传 pending（placed 逐条守恒 + run=null）+ 对应族弹窗自动开结果态（密度对拍）→ 待确认期改数量未重解 → checkpoint `stored:false reason:'conservation'`（last-good）→ 再过期恢复**改前完整快照**（数量矩阵回滚 + 弹窗重现）→ 应用 → run 块入档槽退场 → 主画布 polygon/状态行/来源小字对拍 → 导出 PLT placed 与过期前槽逐条一致 → 应用后再过期恢复：已应用 run 在场 + `pending=null` + **任何族弹窗不开**（无 pending 老快照行为对拍不变）；**bg 段（2026-09-14 陈旧 run 背景 + pending 弹窗 = 用户报告 bug 的端到端回归锁）** 5s 普通求解先铺 run 块背景 → 改数量 g01@32 1→2 → race 10min 不确认 → checkpoint `{stored:true, stale_run:true}`（陈旧 run 打标保留 + pending 按现行数量守恒入库）→ 过期刷新恢复：run.stale=true + 背景旧布局**仍在弹窗之下**（polygon=旧 placed 非 pending 新解）→ 取消 → 画布不清空（背景保留 + 来源「普通求解」）→ 再过期恢复 → 确认应用 → 背景被新解置换（polygon=pending placed + 来源「策略运行·race」）；**extreme 段（续段①）** 极限 16min 同款（mode=extreme 槽 + 与已应用 run 块并存 + 极限族弹窗不串台策略族）；**legacy 段（US-005 五路径原样）** 停留期引导弹窗（新文案 + 不自动恢复）→ 弹窗刷新恢复对拍 / 直接 F5 无弹窗恢复 / F5 存活 DELETE 清理 / 无 checkpoint 404 静默新会话 / `MS_SESSION_MAX=1` 双客户端 429 弹窗 + 名额释放后 recover 200（checkpoint 不删证明；skip-if-flaky）。
...
- 422：会话 pieces 快照空；intermediate doc 缺 `doc_id` 且 doc 无 pieces（旧 intermediate → 「母版信息缺少 doc_id，请重新上传并 commit」）；`uploads/<doc_id>.dxf` 丢失**且 doc 不带 pieces**（状态文件 US-005 起恢复会话例外：母版失盘但 doc 带 pieces → 走 intermediate 分支，不再 422）
...
- 202：清理本会话前缀上一轮产物 → 写 config JSON 到 `out/uploads/strategy_cfg_[<sid6>_]<stamp>.json`（数据源键二选一：**正常会话** `master_dxf` = 母版原件**绝对路径** / **恢复会话**（状态文件 US-005，2026-09-12：母版失盘 + doc 带 pieces）`intermediate` = `out/config_runs/web_[<sid6>_]int_<stamp>_<rand6>.json` —— doc 原样落盘、写在清理之后；`gate_mm` 请求值回退 state（恢复会话下 doc 原 gate 只存档，求解 gate 由 config 覆盖）；`seeds=[seed]`；band 合法开启 → `{'enabled':true,'label':g码}` 写进 config；可选键 truthy 才写）→ spawn（stdout=DEVNULL、stderr=临时文件前缀 `web_strategy_err_[<sid6>]_`）→ 快照 `out/config_runs/`（**先于 spawn**，回退发现路径防 diff 扑空；主认领路径 = 前缀 glob 不依赖快照）→ 写本会话 marker → `{started, pid, mode, minutes, run_name}`（run_name = `web_[<sid6>_]<mode>_<rand6>`）。band on 的策略 run 在 CLI 侧 worker 进程内成带（`--lns` 自动 warn 跳过）
