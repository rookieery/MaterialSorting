...
| 成功入库但**丢弃了陈旧 run 块**（2026-09-14：仅 run 守恒失败而 pending 槽在场且守恒通过 —— .msn 恢复的已求解 run 在用户切码/改数量后与 form 失配的常态；run 键不入快照保证内部自洽，恢复端守恒终检不拒） | 200 | `{"stored": true, "dropped_run": true}`（additive 观测键，前端无感） |
...
2. **last-good**（FR-2）：守恒失败不覆盖上一份好快照（put 未发生，字节级断言）、返回 `stored:false` 而非报错 —— 自动后台任务不打扰用户。**唯一例外（2026-09-14 修复，真实事故回归锁）**：仅 run 块守恒失败而 pending 槽在场且守恒通过 → 丢弃 run 块照常入库 `{stored:true, dropped_run:true}` —— 陈旧 run 不连坐待确认 done 结果（快照须内部自洽：恢复端 `parse_state_document` 守恒终检会拒带病 run；无 pending 时维持整载荷 last-good，改数量未重解 → 恢复改前完整快照的既有定案不变）。
...
10. **端到端冒烟（US-005 五路径 + US-003 pending 槽扩展，2026-09-13）**：`materialSorting-web/scripts/smoke_session_recovery.mjs` 自举起服（:8010，`MS_SESSION_TTL_SEC=60 MS_SESSION_MAX=1 MS_EDIT_HOLD_SEC=5 MS_RESULT_GRACE_SEC=3`—— 末项收敛终态宽限窗，冒烟「跑完不确认 + 宽限窗外过期」相位不必等生产 600s）。三相位（命令行逗号子集分段复跑，缺省全量、验收口径 = 全量一跑全绿）：**pending 段（US-003 主路径 + 续段②③）** race 10min 不确认 → 槽落 checkpoint（无 run 块）→ 过期刷新恢复：响应回传 pending（placed 逐条守恒 + run=null）+ 对应族弹窗自动开结果态（密度对拍）→ 待确认期改数量未重解 → checkpoint `stored:false reason:'conservation'`（last-good）→ 再过期恢复**改前完整快照**（数量矩阵回滚 + 弹窗重现）→ 应用 → run 块入档槽退场 → 主画布 polygon/状态行/来源小字对拍 → 导出 PLT placed 与过期前槽逐条一致 → 应用后再过期恢复：已应用 run 在场 + `pending=null` + **任何族弹窗不开**（无 pending 老快照行为对拍不变）；**extreme 段（续段①）** 极限 16min 同款（mode=extreme 槽 + 与已应用 run 块并存 + 极限族弹窗不串台策略族）；**legacy 段（US-005 五路径原样）** 停留期引导弹窗（新文案 + 不自动恢复）→ 弹窗刷新恢复对拍 / 直接 F5 无弹窗恢复 / F5 存活 DELETE 清理 / 无 checkpoint 404 静默新会话 / `MS_SESSION_MAX=1` 双客户端 429 弹窗 + 名额释放后 recover 200（checkpoint 不删证明；skip-if-flaky）。
