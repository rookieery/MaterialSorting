...
> 机器对接 `/api/machine/*`（YL 排料对接二期，2026-09-21 立项）：US-001 已落骨架模块 `web/machine.py`（空 APIRouter + server.py 文件尾 strategy 注册之后 `register_machine_routes(app)`），**尚无已注册端点**；五端点（`POST /api/machine/solve`、`GET/POST /api/machine/solve/{task_id}/status|stop|result`、`POST /api/machine/export`、`DELETE /api/machine/solve/{task_id}`）自 US-002 起逐故事挂载，契约专节随 US-005 落册。
