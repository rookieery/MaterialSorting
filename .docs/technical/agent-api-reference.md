...
> 机器对接 `/api/machine/*`（YL 排料对接二期，2026-09-21 立项）：US-001 骨架 + **US-002 `POST /api/machine/solve` 已注册**（multipart 母版+config → parse+commit → spawn → 202 task_id，见上表行）；其余端点（`GET/POST /api/machine/solve/{task_id}/status|stop|result`、`POST /api/machine/export`、`DELETE /api/machine/solve/{task_id}`）自 US-003 起逐故事挂载，契约专节随 US-005 落册。机器任务 = strategy 家族第三成员（`mode='machine'`，每任务独立 'm' 前缀 sid —— 复用 `_STRATEGY_STATES` 状态槽 / marker / run_dir 发现 / 树杀 / run 存活钉住骨架，浏览器工作台会话不受扰）。
