...
│   ├── types/              # US-002：纯数据契约（与 server.py 字段名 1:1）；上传预览 US-005：parsed.ts；上传预览 US-011：qty.ts；US-018：ptype.ts（PtypeRepresentative + PtypesResponse，GET /api/ptypes 契约）；策略 PRD US-005：strategy.ts（/api/strategy/* 四路由响应 TS 镜像：StrategyPhase 七态 / StrategyStatus（全字段可选除 state）/ StrategyResult（manifest 嵌套键）等）
...
5. **应用按钮 disabled 直到 US-006 接线** —— onApplyStrategy 未传即 disabled（title 提示）；US-006 经 ControlPanel→App→NestingPage prop 链接线时不得改 StrategyRunModal 内部。
