...
## edit-drag-snap US-003 落地（EditCanvas 右键贴附会话接线；2026-09-06）

### 文件

| 文件 | 改动 |
|------|------|
| `src/components/edit/EditCanvas.tsx`（扩） | 组件头注 US-003 分节；import snap.ts 三符号；常量 `PARTNER_FLASH_MS=900`/`PARTNER_FADE_MS=300`；`MoveDrag` 加 `snap: boolean`（右键贴附会话标志）；refs 三枚（`snapSessRef: SnapSession\|null` / `partnerGRef` / `partnerTimerRef`）+ 骨架重建/卸载/pointercancel 三路作废清理；`ensureUiLayers` 第三子层 partnerG（子层序 [partnerG, overlapG, handleG]）；refreshMetrics try 分支 lastSafeTr 免费续写（首帧落基线 + 谓词同 eps）/catch 分支保守跳过；`flashPartner`/`clearPartner`/`cancelPartnerFlash`（绿色虚线 #2ea06c + 900ms 常驻 + 300ms 淡出）；`applySnapOnRelease`（取清会话态 → 防御链 → 幂等 clamp → computeSnapCorrection → kind≠free 经 commitDragPlacement 唯一落笔出口 + flashPartner）；onPointerDown 顶部按键门控（`button!==0 && button!==2` return）+ 旋转柄/空白分支内 `button!==0` return + 毛版分支 `snap = button===2` 起会话；endDrag 末 `d.mode==='move' && d.snap` 分支；svg `contextmenu` preventDefault native listener；指南卡「贴附：右键拖动松手贴附」行 |
...
| 测试 | `components/edit/__tests__/EditCanvas.test.tsx` +12 项（1072−1060，头注 14) 分节注册）：右键 describe —— attract 6mm 缝（x≈500+1e-9 + 指标 0.00 + 伙伴 points/stroke + rot/mirror 原值）/ retreat 单跳（中途违例如实显示 50000 → 松手归 0）/ 两帧拖动 lastSafeTr 锚点精确回 [500,0] / 缝 60mm>10 free / 左键重叠原样 [400,0]（回归红线）/ 吸附后左键拖离不回吸 / rot180+mirror 片只改 translation / 非主键门控（中键不拖不转、右键空白不平移不取消选中）/ contextmenu 吞除（svg + 裁片）/ 伙伴高亮 1400ms 消退 / 右键无位移同款选中 / 指南卡文案；`firePointer` 加 button 参（缺省 0 既有用例零改动）；UI 覆盖层子层断言 2→3 |
...
4. **会话态生命周期**：右键 pointerdown 起（基线 Infinity 占位、首帧 refreshMetrics 落真实值）、pointerup 求解时**先取后清**（落笔触发的 refreshMetrics 不再续写）、pointercancel/骨架重建/组件卸载一律作废不求解；左键会话显式置 null。
5. **非主键零会话**：中键（1）/后退（3）/前进（4）不起拖动/旋转/平移任何会话；右键空白不平移、不取消选中、不弹菜单（contextmenu preventDefault 覆盖 svg 全域）。
...
- 浏览器验证（Playwright + Edge headless，真实 5336 母版 → 20s 求解 30 片 → 编辑弹窗）
  **17/17 PASS**：期望值由 Node 侧导入**真实引擎 esbuild bundle**（snap.ts+overlap.ts）
...
右键拖动松手贴附收官故事：`scripts/smoke_drag_snap.mjs` 双护栏端到端冒烟 + 吸附语义
...
| 文件 | 改动 |
|------|------|
| `materialSorting-web/scripts/smoke_drag_snap.mjs`（新） | 31 检查点 Playwright 冒烟（模板 smoke_edit_polish.mjs 流程骨架 + smoke-band-preview.mjs 的 addInitScript 预置 ms.tour.*）：S1 上传 5336 → 3 码 20s 求解 30 片 → 基线 PLT 导出（placed 与 WS 末帧逐位全等）；S2 编辑弹窗 + **真实引擎预模拟选片对**（esbuild 即时 bundle `src/lib/{snap,overlap,editGeometry}.ts` 到 out/smoke_drag_snap/engine_bundle.mjs，Node 在 WS 帧数据上跑 predictSnap(S,R) 单跳会话模型，attract/far-free/retreat/final-attract 四路全预测通过才采用 —— 全流程确定性）；B 右键 pointer 序列行为段（attract DOM points == 引擎 tr / 伙伴高亮轮廓 == partnerKey 世界轮廓 / 指标 0.0 / 高亮 ~1.2s 淡出 / trusted 右键 contextmenu 被吞 + 无位移 + 选中保持 / 拖动中合成 contextmenu 被吞 / 拖离远端 free / 深叠 retreat + 独立布尔交复核）；R 左键红线两档（深叠压线原样 >1000mm² + 小缝落点原样永不 attract）；C 末次 attract → 保存 → 导出（placed translation 与引擎值 \|Δ\|≤1e-6 双轴 + 亚 0.01mm 精度未截断 + 终态接触几何首触 ≈1nm/全邻居布尔交 0 + 其余 29 项与求解末帧逐位全等 + PLT 未动片轮廓区逐位全等 + 被拖片轮廓 = 基线平移吸附位移 ±2 unit）；报告 `out/smoke_drag_snap/report.json` + 逐相位截图 + 基线/末态 PLT 正文 dump |
| `materialSorting-web/AGENTS.md`（扩） | 文末「edit-drag-snap US-004 关键约定」节：**吸附语义六条单一真相**（触发 = 仅右键会话 / 求解 = 松手一次 / 效果 = 只改 translation / 永不吸附路径回归红线 / ATTRACT_MAX_GAP_MM=10 / erode 口径脚注**引用 overlap.ts 文件头原文**——单一真相源在 src/lib/overlap.ts，只引用不复制防两处文案漂移）+ 冒烟脚本运行口径与三处实现坑备档 |
