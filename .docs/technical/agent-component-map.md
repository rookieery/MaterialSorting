...
| `src/components/ControlPanel/PerTypeOverridesModal.tsx` | **新建** 高级配置弹窗（声明式受控 Portal → document.body；订阅 controlPanelStore.modal）。**US-003 改写**：列集 = /api/ptypes reps 键（g 码）∪ form.per_type 已配置键，compareByLabel 数值序；列头恒 `.qty-label-badge` g 码（V03_PTYPES/.ptype-name 兜底已删）；aria/testid = `d-${label}`/`tol-${label}`。结构：head(标题+✕) + table-wrap(overflow-x:auto) + actions(取消/确定)。表格 thead 10 列（缩略图 64×64 + 编号徽章 rep.label→`.qty-label-badge`，缺席兜底片型名；**列序 = `orderedPtypes` 按编号 A→J 排序**（`compareByLabel`：先长度再字典序 A<…<Z<AA，无 label 殿后；reps 全无 label/loading 时退回 V03_PTYPES 原序））+ tbody 2 行（同序渲染，重合 input max=10 + 旋转 input max=45，**全局上限**，blur `clampDraft` 规整到 [0,max]）。缩略图 hover/aria/title = `${label}-放大预览`（无 label 兜底片型名，不含片型名与编号并列的旧「A · 前片」格式）。挂载时 fetch GET /api/ptypes（mockImplementation 模式，StrictMode 双 mount 安全）。草稿+确定（未配置片型统一预填 '0'/'0'，2026-08-17 起不再按内部片预填 10）。缩略图点击 openPreviewPtype(ptype)。双层 ESC 独立（previewPtype!==null 时本组件 ESC 不关） |
...
| `src/components/ControlPanel/__tests__/PerTypeOverridesModal.test.tsx` | **新建** 23 项集成测试：modal=null 不渲染 / modal=per_type 渲染 overlay+modal+aria-label / thead 10 列片型名（无内外徽章）/ tbody 2 行（重合+旋转）/ 挂载 fetch /api/ptypes / fetch 失败降级片型名 / fetch 成功渲染 2 个 compact svg（前片+后片）+ 编号徽章 2 个（A/B，其余列兜底片型名）/ 列序按编号 A→J（腰=A/后片=B/前片=C 乱序映射 → 前 3 列 A/B/C + 列头与 d-input 对齐 + 无 label 7 片型殿后保持相对序）/ 无 reps 退回 V03_PTYPES 原序 / 缩略图 hover/aria=「A-放大预览」不含片型名（无 rep 兜底「单排-放大预览」）/ 初值统一预填 '0'/'0'（2026-08-17 起单排不再预填 10）/ 初值保留 form.per_type 非空值 / 编辑 draft 不立即回写 / 确定回写+关闭 / 取消丢弃 / 遮罩关闭 / ESC 关闭（previewPtype=null 时）/ ESC 不双层关闭（previewPtype!==null 时只关预览，底层 modal 保留）/ ✕ 关闭 / 点击缩略图 openPreviewPtype / 输入框全局上限（placeholder `d≤10`/`t≤45` + max 属性，10 片型逐一断言）/ blur 规整到 [0,max]（99→10、-3→0）/ modal 内 mousedown 不冒泡 |
...
2. **PerTypeOverridesModal 草稿 + 确定（与 PieceQtyDialog 同模式）** —— 打开时从 form.per_type 初始化 draft（key 强制每次 open 重建）；编辑仅改 draft；确定调 onChange + 关闭；取消/遮罩/ESC 仅关闭、draft 丢弃。改即时回写会破坏 AC#5 草稿语义。
3. **统一默认 0（2026-08-17 修订，D7 预填已删）** —— 未配置片型预填 '0'/'0'（不再按内部片预填 10）；全局上限重合 10mm / 旋转 45°（`constants/v03.ts MAX_OVERLAP_MM/MAX_ROTATION_TOL_DEG`，与后端 `constraints.py` 同名常量一致），input max 属性 + blur `clampDraft` 双重收边。改上限需前后端两处同名常量同步。
