...
│   │   │   ├── QtyMatrix.tsx      # 矩阵化重构 US-002 数量矩阵（行=label 并集[行头=徽章+名+compact 缩略图+填充按钮] × 列=doc.sizes 全码+合计；格内 clampQty 编辑 + Enter/Tab 跳格 + 0/缺片/特例格样式；列头 button setSize；sticky 表头/首列/底行 + 45vh 内滚；Σdemand 小计；接入 PreviewPage 在矩阵化重构 US-003）
│   │   │   └── __tests__/
...
│   │   │       ├── QtyMatrix.test.tsx        # 矩阵化重构 US-002 单测（32 项：行列结构 6 + 列头切码 2 + 缩略图 openZoom 1 + 格内编辑 6 + 0/特例高亮 5 + 行填充 popover 7 + 小计与总片数 4 + 重置 1）
