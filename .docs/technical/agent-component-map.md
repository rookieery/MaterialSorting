...
│   │   │   ├── QtyMatrix.tsx      # 矩阵化重构 US-002 数量矩阵（US-003 起接入 PreviewPage）：行=label 并集[行头=徽章+名+compact 缩略图+填充按钮] × 列=doc.sizes 全码+合计；格内 clampQty 编辑 + Enter/Tab 跳格 + 0/缺片/特例格样式；列头 button setSize（US-003 起承担尺码浏览职责，原 SizeTabs）；sticky 表头/首列/底行 + 45vh 内滚；Σdemand 小计
│   │   │   └── __tests__/
