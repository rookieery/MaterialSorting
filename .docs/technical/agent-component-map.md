...
│   ├── constants/          # US-004：SIZES / PHASE_COLORS / SEED_COLORS / V03_PTYPES + 全局上限 MAX_OVERLAP_MM/MAX_ROTATION_TOL_DEG
...
| `src/constants/v03.ts` | `V03_PTYPES` 10 片型顺序 + **全局上限** `MAX_OVERLAP_MM=10` / `MAX_ROTATION_TOL_DEG=45`（2026-08-17 起与后端 `constraints.py` 同名常量一致；旧 `V03_TABLE` 每片型上限表已删） |
