...
│   ├── constants/          # US-004：SIZES + 全局上限 MAX_OVERLAP_MM/MAX_ROTATION_TOL_DEG（US-003 起 V03_PTYPES 固定 10 片型清单已删，高级配置列集改由 /api/ptypes reps 键动态驱动；PHASE_COLORS/SEED_COLORS 已随收敛曲线功能删除 2026-09-12，现余 LAYER5_COLORS/NOTCH_LEN_MM 在 colors.ts）
...
│   │   ├── nests/          # US-003 NestSVG/NestCard/NestLabel + US-005 NestsGrid；US-006 NestSVG 加 hover（seek 回放分支已删 2026-09-12，恒渲染 lastFrame）；2026-08-27 NestLabel 标签追加用布长度
...
│   │   ├── curve/          # 【已删 2026-09-12】US-005 ConvergenceCurve 随收敛曲线功能移除
│   │   ├── playback/       # 【已删 2026-09-12】US-006 PlaybackBar/Seekbar/SeekReadout 随回放功能移除
...
| `src/constants/colors.ts` | `LAYER5_COLORS`（5 层工艺色）+ `NOTCH_LEN_MM`（PHASE_COLORS/SEED_COLORS 已随收敛曲线功能删除 2026-09-12） |
...
| `src/components/curve/ConvergenceCurve.tsx` | **已删（2026-09-12 收敛曲线功能移除）** |
...
| `src/components/curve/__tests__/ConvergenceCurve.test.tsx` | **已删（2026-09-12 随曲线功能移除）** |

## US-006 落地：片 hover tooltip（回放 seekbar 已于 2026-09-12 移除）

> **2026-09-12 回放功能移除**：`lib/seek.ts`、`appStore.seekTime`、`playback/` 三组件、`NestSVG` seek 分支及 4 个测试文件（seek.test / PlaybackBar.test / NestSVG.seek.test / ConvergenceCurve.test）整体删除。NestSVG 恒渲染 `run.lastFrame`。下表仅存仍有效的行。

| 文件 | 角色 |
| --- | --- |
| `src/lib/seek.ts` | **已删（2026-09-12）** |
| `src/store/appStore.ts` | 单字段 `renderTick`（`seekTime`/`setSeekTime` 已随回放功能删除 2026-09-12） |
...
| `src/components/playback/Seekbar.tsx` | **已删（2026-09-12）** |
| `src/components/playback/SeekReadout.tsx` | **已删（2026-09-12）** |
| `src/components/playback/PlaybackBar.tsx` | **已删（2026-09-12）** |
| `src/components/nests/NestSVG.tsx` | 增量（现行）：`f = run.lastFrame`（live 恒定）；flipGroup 上事件委托 mousemove+mouseleave（AC#4..#6） |
| `src/App.tsx` | 增量：挂一个 `<Tooltip/>`（setSeekTime 两处调用已随回放功能删除，clearHovered/hideTooltip 保留） |
| `src/lib/__tests__/seek.test.ts` | **已删（2026-09-12）** |
...
| `src/components/playback/__tests__/PlaybackBar.test.tsx` | **已删（2026-09-12）** |
| `src/components/nests/__tests__/NestSVG.seek.test.tsx` | **已删（2026-09-12）**（hover 用例由 Tooltip.test.tsx / NestSVG.test.tsx 承接） |
...
### 关键不变量（US-006 立，后续故事不得破坏；2026-09-12 回放移除后仅存 hover 三条）

1. **Tooltip 是模块级单例** —— `_el` / `_hovered` 是模块顶层的 let 变量；Tooltip 组件 mount 时 registerTooltipEl，NestSVG mousemove 处理器调 showTooltip/hideTooltip/setHovered。**App 内只能挂一个 `<Tooltip/>`**（多挂会互相 clobber）。
2. **Tooltip style 只能由 imperative 写** —— Tooltip 组件 JSX 不带 `style` prop（仅 className）；display/left/top/innerHTML 由 showTooltip/hideTooltip 直接 mutate。React reconciliation 不会覆盖。修改时不要在 JSX 加 style，否则重渲染会 reset display:none。
3. **flipGroup 上事件委托 mousemove + mouseleave（不是 svg）** —— AC#4 明确要求；与旧 vanilla 实现 setupHover(svg) 行为等价（多边形均在 flipGroup 内）。listener 在 `if (run.manifest && !flipRef.current)` 块内 attach，幂等保护防止 StrictMode 双 mount 双注册。
4. **面积换算 `mm² → cm²` 用 `÷100`** —— `parseFloat(dataset.area)/100`，与旧 vanilla 实现 一致；toFixed(1)。改单位 / 精度需同步 `Tooltip.test.tsx` / `NestSVG.test.tsx` 相关用例。

### 关键不变量（US-005 立，后续故事不得破坏；2026-09-12 曲线移除后仅存多 seed 编排四条）
...
2. **App all-done 检测用 ref 不用 state** —— `doneCountRef.current += 1; if (< totalSeedsRef.current) return;`；每次 handleStart 重置两个 ref。多 seed 收尾 setStatus 含 summary + best（`runs.reduce((a,r) => r.finalDensity > a.finalDensity ? r : a)`）。
3. **useRafThrottle(seeds.length>0) 不在 solving=false 时停** —— 求解结束后 NestLabel 仍需 bump 重绘最终态；下次 start() 才会 `runRegistry.clear + setSeeds([])` 间接停掉。
4. **NestsGrid 只在 seeds 变化时挂载/卸载** —— `<NestCard key={seed} run={rec}/>` 稳定 key；NestSVG 内部已订阅 renderTick 自更新，NestsGrid 不参与高频重绘。
...
| `drawCurve` 收敛曲线 | `src/components/curve/ConvergenceCurve.tsx` | US-005 | **已删（2026-09-12 曲线功能移除）** |
...
5. **收敛曲线 / 回放已删（2026-09-12）** —— 两功能移除后 result 步气泡 body 只描述排料卡片实时刷新，无对应锚点/文案。
...
| `src/components/__tests__/NestingPage.test.tsx` | **新建** 5 项集成（全链路渲染 NestingPage → 弹窗 done 结果态 → 点应用按钮）：①registry 恰 1 条 + 字段齐全（manifest demand/frames FrameMsg 同形/finalDensity 双口径/viewBoxMaxW/done/ws=null/stopped=false，旧 run 被清）②phase==='done'（状态行文案 + #restart；seekbar 断言已随回放功能移除 2026-09-12）③ExportButtons 非 disabled + NestSVG 多副本渲染（demand=2 → 2 可见 polygon、points 互异）④点导出 → POST /export 载荷 = 合成帧（seed/gate_mm/width_mm/density/placed 多副本）⑤不点 apply → registry 不变（显式按钮不自动应用） |

### 关键不变量（策略 PRD US-006 立，后续故事不得破坏）

1. **合成 RunRecord 与 WS 消息同形** —— manifest/frames 必须一字段不差地模拟 ManifestMsg/FrameMsg（含 `type` 判别键）；NestSVG/ExportButtons/useExport/bestRun() 对合成 record 零特判零改动（ConvergenceCurve/PlaybackBar 已随曲线/回放功能删除 2026-09-12）。改 RunRecord 字段时同步修 applyStrategyResult。
...
> 2026-08-25（ralph/prefix-head-set 分支）。后端协议（StartPayload.prefix / stage('prefix') / final.prefix）US-003 已落地；本故事补前端链路与 UI。**无尺码下拉**——资格码后端选取（2026-08-25 `pick_prefix_size` seeded 随机决策②；**2026-09-02 起 `select_prefix_plan` 近满幅几何搜索确定性选定，seed 仅兜底路径**），stage 消息 size 回显。
...
4. **stage='prefix' 不进 phase 五态** —— 秒级状态行提示（band 同款）；size 由 stage 消息回显（前端无法预知几何搜索结果；2026-09-02 补片双形态见下补记）。
5. **collectPrefix 无效输入静默降级 null** —— 与 collectBand 同防线语义：ControlPanel 闸门在前，collect 是兜底（后端仍会拦截）。

### 2026-09-02 补记：异码补片前端类型 + 双形态文案（prd-prefix-extra-piece US-004）

后端 US-002/US-003（stage `extra_label`/`extra_size`/`residual_mm` + prefix-preview `extra`/`residual_mm`/`gate_mm`/`fallback` + `select_prefix_plan` 近满幅几何搜索取代 seeded 随机）落地后，本故事补前端类型与两处呈现。协议 additive 零破坏：

| 文件 | 改动 |
|------|------|
| `src/types/ws.ts` | `StageMsg` 加可选 `extra_label`/`extra_size`（补片 g 码/尺码，null=兜底、**键缺席同判**）/`residual_mm`（gate−组合片高，round 3 回显）；注释 seeded 随机 → 近满幅几何搜索（seed 仅兜底） |
| `src/types/band.ts` | `PrefixPreviewResponse` 加可选 `extra`（`{label,size}` 只两键——PS_/rotation 永不出端点；兜底 4 片=null）/`residual_mm`/`gate_mm`/`fallback`；`n_members` 注释「4（兜底）或 5（补片）」 |
| `src/components/NestingPage.tsx` | stage='prefix' 状态行双形态：extra 在案 →「起始端成套构造中（尺码 {size}＋{extra_label}@{extra_size}）…」（＋全角）；兜底/旧后端 → 现行形态 |
| `src/components/ControlPanel/PerTypeOverridesModal.tsx` | prefix-zoom 放大层 hint 双形态：`preview.extra != null` 追加「＋ 顶部 {label}@{size} 异码片 · 余 {residual_mm}mm 近满幅」（residual 缺席显 —）；无键/兜底 = 现行 hint。5 片缩略自动渲染（BandPreviewMember 泛型零改动，异码片 `size_color(B)` 同码同色跨片型，tag=g 码） |

不变量补记：① 新键全部可选，旧后端（无键）前端不炸、两处文案回落现行形态（null 与 undefined 同判）；② `params.ts`/`collectPrefix`/ControlPanel 闸门零改动（prefix 契约仍恰三键）；③ hint 的 residual 直显后端 round 3 原值——DOM 缩略/主画布画的是 **erode 后几何**（比 residual 用的原始轮廓 union 高短 ~2×d_g），勿用 DOM span 反推 residual。测试：useSolveRun.stop.test.tsx +3（有补片/兜底 null/旧后端无键）+ PerTypeOverridesModal.test.tsx +2（5 片缩略渲染+hint 追加/无键回落）。
...
| `scripts/smoke_prefix_extra.mjs` | **新建（2026-09-02，US-005 端到端验收）** 异码补片全链路 CDP 冒烟（模板 smoke-band-preview.mjs 套路，29 检查全过）：上传 5336#老六订单 → 数量矩阵 7 码 Σ105 → per_type g02/g03 d=2/tol=1 → 开 prefix 预览 5 片 + 「＋ 顶部 g02@32 异码片 · 余 1.55mm 近满幅」标注 → 求解状态行「尺码 38＋g02@32」→ 形态判据（贴触 gaps / paired 同型成对序 / 近满幅 residual / min_x 锚定）→ final 无 placed_items 键 + 末帧 placed=105 守恒 + `PS_` 零泄漏 → 导出 PLT（fetch 抓包字节 grep b'PS_' 缺席）→ prefix_runs 工件快照 5 成员；报告落 `out/smoke_prefix_extra/report.json`。**2026-09-03 US-002 随成员序 paired 定案三处判据更新（强度不放松）**：① 5a「interleave 交替」→「paired 同型成对序（后后前前）」精确分段（4 片 flags = B,B,F,F）、report.form 键 `interleave`→`member_order`；② 4a 泄漏探针扩第五处 = 前端 DOM（`sleep(400)` 后 `.nest-card svg polygon` dataset.label 含 `PS_`）；③ 4b 顶异码守恒改按 P0 矩阵量 `SIZE_QTY` 表期望（g02@36 qty=3——旧硬编码 `extraN===2` 只对 qty=2 码成立，非放松是口径校准）；④ 2d 前端放大层文案对齐「4 片同码 paired 同型成对竖排贴靠」；复跑 29/29 PASS（胜者 g03_38→g03_38→g02_38→g02_38 rots 0/180/0/180，residual 23.761mm）。 |
...
1. **apiFetch 是唯一裸 fetch 出口** —— 新增网络请求必须走它（Header 注入 + 会话先行 + 阻断拦截三合一）；绕过 = 会话串台 / mount 竞态 401 / 阻断期间请求泄漏三重回归。**（会话过期自动恢复 US-003 起 amend：恢复链恰两处特权裸 fetch —— api.ts 探测 401 后重探 + lib/sessionRecovery.ts POST /api/state-recover；业务请求仍零裸 fetch，见文末专节）**
...
4. **~~session_expired 必弃 sid~~（会话过期自动恢复 US-003 起废止）** —— 后端墓碑 1h 拒重建旧 sid 的前提不变，但**停留期过期不再清 sid**：旧 sid = 刷新后启动期恢复的 from_sid（checkpoint 恢复锚点），SessionExpiredModal 文案改「会话已过期，刷新页面后将恢复工作状态」；clearPersistedSessionId 唯一调用方收敛到 sessionRecovery.ts 恢复链。session_limit 必保 sid 不变。
...
## 编辑排料 US-002 落地：编辑弹窗渲染与查看（完整版/毛板 + 缩放平移 + 禁关闭）（2026-09-04）

US-001 计算地基之上的 UI 面：大弹窗全量展示排料布局 + 顶部实时状态条 +
完整版/毛板切换 + 指针锚缩放/平移查看。**禁 ESC/遮罩关闭**（编辑草稿不可误触
丢弃，与全站弹窗惯例的有意偏离，唯一关闭路径 = 右上 ✕ 与右下保存）。零后端改动；
拖动/旋转（US-003）、保存/✕ dirty 确认/主界面集成（US-004）、冒烟（US-005）叠加。

### 新增 / 改造文件

| 文件 | 改动 |
|------|------|
| `src/components/nests/pieceDom.ts`（新） | 5 层节点构建（`SVGNS`/`PieceEntry`/`createPieceEntry`）自 NestSVG.tsx **机械提取**（唯一动现有渲染代码的点，纯搬移零行为变化）—— NestSVG 与 edit/EditCanvas 共用，弹窗与主视图同构同观感的地基；layer-aware 缺数据层不建节点、初始 display:none、4 层工艺 pointerEvents='none' |
| `src/components/nests/NestSVG.tsx` | import 改自 pieceDom（SVGNS/PieceEntry/createPieceEntry 原地删除），渲染循环/transformPt/hover 逐行不动 —— 41 项既有单测锁回归 |
| `src/store/controlPanelStore.ts` | `ControlPanelModalId` 联合加 `'edit_layout'`（注释明示有意偏离 ESC/遮罩惯例） |
| `src/components/edit/EditCanvas.tsx`（新） | 命令式 SVG 画布（骨架 React、节点全 createElementNS+setAttribute 逃逸 reconciliation）：数据源 = editStore.working（非 frames/renderTick）；翻转组 `translate(0 gate) scale(1 -1)`；**每 working 下标恰一份 5 层节点**（出现序语义天然满足）；初始 viewBox = `0 0 ceil(包络) gate`；毛板模式 4 层工艺 display:none、毛版+尺码色保留（即时可逆）；**滚轮指针锚缩放**（native listener passive:false；锚 = clientToWorld 世界点经 `userY=gate−worldY` 换算到用户空间 —— 直接用 worldY 会上下镜像，单测锁死；CTM 不可得退化中心锚）+ 空白拖动平移（毛版 polygon pointerdown 不起平移，US-003 拖动接管预留；比尺 = min(宽比,高比) 无需 CTM）+ ＋/－/重置视图按钮（工具置**左上**，右上留 US-003 指标面板；**2026-09-05 UI 优化：± 按钮删除（滚轮唯一缩放入口）、「重置视图」改名「全览」（仅复位缩放/平移不还原草稿）、形态 select 移入左上工具区、画布右下新增 .edit-guide 操作指南卡 —— 现行口径见 materialSorting-web/AGENTS.md 同日注记**）；zoomBy 钳制 MIN_VB_W=20/MAX_VB_SCALE=40 且**必须回写 vbRef**（漏写则第二档起锚在陈旧视图）；writeViewBox 显示值 r6 截断（CTM 反变换 ~1e-13 float 噪声不进 attr）；bg 跟随 viewBox（无限画布感）、fab 世界锚定（宽 = computeLayoutStats 同真相源） |
| `src/components/edit/EditLayoutModal.tsx`（新） | 声明式受控 Portal（范本 ExportInfoModal：订阅 `modal==='edit_layout'` 自显隐、Portal body、Inner key 重挂载 = 重新 open 快照）；mount 自 `runRegistry.bestRun()` 开 editStore（无解 → invalidate + 空态）；顶部状态条（料长/利用率/Δ 三指标 data-testid edit-layout-{width,density,delta}）—— **stats 与 Δ 基线都走 computeLayoutStats**（Δ 基线 = `computeLayoutStats(baseline.placedItems)` 同 ceil 口径，非裸 frame.density —— solver 小数 width_mm 下后者未编辑就 −0.01pt 取整伪影；Δ 只度量编辑效果，初值恒 +0.00pt）；右上 ✕（US-002 直接关，US-004 接 dirty 确认）+ 形态 select（2026-09-05 起渲染在画布左上工具区、state 仍在本组件经 onModeChange 受控）+ 右下**仅**「保存当前布局」单按钮（取消已废弃；handleSave = save()+close）；**不挂 ESC listener、遮罩无 mousedown handler**。**2026-09-05 edit-keyboard US-007 dated 注记**：本组件零改动 —— 保存→重开承接镜像片（open 自 lastFrame 深拷贝含 mirror 键）、微调请求/响应 mirror 逐位透传（buildPolishPayload placed 直传）的不变量改由 `scripts/smoke_edit_polish.mjs` S8 段端到端锁死（46 检查），详见文末「edit-keyboard US-007 落地」节 |
| `src/components/ControlPanel/ControlPanel.tsx` | 挂 `<EditLayoutModal />` 单例（ExportInfoModal 旁；打开入口 US-004 EditLayoutControls 接） |
| `src/style.css` | `.edit-layout-*` 家族（overlay z-index **1250** = ptype-preview 1200 之上、band-zoom 1300 之下；modal 92vw×92vh 暗色 #26282e 同款；canvas-tools 左上竖排；save 绿主按钮；svg `touch-action:none`） |
| 测试 | 新建两套 **26 项** + 扩一项：`components/edit/__tests__/EditCanvas.test.tsx` 14 项（骨架/翻转组/初始 viewBox、5 层 points·配色逐属性同构、旋转平移同公式、出现序多副本、working 变化只 setAttribute 不重建 + fab 随包络伸缩、毛板切换可逆、指针锚缩放手算锁死 "88 60 880 800"（Y 翻换算陷阱）/两档缩放跨 viewBox锚不动/缩小/CTM null 中心退化、拖动平移 "−60 20 1100 1000"、polygon 不起平移、±与重置、MIN 钳制 20）、`EditLayoutModal.test.tsx` 11 项（受控显隐/空态/状态条初值=主视图/Δ 基线 ceil 口径（frame.density 0.99 脱钩仍 +0.00）/5 层同构+footer 单按钮/形态切换/ESC 不关/遮罩不关/✕ 关/保存关+幂等写回）、controlPanelStore +edit_layout 切换。jsdom 缺口：PointerEvent polyfill（beforeEach `window.PointerEvent = class extends MouseEvent {}`）、setPointerCapture try/catch optional、getScreenCTM/createSVGPoint mock 矩阵（editGeometry.test mockMat 套路复用） |

### 关键不变量（US-002 立，后续故事不得破坏）

- **禁 ESC/遮罩关闭**：EditLayoutModal 永不挂 ESC keydown listener、遮罩 mousedown 永不调 close；唯一关闭路径 = 右上 ✕（US-004 dirty 确认）与右下保存。新增弹窗功能不得「顺手补上」ESC。
- **弹窗与主视图同构**：5 层节点只经 `nests/pieceDom.createPieceEntry` 建（配色/dataset/pointer-events 逐属性一致）；points 只经 `lib/geometry.pointsStr`（r2 截断）写 —— 两视图对同一 placement 必须产出相同 points 串（冒烟 S2h points 集合对拍锁）。
- **状态条唯一真相源**：料长/利用率只调 `computeLayoutStats(working, manifest)`（组件不得另写公式）；Δ 基线 = `computeLayoutStats(baseline.placedItems)`（同 ceil 口径，杜绝取整伪影）。solver 原始 width_mm 可为小数 → 弹窗 ceil 口径与主视图 NestLabel 差 ≤0.02pt 是「料长双向伸缩」设计固有取整，保存后主视图同步 ceil 即一致，非 bug。
- **缩放锚必须 world→user 换算**（`userY = gate − worldY`）：viewBox 数学在用户空间（翻转组之前），clientToWorld 返回世界空间（Y 向上），直接混用会上下镜像。
- **zoomBy 回写 vbRef**：滚轮/按钮/平移/重置全部以 vbRef 为当前视图 SSOT，任何视图变更函数漏回写 = 后续操作锚在陈旧视图（本项目已修，US-003 拖动同步渲染时沿用此约定）。
- **EditCanvas 事件分层**：毛版 polygon pointerdown 归 US-003 拖动（当前 no-op 预留）；4 层工艺节点 pointer-events:none；空白（svg/bg/fab）pointerdown = 平移。
...
版师单片级微调交互全量落地：EditCanvas 指针三分发（旋转手柄 / 毛版 polygon 拖片 /
空白平移），拖动/旋转帧 rAF 节流 + 钳制 + 提层，重合三指标面板（红色交集高亮），
状态条随 working 实时刷新（与保存同一真相源）。零后端改动。

| 文件 | 职责 |
| --- | --- |
| `src/components/edit/EditCanvas.tsx`（扩） | **指针三分发**（单一 effect、状态全走 refs/store 防陈旧闭包）：①pointerdown target 带 `data-edit-role="rotate"` → 绕选中片**质心**旋转拖柄（角度 = clientToWorld(CTM) 指针方位角，无 CTM 防御 no-op）；②`closest('polygon')` → selectPiece（选中 + ensureUiLayers 建覆盖层 + raiseEntry 提层到 UI 层之下）+ 平移拖片（setPointerCapture，失败不阻塞——监听挂 svg 自身）；③空白 → 平移（<3px down-up = 取消选中）。**拖动帧 rAF 节流**：pointermove 只记最新坐标 + scheduleFrame（rAF 缺失同步回退）；帧内 clampPlacement（Y 硬钳 [0,gate] 按被拖片 bbox：maxY>gate 下压、minY<0 上抬、x<0 钳 0、右界自由）→ 只 setAttribute 被拖片 5 层 → setWorkingItem（状态条同帧经 zustand 响应刷新）→ 池内 applyEditPlacement 增量 → refreshMetrics + updateHandle；pointerup 先同步 flushFrame（尾帧不丢，jsdom 测试确定性路径）。**平移拖片增量 = 屏幕位移/viewScale**（meet min 比，letterbox 对 dx 抵消，无需 CTM）；**旋转 pivot 公式** `q'=C+R(dθ)(q−C)` ⟹ `t'=C+R(dθ)(t−C)`（translation 随动、质心不动），wrapDeg180 归一。**重合指标**：选中/拖动帧 refreshMetrics = ensurePool（自 working 全量展开一次）→ computeOverlap（异常 catch → 降级 bbox 交估算：矩形环高亮 + 面积标「bbox 估算」+ data-degraded，穿透深度独立纯函数照常）→ 红色半透明交集高亮 polygon（overlapG pointer-events:none）+ 面板三值：面积 mm²（÷100 cm²）/ 最大穿透（>MAX_OVERLAP_MM=10 红 / >0 琥珀）/ 旋转偏离 `min(|r|,|r−180|,|r−360|)`（>MAX_ROTATION_TOL_DEG=45 红），脚注「按算法碰撞口径」（manifest erode 几何）。**旋转手柄**：选中片质心 + (0, 3r)（r=clamp(vb.w×0.015,4,30)），连线 dashed + 实心圆可抓；UI 覆盖层懒建（US-002 childNodes 断言不破）。渲染 effect **签名跳过**（`mode|rot|tx|ty`）：拖动帧已即时应用的片零 setAttribute，working 变化只触碰变更片。 |
| `src/lib/overlap.ts`（扩） | `precomputeEditPiecesFromItems(manifest, items)`：PlacedItem[] 直入口（编辑画布池源 = working 非 lastFrame；frame 版薄委托）；`applyEditPlacement(ep, rot, tr)`：池内单片**原地**增量（worldPolygon/bbox/rot/tr 覆写，basePolygon 只读共享引用不动）—— 拖动帧只重算被拖片一项。**polygon-clipping 双产物互操作**：ESM dist（vite/rollup 解析）只导出 default、CJS/UMD（vitest deps 预打包）具名导出 → namespace 导入 + default 优先回退拿 intersection（具名 import 在 rollup 下构建报错「"intersection" is not exported」，US-001 期未炸是因无生产代码引用）。 |
| `src/components/edit/EditLayoutModal.tsx` | **零改动**：状态条本就订阅 working 响应式计算（stats/baselineStats 每渲染重算 computeLayoutStats）—— US-003 拖片 setWorkingItem 即实时刷新，料长/利用率/Δ 与保存同公式同真相源（AC 语义由设计保证，新增 3 测试锁死）。 |
| `src/style.css`（扩） | `.edit-metrics`（右上角固定面板，pointer-events:none 不挡拖动）+ `-title/-row/-label/-val` + `--warn #d9a05b` / `--danger #e06666` + `-foot`；`.edit-rotate-handle` cursor grab/grabbing。 |
| 测试 | EditCanvas.test 14→**27**（拖动 translation 精确值/提层序 [b,a×5,uiLayer]/多副本出现序、Y 上下钳/x 钳 0 右界自由、旋转手柄 +90° 手算锁死（CTM mock）/手柄仅选中片+空白点击取消、指标三值精确 22500/50.0/90.0° + ≤10 琥珀 + 拖动中 rAF 实时）+ **EditCanvas.degraded.test 新建**（vi.mock polygon-clipping 双形态抛错 → data-degraded=1 + bbox 估算 + 拖动不阻塞）+ EditLayoutModal.test 11→**14**（拖右 width↑/pct↓/Δ− 且 lastFrame 不动、拖左 width↓/pct↑/Δ+、编辑后保存 lastFrame 继承编辑值）+ overlap.test +2（FromItems≡frame 版 / applyEditPlacement 原地≡全量重展开）。 |

### 关键不变量（US-003 立，后续故事不得破坏）

- **拖动帧只触碰被拖片**：pointermove → rAF 帧内只 setAttribute 被拖片 5 层 + setWorkingItem（单下标）+ 池内 applyEditPlacement（单元素）；渲染 effect 靠签名跳过保证其余片零写。给全部片重建节点 = 违约。
- **每 rAF 帧一次 setWorkingItem（非每 pointermove）**：状态条实时性的唯一通路是 zustand working → EditLayoutModal 响应式重算 computeLayoutStats；组件内不得另设旁路指标公式。
- **钳制口径**：Y ∈ [0, gate] 按**被拖片 bbox**（minY<0 上抬 / maxY>gate 下压，两违同时以下压后；超高片贴底）；x<0 钳 0；右界**永不钳**（料长双向伸缩是编辑特性）。gate = manifest.gate_mm。
- **旋转自由角**：无 0/90/180 吸附（2026-09-04 决策）；偏离角只**如实显示**（>45° 红）不拦截不报警。
- **旋转手柄位置** = 选中片质心 + (0, 3r) 世界系正上方（翻转组内即屏幕上方）；手柄拖动的角度必须经 CTM（clientToWorld），平移增量经 viewScale —— 两通路不可互换（letterbox 对 dx 抵消但角度不抵消）。
- **UI 覆盖层懒建**：首次 selectPiece 才建 overlapG/handleG（US-002 的 childNodes 快照断言依赖「无选中 = 无覆盖层」）；交集高亮层永远 pointer-events:none。
- **降级不阻塞**：computeOverlap 布尔交异常 → catch bbox 估算 + data-degraded 标注（面积标「bbox 估算」、穿透照常、矩形环高亮），拖动照常进行；纯函数 overlap.ts 本身不吞错上抛。
- **提层置顶**：selectPiece 即 raiseEntry（被拖片 5 层移到全部裁片之上、UI 覆盖层之下）—— 注意**提层后文档序 ≠ working 序**，按 DOM 下标寻址会错片（浏览器验收 S3e 教训：以 points 字符串为片身份）。
...
- 浏览器验收（vite dev :5173 + ms-web :8000，882 母版 3 码 20s 求解 → final
  ...
  状态条料长 6149→6693 实时增长**、料长 6149→7236/利用率 83.59→71.03/Δ −12.56pt、
  被拖片提层置顶（末位毛版 maxX 增量吻合）；点选带布纹线片 → 手柄+面板在场，
  拖柄自由转角 111.94°（非 90 倍数）、质心漂移 550.9mm < 半对角线 621mm（绕质心
  pivot 生效）、布纹线端点随片变化；左移压邻居 → 面板三值（44861.0mm²/32.0mm
...
## 编辑排料 edit-polish US-004 落地：端到端冒烟与文档闭环（2026-09-05）

edit-polish PRD 收官（镜像 prd-edit-nesting-layout US-005 冒烟收官形态）：不新增组件
（US-003 的 EditLayoutModal 微调区/EditCanvas 对比卡零改动），智能微调整条链路
（上传 → per_type 放开 d/tol → 求解 → 微调 → 撤销 → 确定性双跑 → 保存 →
PLT/DXF 导出守恒 → band exclude 抽验）用一条冒烟锁死回归；**红字口径差异**
（编辑画布 = erode 后轮廓、polish 报告 = 物理毛版）入册
`agent-api-reference.md` /api/edit-polish 专节 ⚠️ 注记。

**EditLayoutModal 微调区 dated 注记（2026-09-05）**：工具区「智能微调」按钮 + 右下
对比卡 + 一级撤销 = US-003 落地形态（详见上一节）；本故事不改其任何行为，冒烟
S2g/S3a/S4a/S6c~e 即对该形态的回归锁（六指标行/口径脚注/撤销按钮/卡清空/
exclude labels 组装）。
...
- **冒烟四守恒是引擎 AC 的端到端执行器**：overlap_pairs 严格下降 / rotΣ 下降 /
  width ≤ before / density ≥ before−1e-6 —— 改 polish 引擎或端点行为必须先对齐
  冒烟判据（US-001 单测夹具之外的第二个回归层）。
- **导出守恒双格式**：PLT 与 DXF 两次导出的 payload placed 都必须与微调响应 placed
  深相等（条数 + 逐字段）——「微调结果经保存进导出」链路断裂即 FAIL。
- **band exclude 抽验锁 v1 over-conservative 口径**：band 记录 → exclude.labels 单键
  命中该 g 码全部实例（report.excluded 与实例下标精确对拍），带形态区域 points
  前后不变是排除语义的几何证据。

### 验证

- 冒烟 **24/24 PASS**（`out/smoke_edit_polish/report.json` + 4 截图，退出码 0；
  实测 overlap 42→38 / rotΣ 210→190° / width 1762.039→1761.778mm /
  density 89.707→89.72% / moves=4 residual=61 / band excluded=[6,7,8] 不变）。
- 质量门全过：`python -m materialsorting.nesting_engine.polish` 合成夹具八项自检
  exit 0；pytest 全量 **728 passed**；vitest 全量 **962 passed**（64 文件，零新增
...
- **keydown 监听器只挂一次、prop 经 ref 读现值**：`interactionRef.current =
  interactionEnabled !== false` 渲染期回写（modeRef 先例）—— 确认层开关即时
  生效无需重挂监听；守卫链顺序固定（总闸 → 表单控件 → 无选中），R 键分支
  留 US-006（不得在 US-006 外实现）。
- **幂等键与连发键的 e.repeat 分界**：L/K 放行（auto-repeat 每击一档）、
  空格/O/I 忽略（一次 keydown 一次变换，按住不抖动）；空格必 preventDefault
...
- vitest 全量 **1016 passed**（1005+11，64 文件）/ `npx tsc --noEmit` 干净 /
  `npm run build`（tsc+vite）过（304.77KB gzip 99.65KB）、static/ 已重建。
- **浏览器 dev 模式 26/26**（`out/usk005_keyboard_verify/verify2.mjs` +
  report2.json + v2_p3c_mirror.png/v2_p3e_vmirror.png/v2_final.png）：P1 上传
  5336 → 3 码 30 片短求解 + 编辑弹窗 + 指南键盘行在场（无「形态」「保存」回潮）；
  P2 真实 trusted 按键 L/K/Shift+L/Shift+K —— 页面内动态 import
  editGeometry/editStore 纯函数直算期望 placement（rot/tr/mirror + points），
  DOM↔纯函数逐字节对拍（内容匹配恰 1 片）+ 指标面板旋转角同帧刷新；P3 空格×2
  掉头往返 / O×2 镜像双向 / I×2 垂直镜像复合往返（截图归档）；P4 连发 20 次
  repeat:true L → +20° 耗时 78.1ms（<3000ms 不卡顿）+ 空格/O/I repeat 全忽略；
  P5 守卫链：形态 select 聚焦零变换 / 无选中零变换 / ✕ dirty 确认层全键禁用 /
  取消确认层后 **L 恢复生效**（interactionRef 渲染期回写，前轮实现失败点）；
  P6 全程无 pageerror。

## 编辑排料 edit-keyboard US-006 落地：R 键片级重置（2026-09-05）

US-005 键盘层的收官键：R = 片级重置交互入口（定案 2026-09-05：R 键非右键菜单）。
重置语义 = 恢复**算法基线**（会话首次 open 的算法原始布局）⇒ 会抹掉该片已保存的
编辑（定案接受；本期不建撤销栈）。零后端、零渲染公式改动 —— store 侧复用 US-002
的 `resetItem`，视图侧复用拖动帧链路（拆出共用段）。

| 文件 | 改动 |
| --- | --- |
| `src/components/edit/EditCanvas.tsx` | ①R 分支接线（位置在幂等键 `if (e.repeat) return` 闸之后 —— R 与空格/O/I 同为幂等键防抖；守卫链与 US-005 六键同链）：调 `editStore.resetItem(selRef 下标)`（同 pid 多副本按下标寻址绝不按 pid）→ 返回 false（store 层 baseline/越界/id 错位守卫命中）静默不动不炸 → 成功后重读 `working[index]` 按基线值（rot/tr/mirror === true）走 refreshPieceView 单点同帧刷新；②`refreshPieceView(index, entry, rot, tr, mirror)` 自 commitDragPlacement 拆出（视图段：applyEntryPlacement DOM 5 层 + 签名登记 + applyEditPlacement 池增量 + refreshMetrics/updateHandle —— 不含 store 写入）；commitDragPlacement 重构 = setWorkingItem + refreshPieceView（指针/键盘变换路径行为不变） |
| 测试 | +5 项（1016→1021）`EditCanvas.test` R describe：正常恢复（指针拖动 + L 旋转 + O 镜像后 R → working 逐字段回基线、mirror 键消失、其余片与 lastFrame 不动、毛版 points 手算回基线、指标面积 90000→50000 精确重算 + 旋转偏离 1.0°→0.0° + 手柄 cy 767.5→567.5 同帧回基线）；e.repeat 忽略 + 非 repeat 生效；未选中无效（空白点击取消后）+ 重新选中生效；确认层打开（interactionEnabled=false）禁用 + 恢复 true 生效；resetItem 返回 false（人为 baseline id 错位，pidSeq 不变直达守卫）静默不动不炸 + DOM 不动 |

### 关键不变量（edit-keyboard US-006 立，后续故事不得破坏）

- **R 的 store 写入唯一出口 = resetItem**：成功后不得再 setWorkingItem（resetItem
  已写 working 草稿）—— 视图侧只走 refreshPieceView 读草稿基线值刷新。
- **resetItem 返回 false 必须静默**：不炸、不 fallback 按 pid 寻址、DOM 不动。
- **refreshPieceView 与 commitDragPlacement 共用**：改「同帧刷新」逻辑只改
  refreshPieceView 一处；两条路径（store 写入在前/无 store 写入）不得分叉。
- **幂等键集合 = {空格, O, I, R}**：repeat 闸之后分发；L/K 连发键在闸之前。

### 验证

- vitest 全量 **1021 passed**（1016+5，64 文件）/ `npm run build`（tsc+vite）过
  （304.95KB gzip 99.74KB）、static/ 已重建。
- **浏览器 dev 模式 26/26**（`out/usk006_reset_verify/verify.mjs` + report.json +
  p3b_edited.png/p3d_reset.png/final.png）：P1 上传 5336 → 3 码 30 片短求解 +
  弹窗 + 指南 R 行在场；P2 L+1° → 保存并关闭 → 重开承接已存编辑（open 自
  lastFrame 深拷贝）→ O 镜像 → R 回**算法基线而非已存值**（「连已保存编辑一起
  抹掉」定案语义锁；working 逐字段/基线对拍 + DOM 基线 points 内容匹配恰 1 片）+
  lastFrame 保持已存编辑不动 + 全 30 片 points 多重集回基线 + 指标面板旋转偏离
  同帧 0.0°；P3 指针拖片 5 + Shift+L +10° + O 镜像复合编辑 → R 逐字段回基线
  （镜像片复位 + 全 30 片多重集回基线，截图归档）；P4 repeat:true R ×3 全忽略 +
  单击正常复位；P5 守卫链（无选中零动作 / ✕ dirty 确认层 R 禁用 / 取消后 R 恢复
  生效）；P6 全程无 pageerror。验证后 ms-web/vite 进程已杀（端口 8000/5173 释放）。

## 编辑排料 edit-keyboard US-007 落地：端到端冒烟扩展与文档同步（2026-09-05）

键盘/镜像/重置新不变量的**冒烟回归收官**：零组件、零后端、零渲染公式改动 ——
`smoke_edit_polish.mjs` 在既有 29 检查（US-004 24 + US-005 S7 compact 5）之上
新增 S8 键盘/镜像段 17 检查（46 全 PASS），把「O 镜像→保存→导出→微调→R 重置」
全链锁进端到端门。

| 文件 | 改动 |
| --- | --- |
| `scripts/smoke_edit_polish.mjs` | **S8 段 17 检查**（S8a~S8q）：S7 compact 结果保存 → 镜像前基线 DXF 导出（placed 守恒 + layer1 闭合轮廓 30 条解析器）→ 选片决策（eroded 质心锚定预测不触门幅 ≥2mm + 镜像可辨位移 ≥20mm + 非带排除片）→ 重开承接（points 内容匹配恰 1 片）→ 合成 pointerdown 选中 → **O 键镜像**（blurActive 后 trusted keydown；DOM 多重集差恰 1 出 1 进 + 新 points = 质心锚定反射对拍 ≤0.05mm）→ 保存 → 导出 PLT/DXF：**POST placed 恰一项 `mirror:true`**（下标=选中片 + rot 不变 + eroded 质心锚定补偿 ≤1e-6 + 其余 29 项与镜像前逐位全等 + PLT/DXF 载荷一致）+ **正文几何镜像坐标对拍**（期望 = R(θ₁)·M·R(−θ₀)·(镜像前 DXF layer1 顶点 − t₀) + t₁ —— θ/t 取两次导出 POST placed[k]，PLT ≤2 HPGL unit（lead 偏移自门幅框 stroke 首点推导）/ DXF ≤0.05mm，且非镜像反事实 ≥40unit/≥1mm = mirror 键真实驱动几何）→ 重开承接镜像片 → **微调 mirror 逐位透传**（请求/响应恰一项同下标同 pid，其余 29 项无 mirror 键）→ **R 键重置**（恰改一片 + points 逐字节 = S6 末帧算法基线串（solverPlaced2[k] + manifest eroded 多边形 Node 侧复刻 pointsStr）且 ∈ 首开快照 + 基线 pid 对位）。配套纯 Node 算子：`pointsStrJs`（lib/geometry.ts 逐字节复刻 —— prod 构建页面内无法 import 源码模块）/ `parseDxfOutlines`（R12 ASCII 严格 code/value 成对步进；**边框无 layer 键落缺省 '0' 不入 layer1 列表**）/ `parsePltStrokes`（PU 起新笔 / PD ≤10 点分块拼接；**stroke[0]=门幅框、stroke[k+1]=placed[k]**）/ `selectByPoints`（按 points 内容寻址 + 直发 polygon pointerdown —— 真实 hit-testing 会被叠片抢 target）/ `blurActive`（键盘前置：按钮聚焦命中守卫②）/ `multisetDiff`（恰改一片判据） |
| 文档 | materialSorting-web/AGENTS.md（edit/ 树行 US-007 fragment + 冒烟节 29→46 + 文末「编辑键盘/镜像关键约定」汇总节：六键语义/质心锚定/omit-when-false/resetItem 下标守卫）+ 本文件（头部覆盖行 + 编辑弹窗节 dated 注记 + 本节）+ agent-api-reference.md（/api/edit-polish 专节与 /export placed 的 mirror 端到端对拍注记） |

### 关键不变量（edit-keyboard US-007 立，后续故事不得破坏）

- **S8 几何对拍公式是导出真值的闭式复算**：期望 = R(θ₁)·M·R(−θ₀)·(v−t₀)+t₁
  （两端取两次导出的 POST placed[k] + 镜像前 DXF layer1 轮廓）—— 不依赖
  intermediate 磁盘文件（会话几何真相随请求走）。改 export_geometry apply_transform
  mirror 分支 / 前端 O 键锚定公式任一处，S8l/S8m 即红。
- **选片寻址按 points 内容**：提层置顶后 DOM 序 ≠ working 序 —— 冒烟/验证脚本
  一律 `selectByPoints(pointsStrJs(...))` 内容匹配，不得按 querySelectorAll 下标。
- **O 键锚 eroded 质心、导出画 raw 轮廓**：反射预测的锚 = eroded 世界质心
  （EditCanvas cWorld 同源顶点均值），几何源 = raw 毛版 polygon —— 两口径差由
  锚定 translation 吸收（S8k anchorErr ≤1e-6 与 S8m err ≤0.05mm 同时成立是
  该等价性的实证）。
- **既有 29 检查复跑全绿 = mirror=false 零回归门**：S1~S7 段不动，S8 段纯追加。

### 验证

- 冒烟 `node materialSorting-web/scripts/smoke_edit_polish.mjs` **46/46 PASS**
  （报告 `out/smoke_edit_polish/report.json` mirror 段：domReflectionErr 0.006mm /
  anchorErr 0 / PLT err 1.41unit·counter 50801 / DXF err 0.0095mm·counter 1270 /
  polishPassthrough req+resp byteEqual / resetToBaseline）。
- 全量回归：vitest **1021 passed**（64 文件）+ 后端 pytest **745 passed** +
  `npm run build` 过（304.95KB gzip 99.74KB）、static/ 已重建（冒烟跑在重建后的
  prod 构建上）。

## edit-drag-snap US-001 落地：吸附几何算子 contactT / firstContactDistance（2026-09-06）

> PRD tasks/prd-edit-drag-snap.md（编辑弹窗右键拖动松手贴附）首个故事：纯前端零后端，
> 为松手吸附引擎（US-002 snap.ts）提供解析首触距离算子。

| 文件 | 改动 |
| --- | --- |
| `src/lib/editGeometry.ts`（扩） | 中段新分节「edit-drag-snap US-001」：`contactT(v, dir, a, b)` —— 顶点 v 沿单位方向 dir 平移到线段 ab 的参数 t 解，叉积线性解 `t = −cross(v−a, d)/cross(dir, d)` + 命中点须落线段本体参数区间（含端点，容差 1e-9 吸收浮点噪声）；返回 null 不抛异常的三情形 = 零长退化边 / dir 与边近平行（`|cross(dir,d)| ≤ EDGE_PARALLEL_EPS·|d|`，即 |sinθ|≤1e-9 —— 弧片密集顶点实况，恰共线时无 eps 会除零产 ±Infinity/NaN 污染最小值；真实触点在端点、由相邻边覆盖）/ 命中点在线段延长线上。负 t 原样返回（触点在身后），过滤正 t 是调用方职责。`firstContactDistance(moved, obstacle, dir)` —— 两多边形沿 dir 首触距离：moved 全部顶点×obstacle 全边（正向）+ obstacle 全部顶点×moved 全边（contactT 传 −dir 相对运动视角）两向取**最小正 t**（mm）；解析零迭代零 RNG、固定枚举序（`<` 保留首遇，值相等故与序无关）、空多边形/近平行对安全 null。文件头注补一行分节指针 |
| 测试 | `lib/__tests__/editGeometry.test.ts` +16 项（24→39，全量 1022→**1038**）：contactT 7（垂直命中精确 100±1e-9 / 斜向 50√2±1e-9 / 延长线命中 null / 近平行斜率 1e-12 与恰共线 null 不抛 / 负 t 原样 −50 / 端点命中含 s=0 与 s=1 / 零长边 null）+ firstContactDistance 9（矩形夹具 +x 精确 150±1e-9 / 3-4-5 斜向两向枚举同值 250/3±1e-9 / 凹形 L 首触点恰落凹口内角 (50,50) t=10 整数精确（moved 底边与凹口底边共线的近平行对被跳过）/ 近平行弧段密集顶点两向全无良态交点 → null 不抛 / 近平行弧弦在场但真触点良态 → 照常手算 40 / 前方无交点 null / 空多边形防御 null / 镜像+旋转 vs 直接构造同形态同 t 口径无关性锁（x 预取负恒等式 + 逆序/换起点顶点序无关 + 非对称 base 自证 t=200≠136.11）/ 确定性双跑逐位全等） |

### 关键不变量（edit-drag-snap US-001 立，后续故事不得破坏）

- **吸附数学只发生在 worldPolygon 上**：contactT/firstContactDistance 只读顶点数组，
  与 rot/mirror 口径无关（镜像+旋转片 vs 直接构造同形态片同 t，单测锁死）——
  US-002 snap.ts 喂 EditPiece.worldPolygon，不得在算子里重算变换。
- **dir 单位向量契约**：t = mm 的前提是调用方传单位方向；算子不归一化。
- **近平行 null 是特性不是 bug**：|sinθ| ≤ 1e-9 的顶点-边对返回 null（跳过），
  真实触点由相邻边端点命中覆盖；恰好去掉该守卫会让恰共线对产 ±Infinity/NaN。
- **返回值语义 = 沿 dir 前方的下一次接触**：负 t 解（已交叠回看）在
  firstContactDistance 内部排除；contactT 单元层保留负 t 原样返回。

### 验证

- `npx tsc --noEmit` 干净 + vitest 全量 **1038 passed**（64 文件，1022+16）+
  `npm run build` 过（305.07KB / gzip 99.81KB）、static/ 已重建；零后端改动、
  零组件改动（无 SVG/UI 变更，浏览器验证不适用本故事）。

## edit-drag-snap US-002 落地（lib/snap.ts 松手吸附引擎纯函数；2026-09-06）

### 文件

| 文件 | 角色 |
| --- | --- |
| `src/lib/snap.ts` | 新建：松手吸附引擎纯函数。`computeSnapCorrection(dragged, rawTr, sess, others, opts)` → `{tr, kind:'free'\|'retreat'\|'attract', partnerKey}`；常量收口顶部（ATTRACT_MAX_GAP_MM=10 导出 / COARSE_SCAN_STEP_MM=20 / BISECT_ITERS=14 / SEP_NUDGE_MM=1e-9 / COLLIDE_AREA_EPS_MM2=1e-9 —— 后两者与后端 polish.py 同名同口径，另 CLAMP_EPS_MM=1e-6 同 GATE_EPS_MM）。依赖仅 editGeometry（firstContactDistance/transformPolygon/bboxOf）+ overlap（computeOverlap oracle），零 DOM 零组件。 |
| `src/lib/snap.test.ts` | 新建（PRD 指定扁平路径，vitest include 覆盖）：22 项 —— retreat 解析对拍 0.01mm + 布尔交 ≤1e-9 + 分离 ≥1e-9（1nm 断言容 1e-12 ulp 消去噪声）、隧道穿越夹具（朴素二分 300 vs 粗扫 50）、基线不恶化不误清（恶化位回基线 / 改善·持平位不动）、5 副本同 pid 求和锁（各 50 合计 250 > 基线 200）、attract 6mm 吸 / 15mm 不动 / 零位移质心退化（rawTr===lastSafe 与 null 两变体）/ 平手 key 小（数组顺序无关）/ 同 pid 寻址锁 / clamp 三变体（gate 顶、minX、minY）+ retreat 候选违 clamp / getter 注入内部异常 fail-open / 确定性双跑 JSON 逐位全等 / 空邻居·无锚点·含自身 key 跳过防御。 |

### 不变量（改引擎前必读）

1. **触发归调用方**：引擎不感知按键/会话，只被 US-003 起右键 snap 会话的 pointerup 调用（左键/键盘/旋转柄永不吸附是接线层红线）；结果只改 translation，永不动 rot/mirror。
2. **retreat = 粗扫+二分**：lo = `sess.lastSafeTr` / hi = rawTr，20mm 粗扫括出**首个**穿越小段（防隧道穿越）→ ≤14 轮二分 → 自由侧 1nm 微退；锚点 null / 零长 / 陈旧（自身违谓词）/ 粗扫不可达 → fail-open raw。
3. **attract = 解析首触**：末帧位移方向（零位移帧退化质心连线）；`t ≤ 10mm` 吸到触点 −1nm；多邻居 t 最小、平手 key 最小。
4. **谓词 = 基线不恶化**：computeOverlap 总面积 ≤ `sess.startOverlapMm2 + 1e-9`（存量重叠不清不增）；候选违 clamp（y∈[0,gate]、minX≥0，容 1e-6）或内部异常 → 弃用回落 raw（fail-open）。
5. **多副本按 key**：绝不 pid 去重（求和 + 寻址双锁）。

### 验证

- `npx tsc --noEmit` 干净 + vitest 全量 **1060 passed**（65 文件，1038+22）+
  `npm run build` 过（305.07KB / gzip 99.81KB，snap.ts 尚未进组件图 —— US-003 接线后入包）、
  static/ 已重建；零后端改动、零组件改动（纯函数层，无 SVG/UI 变更，浏览器验证不适用本故事）。

## edit-drag-snap US-003 落地（EditCanvas 贴附会话接线；2026-09-06；**2026-09-17 迁移：触发手势右键 → Alt+左键**）

> 2026-09-17 追记：原触发 = 右键（`button===2`），与 Edge 内置「鼠标手势」冲突 —— 手势在浏览器层拦截右键按住拖动（画轨迹执行后退/关标签页等），Microsoft 官方确认网页无法用 JS 压制（preventDefault/contextmenu 拦不住，chess.com 右键画箭头同款），且右键向左拖会被识别为「后退」直接导航离开页面丢编辑态。用户定案**整体切换 Alt+左键**：纯左键零回归红线不动（修饰键+左键同样不占左键自由拖动）；右键全域失效（无任何画布会话）；contextmenu 吞除维持（防误弹「后退」类系统菜单）；snap 标志 pointerdown 锁存（拖动中松开 Alt 不影响松手求解）。下表描述以迁移后口径为准。

### 文件

| 文件 | 改动 |
|------|------|
| `src/components/edit/EditCanvas.tsx`（扩） | 组件头注 US-003 分节；import snap.ts 三符号；常量 `PARTNER_FLASH_MS=900`/`PARTNER_FADE_MS=300`；`MoveDrag` 加 `snap: boolean`（贴附会话标志，pointerdown 按左键 altKey 锁存）；refs 三枚（`snapSessRef: SnapSession\|null` / `partnerGRef` / `partnerTimerRef`）+ 骨架重建/卸载/pointercancel 三路作废清理；`ensureUiLayers` 第三子层 partnerG（子层序 [partnerG, overlapG, handleG]）；refreshMetrics try 分支 lastSafeTr 免费续写（首帧落基线 + 谓词同 eps）/catch 分支保守跳过；`flashPartner`/`clearPartner`/`cancelPartnerFlash`（绿色虚线 #2ea06c + 900ms 常驻 + 300ms 淡出）；`applySnapOnRelease`（取清会话态 → 防御链 → 幂等 clamp → computeSnapCorrection → kind≠free 经 commitDragPlacement 唯一落笔出口 + flashPartner）；onPointerDown 顶部按键门控（`button!==0` return 仅左键进门）+ 毛版分支 `snap = e.altKey` 起会话（Alt+左键 = 贴附、纯左键 = 既有路径零改动；旋转柄/空白分支不分流修饰键）；endDrag 末 `d.mode==='move' && d.snap` 分支；svg `contextmenu` preventDefault native listener（2026-09-17 迁出后维持吞除）；指南卡「贴附：Alt+左键拖动松手贴附」行 |
| `src/lib/snap.ts`（扩一行） | `COLLIDE_AREA_EPS_MM2` 改 export（原值 1e-9 不变）—— EditCanvas 跟踪谓词与引擎判定必须同 eps（否则跟踪位可能落在引擎谓词违例侧 → fail-open 白白退化吸附） |
| 测试 | `components/edit/__tests__/EditCanvas.test.tsx` +12 项（1072−1060，头注 14) 分节注册；2026-09-17 迁 Alt+左键后同 describe 改口径复用）：贴附 describe —— attract 6mm 缝（x≈500+1e-9 + 指标 0.00 + 伙伴 points/stroke + rot/mirror 原值）/ retreat 单跳（中途违例如实显示 50000 → 松手归 0）/ 两帧拖动 lastSafeTr 锚点精确回 [500,0] / 缝 60mm>10 free / 纯左键重叠原样 [400,0]（回归红线）/ 吸附后纯左键拖离不回吸 / rot180+mirror 片只改 translation / 非左键门控（中键/右键不起拖动/旋转/平移/贴附，右键毛版全域失效为迁移后新增断言）/ contextmenu 吞除（svg + 裁片）/ 伙伴高亮 1400ms 消退 / Alt+左键无位移同款选中 / 指南卡文案；`firePointer` 加 button/alt 双参（缺省 0/false 既有用例零改动，贴附用例 `0, true` 派发 altKey）；UI 覆盖层子层断言 2→3 |

### 关键不变量（edit-drag-snap US-003 立，后续故事不得破坏）

1. **左键路径逐字节不变（回归红线）**：button===0 的拖片/平移/转柄、applyKeyTransform（L/K/空格/O/I/R）、applyRotateFrame 与吸附引擎零耦合 —— 引擎只被 `snap:true` 会话的 pointerup 调用；吸附结果只改 translation（rot/mirror 读 working 现值透传）。
2. **吸附落笔唯一出口**：一切吸附终态经 `commitDragPlacement`（working + DOM 5 层 + 池增量 + 指标/手柄同帧），无旁路 setAttribute/store 写。
3. **拖动帧零新增计算**：lastSafeTr 由 refreshMetrics 既有 computeOverlap 顺带续写（rAF 合帧管线不变）；degraded 帧保守跳过（锚点停末个好帧，全程 degraded = 无锚点 → retreat fail-open 回 raw = 与不吸附一致）。
4. **会话态生命周期**：Alt+左键 pointerdown 起（基线 Infinity 占位、首帧 refreshMetrics 落真实值）、pointerup 求解时**先取后清**（落笔触发的 refreshMetrics 不再续写）、pointercancel/骨架重建/组件卸载一律作废不求解；纯左键会话显式置 null。
5. **非左键零会话**：右键（2）/中键（1）/后退（3）/前进（4）不起拖动/旋转/平移/贴附任何会话（2026-09-17 迁出后右键全域失效）；右键空白不平移、不取消选中、不弹菜单（contextmenu preventDefault 覆盖 svg 全域）。
6. **伙伴高亮层不挡交互**：partnerG `pointer-events:none`、双层计时（900+300ms）重复吸附重置、deselect/重建/卸载取消计时清层；颜色 #2ea06c = 旋转手柄同主题辅助色（与红色重合告警语义区分）。

### 验证

- `npx tsc --noEmit` 干净 + vitest 全量 **1072 passed**（65 文件，1060+12）+
  `npm run build` 过（309.68KB / gzip 101.34KB，snap.ts 随 EditCanvas 接线进组件图）、
  static/ 已重建；零后端改动。
- 浏览器验证（Playwright + Edge headless，真实 5336 母版 → 20s 求解 30 片 → 编辑弹窗）
  **17/17 PASS**（2026-09-06 首验记录，当时触发 = 右键；2026-09-17 迁 Alt+左键后
  单测 64/64 全量复绿 + 冒烟脚本同步改口径，见 US-004 节）：期望值由 Node 侧导入**真实引擎 esbuild bundle**（snap.ts+overlap.ts）
  在帧数据上预计算 —— B0 左键 +30 原样 / B1 右键 5mm 缝 attract（DOM points 串 ==
  引擎 tr、伙伴轮廓 == partnerKey 世界轮廓、指标 0.0）/ B5 高亮 ~1.2s 消退 /
  B6 contextmenu 吞除（synthetic + trusted click，无吸附位移）/ B4 右键拖离远端 free
  原样 + 指标 0.0 / B2 右键深重叠 retreat（points == 引擎 tr、回退 393.9mm、
  全邻居布尔交 0.0mm²、指标 0.0）/ B3 左键深重叠原样（8027mm² 重合如实保留，永不吸附）/
  B7 指南卡文案 / B8 dirty 丢弃确认 → 重开回保存版（multiset 全等）不炸。

## edit-drag-snap US-004 落地（冒烟脚本与文档同步；2026-09-06）

贴附拖动松手贴合收官故事（2026-09-17 起触发 = Alt+左键）：`scripts/smoke_drag_snap.mjs` 双护栏端到端冒烟 + 吸附语义
文档化（AGENTS.md US-004 节 + 本节）。零后端 / 零 src 改动（只新增 scripts/ 与文档）。
2026-09-17 手势迁移同步：B/C 段全部贴附拖动改 `(button:0, altKey:true)` 派发、
B5 保留 trusted 右键吞除回归 + 新增 B5c trusted Alt+左键真实输入链检查、
dragPiece 助手签名扩 `(el, dxMm, button=0, alt=false, midContext=false)`。

### 文件

| 文件 | 改动 |
|------|------|
| `materialSorting-web/scripts/smoke_drag_snap.mjs`（新；2026-09-17 迁 Alt+左键 + 新增 B5c 后 31→32 检查点） | Playwright 冒烟（模板 smoke_edit_polish.mjs 流程骨架 + smoke-band-preview.mjs 的 addInitScript 预置 ms.tour.*）：S1 上传 5336 → 3 码 20s 求解 30 片 → 基线 PLT 导出（placed 与 WS 末帧逐位全等）；S2 编辑弹窗 + **真实引擎预模拟选片对**（esbuild 即时 bundle `src/lib/{snap,overlap,editGeometry}.ts` 到 out/smoke_drag_snap/engine_bundle.mjs，Node 在 WS 帧数据上跑 predictSnap(S,R) 单跳会话模型，attract/far-free/retreat/final-attract 四路全预测通过才采用 —— 全流程确定性）；B Alt+左键 pointer 序列行为段（attract DOM points == 引擎 tr / 伙伴高亮轮廓 == partnerKey 世界轮廓 / 指标 0.0 / 高亮 ~1.2s 淡出 / trusted 右键 contextmenu 被吞 + 无位移 + 选中保持（右键全域失效回归）/ trusted Alt+左键无位移（真实输入链）/ 拖动中合成 contextmenu 被吞 / 拖离远端 free / 深叠 retreat + 独立布尔交复核）；R 纯左键红线两档（深叠压线原样 >1000mm² + 小缝落点原样永不 attract）；C 末次 attract → 保存 → 导出（placed translation 与引擎值 \|Δ\|≤1e-6 双轴 + 亚 0.01mm 精度未截断 + 终态接触几何首触 ≈1nm/全邻居布尔交 0 + 其余 29 项与求解末帧逐位全等 + PLT 未动片轮廓区逐位全等 + 被拖片轮廓 = 基线平移吸附位移 ±2 unit）；报告 `out/smoke_drag_snap/report.json` + 逐相位截图 + 基线/末态 PLT 正文 dump |
| `materialSorting-web/AGENTS.md`（扩） | 文末「edit-drag-snap US-004 关键约定」节：**吸附语义六条单一真相**（触发 = 仅 Alt+左键贴附会话（2026-09-17 起，原右键迁出）/ 求解 = 松手一次 / 效果 = 只改 translation / 永不吸附路径回归红线 / ATTRACT_MAX_GAP_MM=10 / erode 口径脚注**引用 overlap.ts 文件头原文**——单一真相源在 src/lib/overlap.ts，只引用不复制防两处文案漂移）+ 冒烟脚本运行口径与三处实现坑备档 |
| `.docs/technical/agent-component-map.md`（扩） | 头部覆盖行 + 本节（EditCanvas/snap.ts 条目的吸附语义同步归 AGENTS.md US-004 节与本节互为索引） |

### 关键不变量（edit-drag-snap US-004 立，复跑/扩检查点前必读）

1. **期望值口径 = 引擎自己的答案**：冒烟绝不自算吸附坐标 —— Node 侧 `predictSnap(S,R)`
   单跳会话模型（`sess = {lastSafeTr: ovAt(R) ≤ 基线+EPS ? R : S, startOverlapMm2: ovAt(S)}`
   与浏览器事件流逐帧一致），DOM points 串用 pointsStr 的 r2 复刻（键盘用例数学锚点法）
   逐字节对拍；引擎 bundle 由 esbuild 即时构建 src 源码（非复刻、非 snapshot）。
2. **y 链不变量**：attract 沿质心连线方向移动（**y 分量非 0**）—— 首次吸附后整条
   断言链的期望坐标必须继承 `res1.tr[1]`（写死起手 t0[1] 会让 B6/B7/R2/C 全链错位，
   首跑实锤教训）；`R1f = [R1[0], res1.tr[1]]` 是链上重访位的正确基线。
3. **左键零回归红线两档是端到端门**：R1 深叠压线（原样保留 + 重叠 >1000mm² 如实
   显示 + 指标面板同步 + 无伙伴高亮）+ R2 同小缝目标落点原样（attract 永不发生）——
   与单测「左键重叠原样 [400,0]」互为犄角，改 EditCanvas 按键门控即红。
4. **PLT 回读判据 = 未动片轮廓区逐位全等**：整文笔画数漂移属预期差分（门幅框随
   料长伸缩 / 被拖片标注笔画可被 gate 裁剪分裂 / 右侧表格 x 随 width_mm 平移且数字
   字宽可变）—— 只做 \|Δ\|≤12 宽松带；零回归判据 = stroke[0]=门幅框、
   stroke[k+1]=placed[k] 毛版轮廓的**未动片索引 1..30（除被拖片）字节级全等**，
   被拖片轮廓 = 基线笔画平移吸附位移（±2 HPGL unit x/y 双轴，_plt_pt 逐点取整
   复合差 ≤1 unit）。
5. **全精度贯通断言**：保存后导出 `placed[KI].translation` vs 引擎 `resF.tr`
   |Δ|≤1e-6 双轴 + `trOut[0] !== r2(trOut[0])`（亚 0.01mm 精度未被 DOM 显示口径
...
- `npx tsc --noEmit` 干净 + vitest 全量 **1072 passed**（65 文件，零新增用例）+
...
**根因**：编辑画布（渲染/红字指标/吸附池/料长包络）此前消费 `manifest.pieces[].polygon`
= per_type d 腐蚀后的 solver 碰撞轮廓，而 /export（PLT/PNG/DXF）与 polish 报告用
intermediate **原始毛版轮廓**重放同一组变换 —— per_type d=5 时每片每边小 5mm，画布
腐蚀轮廓贴触（显 0 重合）而物理毛版实际重叠最多 d_i+d_j（PLT 可见）；「切到毛版后
裁片变小」= 完整版/毛版两模式画同一份腐蚀轮廓（净版线接近真实外缘造成视觉差）。
注意 d=0 时 polygon 也过 `_clean_polygon`，与 raw 顶点级不同（数学锚点必须同源 raw）。

**方案 A（用户定案）：全端统一物理毛版口径** —— manifest additive 增发
`raw_polygon`（原始轮廓）+ `d_mm`（腐蚀距离），前端一切数值/渲染口径切
`physicalPolygon(p)`（raw ?? polygon 老后端回退），erode 轮廓降级为灰虚线参考线；
求解引擎压线排料行为**零改动**（sparrow 照旧吃 erode 轮廓，合法压线是设计特性）。
腐蚀「细窄部位断裂只取最大块」的次生隐患本次不处理、先观测。

| 文件 | 改动 |
|------|------|
| `materialsorting/web/solver.py`（扩） | `build_pid_meta` 增发 `'raw_polygon': p['polygon']`（腐蚀前快照）+ `'d_mm': d`（d>0 才 erode；之后照旧 `_clean_polygon`）—— additive，老前端忽略新键零影响 |
| `materialsorting/web/routes_ws.py` / `strategy.py`（扩） | manifest 透传 `'raw_polygon': meta.get('raw_polygon') or meta['polygon']` / `'d_mm': meta.get('d_mm', 0.0)` |
| `src/types/piece.ts`（扩） | `PieceInfo` 增可选 `raw_polygon?: Polygon` / `d_mm?: number`（旧 manifest 类型兼容） |
| `src/lib/geometry.ts`（扩） | `physicalPolygon(p)`：raw ≥3 顶点返回 raw、否则回退 polygon —— 物理口径单一真相源（画布/吸附/包络/主视图四处消费） |
| `src/lib/overlap.ts`（扩） | EditPiece 增 `dMm`；池 basePolygon = physicalPolygon；`OverlapResult.allowanceMm` = 实际相交邻居 max(d_i+d_j)（压线额度）；文件头「几何口径」段 = 全端口径脚注单一真相源 |
| `src/components/edit/EditCanvas.tsx`（扩） | 指标着色三态：穿透 > 额度 红 / >0 琥珀（设计允许压线）/ 0 正常（深度值 title 悬浮解释额度）；refreshMetrics 降级路径同口径近似额度；五处几何锚（snap/键盘/拖动/旋转/渲染）全切 physicalPolygon；脚注改「按毛版轮廓口径（与导出一致）· 穿透≤压线额度为琥珀」；polish 按钮 title 同步 |
| `src/components/nests/pieceDom.ts`（扩） | PieceEntry 增 `collideEl`：d_mm>0 才建灰虚线 erode 参考线（#8a8f98 dash 3 2 / fill none，初始隐）—— EditCanvas full 显 rough 隐，NestSVG 主视图恒显 |
| `src/components/nests/NestSVG.tsx`（扩） | 主视图毛版 layer1 points = raw_polygon（与导出一致）+ collideEl 恒显 |
| `src/store/editStore.ts`（扩） | computeLayoutStats 包络按 physicalPolygon（料长/密度与导出同口径） |
| `src/lib/snap.ts` | 零改动（池自动跟随物理口径 —— 吸附按毛版贴触） |

**测试与验收**：后端 pytest **747 passed**（manifest additive 用例）；前端 vitest
**1083 passed**（+14 回归锁：overlap「腐蚀轮廓不相交但物理相交」= bug 本体锁 /
allowance=max(d_i+d_j) / 老后端额度 0 / physicalPolygon 三态 / computeLayoutStats raw
包络 / EditCanvas caliber 夹具 collideEl + 琥珀/超额度红 / NestSVG raw+恒显）+
tsc 干净 + build 过；浏览器冒烟三脚本全绿 —— `smoke_edit_layout` 28/28（S3d 脚注断言
随口径更新）、`smoke_edit_polish` 46/46（S2g title 口径断言更新 + S8 质心锚定/points
...
## 会话过期自动恢复 US-003 落地：启动期恢复与停留期引导弹窗（2026-09-13）

prd-session-expiry-auto-recovery 前端收官（消费 US-001 checkpoint 内存存储 + US-002
`/api/state-recover` 端点）：启动期探测 401 自动换 sid 恢复工作状态（母版/数量矩阵/
布局回显 + 自动切超排 Tab）；停留期 401 不自动恢复、阻断弹窗引导刷新（刷新 = 启动期
恢复入口）。多会话 US-005 节不变量 #1/#4 的 amend 见该节内联标注。

### 新增 / 改造文件

| 文件 | 改动 |
|------|------|
| `src/lib/session.ts` | 增 `peekPersistedSessionId()` **非铸造只读**（缓存→localStorage 双查；空/损坏 → null，绝不 mint 绝不写库）。**时序红线**：恢复链固定 `peek → clearPersistedSessionId() → getSessionId()` —— getSessionId lazy-mint，先 get 会铸造新 sid 覆写 localStorage，旧 sid（from_sid）永久丢失（sessionRecovery.test + session.test 时序链锁死）。clearPersistedSessionId 唯一调用方自此收敛为 sessionRecovery.ts。 |
| `src/lib/api.ts` | ① `registerStartupSessionRecovery(fn)` / `startupRecovery` **钩子槽**（模块级依赖倒置：api.ts 保持 store-free，恢复逻辑全在 sessionRecovery.ts；App.tsx 静态 import 保证注册先于任何 effect 驱动的 apiFetch）；② ensureSession 探测分支扩 **401 三分支**：`code==='session_expired'` 且钩子在 → `await startupRecovery()`（单飞：sid 换新 + recover + 重探全在 once-promise 内，并发 apiFetch 等终态、原请求在探测门前未发出无需重放）→ blocked 则吞、否则**裸 fetch 重探**（特权点 ①）再走 inspect；401 无 code → 不恢复不阻断放行（防网关 401 误触发）；无钩子 → 落回老路径阻断弹窗（装载序缺口防御，api.test 钉死）；③ 抽 `readSessionErrorCode`（clone 读 code，本体留给调用方）；④ `triggerSessionBlock` **不再清 sid**（US-005 顺手弃 sid 废止 —— 旧 sid 留作停留期刷新后恢复的 from_sid）。 |
| `src/lib/sessionRecovery.ts` | **新建** 恢复编排唯一所在（lib→store import 以 stateFile.ts 为先例）：`runRecovery()` = peek 旧 sid → clear → `getSessionId()` 铸新 → `setSessionRecovering(true)` → **裸 fetch POST /api/state-recover**（特权点 ②；头 X-Session-Id=新 sid + body `{from_sid}`，空 sid 空对象）→ 200 = `applyRestorePayload`（状态文件 US-004 同函数零改动复用：五 store 落笔 + run 合成 + 自动切超排 Tab）+ toast「工作状态已恢复」；404（checkpoint 不在/超 TTL/已消费 single-use）或网络错 → toast「上次会话已过期，已开启新会话」（干净新会话）；429 → `triggerSessionBlock('session_limit')` 走既有满员弹窗。`recoverExpiredSession` 外层 try/catch 兜底到新会话 toast + finally 复位 sessionRecovering；模块加载尾 `registerStartupSessionRecovery(recoverExpiredSession)` 自注册。TOAST_RECOVERD/TOAST_NEW_SESSION 常量导出（测试逐字锁）。 |
| `src/store/uiStore.ts` | 扩 `sessionRecovering: boolean`（缺省 false）+ `setSessionRecovering`（轻加载语义字段；唯一写入方 = sessionRecovery）。 |
| `src/components/SessionRecoveryNotice.tsx` | **新建** 恢复期轻提示：idle → null 零 DOM；recovering → `.session-recovery-notice[role=status][aria-live=polite]` 顶部居中「正在恢复工作状态…」（pointer-events:none 不挡交互；订阅 uiStore.sessionRecovering）。 |
| `src/components/SessionExpiredModal.tsx` | COPY.session_expired.text 改版 → **「会话已过期，刷新页面后将恢复工作状态」**（title「会话已过期」与 session_limit 文案不变；刷新按钮 = 启动期恢复入口）。 |
| `src/App.tsx` | 静态 `import './lib/sessionRecovery'`（副作用 import：注册恢复钩子）+ `<SessionRecoveryNotice />` 单例（SessionExpiredModal 与 Toast 之间）。 |
| `src/style.css` | `.session-recovery-notice`（fixed top:14px 居中、z-index 1400、pointer-events:none、#26282e 卡 + #2ea06c 左边框，同全站色系）。 |
| `scripts/us003_recovery_verify.mjs` | **新建**（materialSorting-web/scripts/）浏览器验证 20 检查：A 相位启动期恢复全链（上传→commit→矩阵改值→求解→state-save 载荷手工 POST /api/state-checkpoint 替代打点（US-004 自动调度未落地）→空闲过期→reload：sid 换新 + __netLog 佐证 recover from_sid + 探测两跳 401→200 + toast + 自动切超排 Tab + polygon/矩阵实值回显）+ B1 停留期（弹窗新文案 + ms_sid 未清 + 零新 recover 调用）+ B2 刷新链（checkpoint 已消费 → 404 兜底干净新会话）。**前置**：`MS_SESSION_TTL_SEC=45 MS_SESSION_MAX=6 MS_EDIT_HOLD_SEC=5` 起服 + prod static。 |

### 关键不变量（后续故事不得破坏）

1. **恢复单飞在 ensureSession once-promise 内** —— 探测 401 → await 恢复 → 重探全链
   在 once 内；并发 apiFetch 等同一终态；业务请求在探测门前未发出，**无需请求重放**
   机制。
2. **peek 必须先于 clear**（时序红线）—— getSessionId lazy-mint 会覆写旧值；恢复链
   固定序 peek→clear→get。
3. **停留期（探测已落定后的 401/WS error）不清 sid、不自动恢复** —— triggerSessionBlock
   与 useSolveRun WS 分支保持现状；旧 sid = from_sid，刷新即恢复。
4. **裸 fetch 恰两特权点**（api.ts 探测重探 + sessionRecovery recover），均恢复链专用、
   手工拼 sid 头；业务请求零裸 fetch。
5. **恢复响应三分支路由固定**：200 恢复 / 404·网络错兜底新会话 / 429 满员阻断；
   任何新状态码默认落兜底分支（静默 + 新会话 toast），不炸 UI。
6. **后端语义（测试必读）**：恢复成功即消费 checkpoint（single-use，双次恢复第二次
   404）；恢复后会话被 edit_hold 钉住（生产「恢复后一段时间内不再过期」—— 落地时
   缺省 2h，**2026-09-13 当日稍后已收敛 600s 对齐 TTL**，用户定案：checkpoint 保存
   兜底落地，2h 钉住不再必要）—— 测「恢复后再过期」相位仍须 `MS_EDIT_HOLD_SEC=5`
   短值起服（浏览器验证实锤踩坑：钉住窗 ≥ TTL 下 B 相位死活不过期）。

### 测试与浏览器验证

vitest **1153 全绿**（68 文件；新增 sessionRecovery.test 10 —— mockRoute 按
X-Session-Id 路由：200 全链 URL 序 `/api/session→/api/state-recover→/api/session→
/api/ptypes` + from_sid + 新 sid 全场 + store 断言 + 自动切 Tab / 404 兜底 / 429 阻断
恰 2 请求 / 网络错 / 3 并发单飞 / sessionRecovering 生命周期 / 健康探测零 recover /
停留期 markSessionProbedForTest → blocked + sid 保留；session.test +5（peek 非铸造
5 项含时序链）；api.test AC8 改写 + 探测 401 两分支 +2；SessionRecoveryNotice 3；
uiStore 2）+ `tsc --noEmit` + `npm run build` 全绿。浏览器
`scripts/us003_recovery_verify.mjs` **20/20** + 恢复后视觉数值对拍（nest-label
"seed 0 · 80.60% · 长度 190.67 cm" == checkpoint run.final density
0.8059803801111168 / width 1906.6954345703125mm 精确一致 + 60 polygon）。

## 会话过期自动恢复 US-004 落地：前端自动 checkpoint 调度与启动清理（2026-09-13）

会话过期时刻的「快照新鲜度」取决于最后一次 checkpoint —— 本故事让前端在工作台
状态变更时**自动**打 checkpoint（无需手动「保存当前方案」），丢失窗口 ≤ 3s 去抖
（求解完成/编辑保存等价值最高时刻立即落，不等去抖）；会话存活时 F5 仍是干净重置
（启动清理 DELETE 旧快照，防「稍后再过期恢复出 F5 前旧状态」幽灵回潮）。US-003
节全部不变量延续有效（本节零触碰恢复链本体）。

### 新增 / 改造文件

| 文件 | 改动 |
|------|------|
| `src/lib/sessionCheckpoint.ts` | **新建** 自动调度唯一所在（App.tsx 静态 import 装载接线，sessionRecovery 同款先例）：① **触发面** —— formStore/qtyStore 变更订阅（引用比较）→ `scheduleCheckpoint()` 去抖 `CHECKPOINT_DEBOUNCE_MS=3000`（常量可调，2026-09-13 定案）；`subscribeRunDone` 观察者 → bestRun 达 done 立即 flush（无 lastFrame 的 error run 跳过；非 best 晚到 run 跳过 = 内容无增量）；editStore 订阅识别 **save 专属写序**（`savedDirty false→true 且 working 引用未换`——save 只 set savedDirty，open 必换 working 新数组，重开不误触）→ 立即；`visibilitychange→hidden` 立即 flush 清空去抖；`pagehide` → keepalive fetch 同步发起（不走 sendChain —— 卸载期微任务不可靠）。② **发送**：一律 `apiFetch POST /api/state-checkpoint`（统一出口不变量；端点本身 peek 口径不刷活性）+ body=`buildSavePayload()` 不带 save_as + **无 doc 静默跳过**（uploadStore.doc null 不发）+ 全异常吞（SessionBlockedError/网络错/`{stored:false,empty/conservation}` 均零处理零重试）。③ **串行+合并**：sendChain promise 链保 POST 依序完成（防乱序回写覆盖）；载荷**发送时刻**现取 → 排队中 send 存在时新 flush 直接跳过（合并）。④ `clearCheckpointAfterProbe()` 启动清理：blocked 或 `wasStartupProbeExpired()` → 不清；否则 DELETE。`resetCheckpointForTest()` 清定时器/链。 |
| `src/lib/api.ts` | 增 `probeExpired` 模块级旗标（探测 401 session_expired 且恢复分支接管时置位）+ `wasStartupProbeExpired()` 读取 + `resetSessionForTest` 一并复位。 |
| `src/store/runRegistry.ts` | 增 done 观察者机制：`RunDoneListener`/`subscribeRunDone(fn)`（Set + 退订）+ `markRunDone(rec)`（幂等；done 置位与通知原子，观察者异常吞）。**run 结束单一出口** —— useSolveRun.finish 与 applySyntheticRun 两处调用。 |
| `src/hooks/useSolveRun.ts` | `finish()` 内 `rec.done = true` 改 `markRunDone(rec)`（再跑 onDone 回调；行为零变更，仅多观察者通知）。 |
| `src/store/synthRunStore.ts` | `applySyntheticRun` 字段全落位后 `rec.done = true` 改 `markRunDone(rec)`（观察者读到终态；registry 只此一条 bestRun 必命中 —— 策略应用/启动期恢复完成即自动重落快照，恢复链在新 sid 下自我延续）。 |
| `src/App.tsx` | 静态 `import './lib/sessionCheckpoint'`（装载接线）+ 挂载 effect `probeSession().then(() => clearCheckpointAfterProbe())`（探测落定后启动清理；StrictMode 双跑 DELETE 幂等无害）。 |
| `src/lib/__tests__/sessionCheckpoint.test.ts` | **新建** 16 例：去抖合并（窗口内 4 变更恰 1 发/最新值/无 save_as）+ 重排窗口 + empty 容忍不重试 + doc 空跳过（变更+hidden 全程零请求）+ hidden 立即（不待去抖 + 不补发 + visible 不触发）+ 求解完成立即（bestRun 门：更差跳过/更优再发/无帧跳过）+ 编辑保存立即（拖草稿不发/save 落定发/重开不误触）+ pagehide keepalive（载荷在场 + blocked 闸）+ 阻断吞错 + 启动清理三态（存活 DELETE 恰一次 / 401 恢复不清且恢复态重落 checkpoint / 阻断不清）。 |
| `scripts/us004_checkpoint_verify.mjs` | **新建** 浏览器验证 15 检查（默认 TTL 起服，不涉过期）：A1 上传 → A2 改数量去抖一发 stored:true（载荷实值/无 run/无 save_as/sid 同源）→ A3 求解完成立即 checkpoint（最后一条含 run 块 + density/placed）→ A4 F5 存活（DELETE 恰一次 + 探测 200 + 零 recover + 干净重置空态）→ A5 重传同母版矩阵默认值（g01@30=1/总 110 = 非恢复态 + 无恢复 toast）。 |

### 关键不变量（后续故事不得破坏）

1. **checkpoint 调度一律 apiFetch**（统一出口）—— 不新增裸 fetch 特权点（仍恰两处，
   US-003 节 #4）；停留期过期时调度 POST 的 401 照常触发阻断弹窗（用户刚改过东西
   = 在场），调度侧只吞错不重试。
2. **run 结束必经 markRunDone** —— 新增任何合成/结束 run 的路径（未来新 run 来源）
   必须调它，否则求解完成自动 checkpoint 断链。
3. **启动清理门序**：`blocked → 跳过`、`wasStartupProbeExpired() → 跳过`（快照留给
   恢复消费 / 恢复成功时后端 single-use 已删）、否则 DELETE —— 三态顺序不可换
   （401 恢复路径里 applySyntheticRun 触发的重落快照依赖 DELETE 被跳过）。
4. **载荷发送时刻现取 + sendChain 串行** —— 改调度时不得把 buildSavePayload 提前到
   入队时刻（合并语义依赖现取）。
5. **editStore.save 识别依赖写序形状**（savedDirty 翻转 + working 引用不变）——
   editStore 若改 save 的 set 结构（例如 save 同时改 working），需同步改识别判据。

### 测试与浏览器验证

vitest **1169 全绿**（69 文件，+16）+ `tsc --noEmit` + `npm run build` 全绿。浏览器
`scripts/us004_checkpoint_verify.mjs` **15/15**（真实链路：去抖 checkpoint stored:true
实拍 + 求解完成立即含 run（density 0.8059803801111168 / placed 30）+ F5 存活 DELETE +
重传默认矩阵）。**生产实拍注记**：上传 hydrate（doc 物化默认矩阵）也会排一发去抖，
若服务端 commit 未完成则后端 200 `{stored:false,reason:'empty'}`（PRD 既定容忍路径，
前端零处理不重试）—— 冒烟断言按「改数量那一发」锚定而非总计数。

## 策略待确认结果 US-002 落地：拉取即落快照与恢复还原弹窗态（2026-09-13）

后端 US-001 已把 checkpoint/.msn 载荷扩出 `pending_strategy_result` 槽（待确认的
策略/极限 done 结果，省键式 additive）；本故事补前端两侧 —— **产**：done 结果拉取
落定（或恢复写回）即随载荷入档（自动 checkpoint 触发面 #4 立即落，不等去抖）；
**消费**：恢复端响应带槽 → 第 6 步路由写回所属族 store 并自动开弹窗在结果态
（新 sid 后端槽恒空，idle 采纳守卫防轮询打回）。应用落定后槽退场 —— 已应用结果由
run 块承载（应用动作经 markRunDone 触发面自动重落快照，无双份数据）。

### 新增 / 改造文件

| 文件 | 改动 |
|------|------|
| `src/store/strategyStore.ts` | createRunStore 家族工厂 RunState 扩 `resultApplied: boolean`（初值 false）+ `markResultApplied()`（置位幂等）；start()/reset() 随 `result:null` 一并清；**refresh idle 采纳守卫** —— 家族过滤后 `st.state==='idle' && result!==null && phase==='done'` 直接 return（idle 载荷不写回、不触发 result 拉取）；两族实例（useStrategyStore/useExtremeStore）同享。 |
| `src/components/NestingPage.tsx` | applyStrategyResult 在 applySyntheticRun 之后按 result.mode 委托 `markResultApplied()`（extreme → useExtremeStore / se·race → useStrategyStore）—— 槽门控位（与 markRunDone 同任务 → sendQueued 合并恰一发，载荷发送时刻现取：槽已退、run 块已入）。 |
| `src/types/stateFile.ts` | 增 `StatePendingStrategyResult {mode:'se'\|'race'\|'extreme', best: StrategyBest, summary: StrategySummary}`；`StateSavePayload.pending_strategy_result?`（省键式）/`StateRestoreResponse.pending_strategy_result: …\|null`（必选）。 |
| `src/lib/stateFile.ts` | ① `buildPendingBlock()` 槽产方：轮询 `[useStrategyStore, useExtremeStore]`，`result` 非空且未 `resultApplied` 且 `state==='done'` 且 mode 三枚举才产 `{mode, best:{...best, placed_items: deepCopyPlaced(…)}, summary}`（stopped 人为操作不入槽 = 后端契约；无路由键的 null-mode 不入）；buildSavePayload 槽在场才带键。② applyRestorePayload 增**第 6 步**：槽在场 → mode 路由写回族 store（extreme → useExtremeStore / se·race → useStrategyStore）：`setState({phase:'done', status:null, result:{state:'done', mode, run_dir:null, manifest:{...res.manifest}, best, summary}, resultApplied:false, errorMessage:null, lastStart:null})` + `openModal(mode==='extreme'?'extreme_run':'strategy_run')` 自动开弹窗（manifest = 恢复端重算值透传 = 单份几何；run_dir 恢复不可知恒 null 前端不展示）；槽缺席 no-op（旧文件零迁移）。 |
| `src/lib/sessionCheckpoint.ts` | 触发面 #4（原 #4 hidden→#5、#5 pagehide→#6 顺延）：两族 store `subscribe` —— `result null→非空`（拉取落定 / 恢复写回同触发）或 `resultApplied false→true`（应用落定）→ 立即 `flushCheckpoint()`；清 result（start/reset）不触发。 |
| `src/store/__tests__/strategyStore.test.ts` / `extremeStore.test.ts` | +5 / +3 例：resultApplied 生命周期（初值 false / done 落定仍 false / 置位幂等 / start+reset 清）+ idle 守卫三态（恢复写回态防降级且零 result 重拉 / result null 的 stuck starting 照常采纳 / phase≠done 照常采纳）。 |
| `src/lib/__tests__/stateFile.test.ts` | +12 例：buildSavePayload 槽 7（两族无 result 省键 / 逐字段形态 / extreme 入槽 / applied 退场 / stopped 不入 / 深拷贝解耦 / mirror omit-when-false）+ applyRestorePayload 第 6 步 5（race·extreme·se 三路由与对方族不动 / 缺席 no-op / fetch idle 端到端守卫闭环）；makeRestore 夹具补 `pending_strategy_result:null`。 |
| `src/lib/__tests__/sessionCheckpoint.test.ts` | +5 例：落定即 flush 携槽 / applied 后 flush 无槽 / markRunDone+markResultApplied 同任务合并恰一发（body.run 在场 + 无 pending）/ 旧口径去抖载荷无该键 / 极限族同触发 + reset 清 result 不触发；beforeEach 增两族 reset。 |
| `src/lib/__tests__/sessionRecovery.test.ts` | 夹具补 `pending_strategy_result: null`（类型必选化）。 |
| `scripts/us002_pending_verify.mjs` | **新建** 浏览器验证 21 检查（:8020 自起 uvicorn，env `MS_SESSION_TTL_SEC=60` + `MS_EDIT_HOLD_SEC=5`——后者必带，state-restore 会钉 edit_hold 600s 默认值会挡 TTL 过期）：Node HTTP parse-dxf + commit-to-nesting（真实 intermediate）→ Python 铸带槽 .msn（build_state_document）→ P3 上传恢复弹窗自动开结果态 + 落定 checkpoint 携槽 stored:true → P4 空闲 75s 过期 → 刷新启动期恢复（sid 换新 / recover from_sid=旧 / 弹窗重现密度对拍 / 恢复态重落 checkpoint 携槽）→ P5 应用（主画布 polygon 数对拍 + 状态行与来源小字 + PLT 导出 placed 逐条对拍 + 应用后 checkpoint run 块在场槽退场）。 |

### 关键不变量（后续故事不得破坏）

1. **槽恒 done、恒未应用** —— stopped 结果（人为操作）与已应用结果（run 块承载）
   均不入槽；buildPendingBlock 判据与后端 `_check_pending_save` 契约对齐，改判据
   须双侧同步。
2. **恢复写回态不被轮询打回** —— idle 采纳守卫是恢复弹窗结果态存活的必要条件
   （新 sid 后端槽恒空 → 弹窗打开/挂载即 refresh 恒见 idle）；守卫只护「result 非
   空且 phase==='done'」，不得放宽成 phase-only（stopped 常驻结果会被误锁死）。
3. **应用 = 槽退场 + run 承载** —— markResultApplied 必须在 applySyntheticRun
   （→markRunDone）之后同任务调用；sendQueued 合并语义依赖载荷发送时刻现取
   （入队早取会把已退槽误带入最后一发）。
4. **第 6 步路由只动归属族** —— extreme 槽只写 useExtremeStore、se/race 只写
   useStrategyStore，对方族保持 idle（双弹窗互不串台）。

### 测试与浏览器验证

vitest **1194 全绿**（69 文件，+25）+ `tsc --noEmit` + `npm run build` 全绿；后端
