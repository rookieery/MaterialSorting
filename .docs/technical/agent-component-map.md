...
| 文件 | 改动 |
|------|------|
| `src/components/ControlPanel/BandPreviewSVG.tsx` | **新建（2026-08-24，9d05fce 起兼 prefix）** 组合形态预览命令式 SVG（PiecePreviewSVG 同范式：React 只渲染空骨架 `<svg ref/>`，useEffect imperative 建翻转组 `translate(0 minY+maxY) scale(1 -1)`，Y-up→SVG Y-down 与 NestSVG/PNG 一致）。渲染三层：成员毛版（fill=`size_color` 半透明 0.55 + 同色实线边 —— 「码序降序 + 最大码在最右」一眼可辨）/ 组合片外轮廓 outline（erode 后，浅色虚线 —— 「主解看到的形状」）/ `showLabels` 码号叠印（仅放大模式，屏幕坐标定位在翻转组外防镜像）。数据 = `BandPreviewMember[]`（后端已变换到组合片归一坐标，前端零变换）；`member.tag` 存在时标注显示 tag（prefix = 前/后幅 g 码）替代码号。band 缩略/放大 + prefix 缩略/放大四处复用。 |
