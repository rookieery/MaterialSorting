...
2. **编辑数量**（US-011/022；矩阵化重构 US-001~005 改「裁片 × 尺码」数量矩阵；图形预览区已拆除）：QtyMatrix 每行一个裁片、每列一个尺码，全部码数量分布一屏看全；格内直接编辑每码排料份数（0 = 该码不排此片）、行头「填充」整行设默认值、个别格子改不同值高亮为特例；点行头缩略图放大查看裁片图形（US-013 PieceZoomModal，5 层）、点列头（码号）切换行缩略图显示的码；配对片行头「×2」徽章（1 份 = 左右 2 物理片），每码小计/总片数按物理片数计；数量随求解 start payload 按码下发（demand per-size）。
