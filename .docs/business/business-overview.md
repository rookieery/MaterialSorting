...
| 腰头成带（waist_band） | ✅ US-014 A/B accept | **US-009 核心模块**（`nesting_engine/waist_band.py`：带内聚排 → union → erode → 组合片 + 展开权威式）+ **US-011~013 编排/UI/预演** + **US-014 成对形态重试选解与 A/B 终验**（2026-08-21，报告 `.docs/business/腰头成带_AB验收报告_US014.md`）：3 seed 成对相邻率 100%（off 对照 15~38%）、密度均值**提升** 0.61pt（接受线 ≤1.0pt 劣化）、同 seed 重跑确定性/导出三格式 WB_ 零泄漏全过 → **v1 合入**。US-010 闸门 no-go（均值 +1.204pt）针对的是朴素单次解形态，被重试选解机制翻案（形态与密度同时达标）；曾试分码分块拼接 100% 成对但 −3.75pt 密度已否决。US-015 v1.1 混填料仍是后续增强方向 |
