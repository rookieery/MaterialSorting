...
| warm-start 热启动一期（warmstart） | 🚧 US-004 落地 | **sparrow `optimize(..., initial_solution)` 真顺延通道**（se 延长段灌冠军解续跑，取代同 seed 全新重放；跨项目拆分：spyrrow-ms 私有 wheel 以规格文档交付，MS 侧只做消费与切换）。**US-001 已落地**（`nesting_engine/warmstart.py`，2026-09-19）：`build_initial_solution` 校验矩阵（形态/未知 pid/每 pid 条数==demand 完整解硬约束/mirror 拒绝）fail-fast 中文 ValueError + `warm_start_supported()` 能力探测（`+ms<N>` 且 N≥1 才 True——ms0 纯重建 wheel 不算，PyPI/异常恒 False 不抛）；**US-002 已落地**（`initial_solution` 求解链纯透传：solve_pieces→solve_with_callback_proc（Process args 第 7 位）→solve_worker 最终消费点三闸门 + json.dumps 只在消费点，缺省 None 逐字节不变）；**US-003 已落地**（`--se-warm {on,off}` 默认 on：se 延长轮读 `best_frame_s{冠军}.json` 边车灌入热启动，回退矩阵（off/unsupported 前置 + no_best_frame/invalid_best_frame/no_composite_view 装载点 + worker 降级 instance_mismatch 等经 final warm_state 带回）任一命中回退现状重放+warn 不炸轮；strategy.json 计划态 + result.json/run_stats 实际灌入态 additive 同键，race/legacy 零新增键）；**US-004 已落地**（`scripts/spyrrow_wheel.py` 双源切换助手：status/use-pypi/use-local 一条命令在 PyPI 0.9.0 ↔ 私有 wheel 间切换，pip 走 sys.executable 不假设 PATH，use-local 错误矩阵清晰报错绝不触 pip，rev 钉板 spyrrow_build.json 只读+漂移提示不自动改写（四字段真相源在 spyrrow-ms 侧）；手册 = .docs/technical/spyrrow私有wheel构建与升级手册.md；ms0 与 PyPI 行为全等故日常生产零影响，本仓零 Rust 构建产物）；US-005 已收官（2026-09-19 ms1 联调 + se A/B PASS 保默认 on）。**二期（同日）band/prefix warm 解禁已落地**：worker `record_composite` 组合视角边车旁路（band/prefix 开时帧附展开前 solver 条目+组合 demand_map → best_frame 边车 `composite` 段 → 装载点构造含 WB_/PS_ 组合视角载荷 + worker `payload_universe_error` 宇宙复检；WS 路径恒 False 不进前端契约，WB_/PS_ 不出进程哨兵不变）。回归红线：不传 initial_solution 一切行为逐字节不变 |
| 机器对接排料 API（YL 排料对接二·三期） | ✅ 六端点全落地（二期 US-005 收官 + 三期 state-file 2026-09-22） | **YLPatternMaking 打版系统经 HTTP 接入 MS 排料引擎**（PRD `tasks/prd-machine-nesting-api.md`，2026-09-21 立项，15 项决策全定）：multipart 提交母版 DXF + config → 三档运行模式求解（normal=plain 180s / advanced=race 1200s / extreme 7200s，全默认参数零暴露）→ 轮询物理毛版包络口径利用率 → 终态 manifest+placed_items（demand>1 发 N 条绝不按 pid 去重）→ 会话无关导出 PLT（默认 plt-clean + 服务端全算表格）。**US-001 已落地（2026-09-21）**：后端端口 8000→8010（`MS_WEB_PORT` 可覆盖，与 YL 后端 :8000 同机并存）+ `web/machine.py` 骨架（空 APIRouter + `register_machine_routes(app)`，strategy 家族第三成员 mode='machine' 规划复用状态槽/marker/树杀骨架；禁 import ..cli.* 与顶层 server import 双 AST 守卫）+ `tests/test_web_machine.py` 5 例。**US-002 已落地（2026-09-21）**：`POST /api/machine/solve` 提交端点 —— multipart 母版（≤20MB 同 parse-dxf）+ config JSON（gate_mm 必填正整数、run_mode 三档缺省 normal、sizes/per_type/quantities/client_ref≤128 幂等键；time/seeds/band/prefix 在场 400 拒绝零暴露）→ 铸 'm' 前缀任务 sid（即 task_id）→ 复用 server parse+commit 管线（per-doc 快照只挂本任务会话、default `_PIECES_STATE` 零触碰对拍锁死，坏 DXF 422）→ `RUN_MODE_SPECS` 烘焙时间/种子写 cfg（normal 180 单 seed 0 / advanced `--strategy race` 1200 / extreme `--extreme` 7200）→ spawn 三档 cmd 金标 + marker 5 键 mode='machine' → 202 {task_id, run_name=machine_<task6>_<rand6>, started_at}；同 client_ref 在飞 409 返回既有 task_id；15 例测试（错误矩阵+三模式金标+default 不动）+ 真实冒烟（202 任务落 run_dir 含 commit 产物、同 client_ref 409 复现）全过，全套件 1106 passed 零回归。**US-003 已落地（2026-09-21）**：status/stop/result 三端点 —— strategy 三公共实现参数化（`sid` 显式传 + `gate=False` 跳会话闸门 + `drift=False`，会话 TTL 逐出后仍可读、缺省参数策略/极限八路由零变化）；GET status 恰 10 键无 placed_items（incumbent 物理毛版包络口径 portfolio 优先→best_frame 边车回落、运行中非法 curve 在场仍 200、total_budget_sec 按模式 180/1200/7200 透传；MS 重启后 orphan：pid 死超 30s 宽限 → error+stderr 尾 2000 字符、machine_cfg time 反查恢复 run_mode/预算、pid 存活可 stop）+ POST stop 树杀 taskkill /T /F 孙进程无残留、run_dir 保留 + GET result 恰三键 {manifest,best,summary}（running 409；placed_items 条数=Σdemand 绝不按 pid 去重；manifest 键集含 raw_polygon/d_mm/demand/color；stopped 档 best_frame 回落仍可读）；25 例测试 + 真实服务器冒烟（normal 全生命周期 advanced stop 双档）全过，全套件 1116 passed 零回归。**US-004 已落地（2026-09-21）**：`POST /api/machine/export` 会话无关导出 PLT —— body `{task_id, fmt?, placed?, table?}` 最小请求仅 `{task_id}`（fmt 缺省 plt-clean 毛版+唛架信息表格 / plt 全量版；placed 缺省用最优布局（portfolio.incumbent 优先→best_frame 边车回落，与 result 同源）、显式全未命中 400 既有兜底；table 缺省服务端全算 6 手输默认值+8 自动计算字段、显式非法 400 中文）；**pieces 直载 run_dir `pieces_intermediate.json`**（solve 事实源在盘 → 会话被 TTL 逐出后仍可导出、与 run 存活期逐字节一致）兜底回退会话快照→状态槽 start 快照；复用 /export 门面全部函数零改动（几何/表格/PLT 单一真相源），Content-Disposition 中文/ASCII 双写；30 例测试（默认档逐字节金标 + 会话逐出对拍 + 三源兜底链）+ 真实服务器冒烟（done 任务导出 78942B 毛版 PLT、二次导出逐字节一致、plt 全量版/显式表格/错误矩阵）全过，全套件 1121 passed 零回归。**US-005 已落地（2026-09-21，二期收官）**：DELETE `/api/machine/solve/{task_id}` 幂等清理（在飞先树杀 killed:true → rmtree run_dir + 清 marker + 弹状态槽 + 删 machine_cfg → 200；**二次 DELETE 也 200**，内存墓碑区分已清理/从未存在 404，MS 重启丢墓碑回落 404）+ X-Machine-Token 认证（env `MS_MACHINE_TOKEN` 设置时五端点强制、`secrets.compare_digest` 常量时间、缺失/错 401 先于一切业务校验，未设置放行 = loopback 同机假设）+ machine_* run_dir mtime>7 天机会式清理（每次 start 触发、非 machine 前缀不动、在册 run_dir 保护集）+ **契约文档落册**：agent-api-reference.md 新增「机器对接 /api/machine/* — YL 后端对接契约」自洽专节（五端点字段表/错误码汇总/状态机/run_mode 三档映射/config 键表（per_type 全 0 = 不发语义全等）/密度物理口径警示/坐标系 Y 向上 SVG scale(1,-1)/对接推荐流程 —— **YL 侧开发者可仅凭该节对接**）；37 例测试 + 真实服务器冒烟（token 401×5、DELETE 在飞树杀、二次幂等、7 天陈年清理、非 machine 前缀不动，:8015 + MS_MACHINE_TOKEN 14 检查 ALL PASS）全过，全套件 1128 passed 零回归。**三期 prd-machine-state-file 已落地（2026-09-22）**：`GET /api/machine/solve/{task_id}/state-file` .msn 状态文件下载（第六端点）—— 机器任务工作台态服务端权威装配为与浏览器「保存状态文件」**同构可恢复**的 gzip JSON 附件（doc 三源链会话无关 + form/quantities 状态槽→machine_cfg 快照合成 + run 最优布局 provenance 三档映射 + 守恒闸/超限防御 409 + 纯配置档省 run 键；statefile.py 导出符号纯复用零改动），用户动线 = YL 下载 → 交付版师 → MS 工作台「状态恢复」继续人工调整（编辑/微调/改数量重解/导出 PNG·DXF·PLT）；US-001 端点（tests/test_web_machine 46 例 + 全套件 1137 passed 零回归 + :8016 真实冒烟含 /api/state-restore 真恢复对拍）+ US-002 契约文档收官（agent-api-reference 机器专节 §6.5 自洽小节：请求/响应契约、错误码触发条件表、.msn 五块来源、纯配置档/best-so-far 语义、密度口径警示、YL 透传指引（不透明字节流/透传 CT·CD/克隆 PLT 下载通道/代理超时 ≥30s/用户后续路径一条链）+ curl 冒烟序列 + §8 错误码表/§10 推荐流程同步 + README 机器对接节）。配套 YL 侧规格 = YL 仓 `tasks/prd-machine-state-file-yl.md`（纯透传：按钮 + 代理 + token 服务端持有） |
