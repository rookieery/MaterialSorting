...
| 起始端成套前后幅（prefix） | 🚧 US-001 核心构造落地 | 建议1 v2（PRD `tasks/prd-prefix-head-set.md`，五项决策 2026-08-25 入档）：用户指认前/后幅 g 码 → 资格码（该码前、后 demand 均 ==2，版师 P2）seeded 随机取一（不出 UI）→ 4 片同码构造性竖排贴靠成 `PS_*` 组合片自由进主解（跨界填充，版师 P3）。**US-001 已落地**（`nesting_engine/prefix.py`，探针 `out/tmp_probe/prefix_lib.py` 移植收敛）：interleave 交错序定稿（0 封闭腔 vs grouped 2）、rot180 负坐标记账单测锁死、展开契约复用 `expand_placements`（rot0/rot180 黄金用例+包络断言）、冒烟 `python -m materialsorting.nesting_engine.prefix` 对拍 P0（5336 size34：bbox 1155×1458 / fill 83.3% / 贴触 0.00mm / 封闭腔 0；资格码 {32,33,34,35,38}）。P0 探针密度代价 −0.14pt ≈ 0、组合片 4/4 seed 自然锚定布头。后续：US-002 段置换钉位守卫 → US-003 web 编排（exclude_pids/stage/带位记录）→ US-004 前端 UI → US-005 A/B 验收 |
...
- **nesting_engine**：sparrow 求解。`constraints`（v0.3 常量 + 位图腐蚀 + 校验）、`sparrow_baseline`（基线 + ★共享层）、`sparrow_experiments`（公差实验）、`labeling`（**g 码赋号单一真相源**，US-001 v2：assign_codes + 母版编号复用，无名称映射）、`waist_band`（US-009 腰头成带 v2 构造性链构造；2026-08-22 简化后 `fill<45%` 唯一守门人，禁 import web）、`prefix`（US-001 起始端成套前后幅核心构造：eligible_sizes/pick_prefix_size/build_prefix_plan → `PS_*` 组合片，BandChunk 同构直接喂 expand_placements；禁 import web/cli）。intermediate 由 `web/server._commit_to_nesting_sync` 生成（US-001 v2 label 先行 / US-024 5 层，schema v2）。
