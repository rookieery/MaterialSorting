...
| 腰头成带（waist_band） | ⚠️ 闸门 no-go | **US-009 核心模块落地**（`nesting_engine/waist_band.py`：带内聚排 → union → erode → 组合片 + 展开权威式）；**US-010 go/no-go 试点闸门实测 no-go**（2026-08-21，报告 `out/config_runs/_probes/band_gate_report.json`）：5336 P0 同配置 A/B 均值劣化 1.204pt > 1.0pt 线（seed 方差 ≳ 组间差，seed2 off 波动 90.04% 拖垮均值）+ NFP 吞吐劣化 35%（comb 实测 ~700 顶点）；fill 曲线 51.8~61.1% 无稳定平台。**已决策：纯腰 v1 不合入（US-011~014 搁置），直接转 US-015 v1.1 混填料路线**（唯一实测过 break-even 线 62.4~63.6% 的形态：混带 72.5%） |
