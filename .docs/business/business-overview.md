...
| warm-start 热启动一期（warmstart） | 🚧 US-004 落地 | **sparrow `optimize(..., initial_solution)` 真顺延通道**（se 延长段灌冠军解续跑，取代同 seed 全新重放；跨项目拆分：spyrrow-ms 私有 wheel 以规格文档交付，MS 侧只做消费与切换）。**US-001 已落地**（`nesting_engine/warmstart.py`，2026-09-19）：`build_initial_solution` 校验矩阵（形态/未知 pid/每 pid 条数==demand 完整解硬约束/mirror 拒绝）fail-fast 中文 ValueError + `warm_start_supported()` 能力探测（`+ms<N>` 且 N≥1 才 True——ms0 纯重建 wheel 不算，PyPI/异常恒 False 不抛）；**US-002 已落地**（`initial_solution` 求解链纯透传：solve_pieces→solve_with_callback_proc（Process args 第 7 位）→solve_worker 最终消费点三闸门 + json.dumps 只在消费点，缺省 None 逐字节不变）；**US-003 已落地**（`--se-warm {on,off}` 默认 on：se 延长轮读 `best_frame_s{冠军}.json` 边车灌入热启动，五类回退（off/unsupported/band_prefix_on/no_best_frame/invalid_best_frame）任一命中回退现状重放+warn 不炸轮 —— 当前 0.9.0+ms0 装载态默认即回退 unsupported；strategy.json 计划态 + result.json/run_stats 实际灌入态 additive 同键，race/legacy 零新增键）；**US-004 已落地**（`scripts/spyrrow_wheel.py` 双源切换助手：status/use-pypi/use-local 一条命令在 PyPI 0.9.0 ↔ 私有 wheel 间切换，pip 走 sys.executable 不假设 PATH，use-local 错误矩阵清晰报错绝不触 pip，rev 钉板 spyrrow_build.json 只读+漂移提示不自动改写（四字段真相源在 spyrrow-ms 侧）；手册 = .docs/technical/spyrrow私有wheel构建与升级手册.md；ms0 与 PyPI 行为全等故日常生产零影响，本仓零 Rust 构建产物）；US-005（集成验收，依赖 ms1 wheel 交付）后续故事。回归红线：不传 initial_solution 一切行为逐字节不变 |
