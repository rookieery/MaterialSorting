...
- **图层语义**（版师 2026-08-10 确认；5156 与 M1787 一致，权威映射见 `dxf_parser/collect.py:LAYER_MAPPING`）：`1`=毛版外轮廓（**提取目标**）、`14`=净版、`7`=**布纹线(LINE，每片一条，方向因裁片而异)**、`4`=**刀口(POINT，带 `#N` 标签作刀口序号)**、`8`=内部线(POLYLINE)、`3`=轮廓密点(POINT，**非刀口**，仅沿毛版边的辅助标注)、`2`=未定参考点。`13` 等其余 layer 不提取。
