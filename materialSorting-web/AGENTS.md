...
标志 —— 不引入第二标志位）。R 键分支已留（return 空实现），**接线归 US-006**
（resetItem 已在 US-002 落地）。零后端改动、零渲染公式改动（全复用 US-003 链路）。
