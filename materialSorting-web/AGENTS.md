...
- **band 草稿三元组 `{enabled, label, ack}`，confirm 是唯一回写路径**：PerTypeOverridesModal mount 从 props 读初值，确定写回 `onBandChange`（ControlPanel patch form.band_* 三字段）；取消/遮罩/ESC 连同草稿丢弃（与 per_type 同约定）。未勾选时 label 原样保留（collectBand 对 enabled=false 恒 null）。
