...
├── types/                 # US-002 ✅：ws.ts / piece.ts / v03.ts；上传预览 US-005 ✅ parsed.ts（US-004 响应契约；矩阵化重构 US-004 ✅ 加可选 ptype?/paired? 物理片数口径）；上传预览 US-011 ✅ qty.ts（PieceQuantity/PieceQuantityMap）；US-027 ✅ solvePhase.ts（SolvePhase 五态）
...
└── components/
    ...
    │   ├── QtyMatrix.tsx       # 矩阵化重构 US-002 ✅ 数量矩阵（US-003 ✅ 接入 PreviewPage）：行=label 并集[徽章+配对×2徽章(US-004)+名+compact 缩略图+填充 popover] × 列=doc.sizes 全码[列头 button setSize+active]+合计；格内 clampQty 编辑+Enter/Tab 跳格；0/.zero、缺片/—.missing、特例/.override 三态；sticky 表头/首列/底行+45vh 内滚；小计 US-004 起物理口径 Σdemand×(paired?2:1)+全 0 警示+重置 1
    │   └── __tests__/
    ...
    │       ├── QtyMatrix.test.tsx        # 矩阵化重构 US-002 ✅ + US-004 ✅ 38 项单测（行列结构 6 + 列头切码 2 + 缩略图 openZoom 1 + 格内编辑 6 + 0/特例高亮 5 + 行填充 popover 7 + 小计与总片数（缺 paired ×1 兼容）4 + US-004 物理口径/配对徽章 6 + 重置 1）
...
- **小计 Σdemand 口径（本 Story）**：行合计 = Σ该行各码 demand；每码小计 = Σ该码各 label demand（缺片格不计）；工具条总片数 = sizeSubtotals 之和；全 0 且有行 → 红色警示。**US-004 升级物理口径 `Σ demand × (paired?2:1)`**——只改求和处乘系数，三处小计同源。
...
- **响应契约字段名严格与 server.py `_build_parse_payload` 一致**：`{doc_id, filename, sizes:[{size, pieces:[{label, name, ptype?, paired?, polygon, internal_lines, notches, net_polygon, grain_line}]}]}`（矩阵化重构 US-004 起可选 ptype/paired，缺字段按非配对 ×1 计）。改任一字段需同步后端 server.py + types/parsed.ts + useParseDxf.test.tsx AC#2。
