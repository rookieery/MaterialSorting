...
├── App.tsx                # US-001 ✅ Tab 骨架：TabBar + 双 .page 容器（display:none 切换）+ Tooltip 单例；US-029 ✅ 加 TourOverlay 单例；**多会话 US-005 ✅ mount useEffect probeSession()（POST /api/session 探测，429/401 code → 弹窗）+ SessionExpiredModal 单例（TourOverlay 之后）**；**2026-08-31 ✅ 加 Toast 单例（全局轻提示，非阻断，z-index 1500；首个触发源 = 解析出 null 通用码）**；**会话过期自动恢复 US-003 ✅（2026-09-13）静态 `import './lib/sessionRecovery'`（模块加载即注册启动期恢复钩子 —— 静态 import 保证先于任何 effect 驱动的 apiFetch）+ `<SessionRecoveryNotice/>` 单例（SessionExpiredModal 与 Toast 之间）**；**会话过期自动恢复 US-004 ✅（2026-09-13）静态 `import './lib/sessionCheckpoint'`（装载接线自动 checkpoint 调度）+ 挂载 effect `probeSession().then(() => clearCheckpointAfterProbe())`（探测落定后启动清理：存活 DELETE / 401·blocked 不清；StrictMode 双跑 DELETE 幂等）**
├── style.css              # 由 vanilla 前身 1:1 迁入；US-001 加 .tabbar/.tab/.page/.hidden/.preview-empty；上传预览 US-006 加 .upload-panel/.drop-zone/.upload-btn/.upload-status；US-007 加 .piece-preview-svg；US-008 加 .preview-page/.preview-main/.size-tabs/.size-chip/.parsed-pieces-view/.piece-grid/.piece-card*；上传预览 US-012 加 .piece-qty-dialog 族/.switch 族；上传预览 US-013 加 .piece-zoom-overlay/.piece-zoom-modal/.piece-zoom-head/.piece-zoom-seq/.piece-zoom-meta/.piece-zoom-name/.piece-zoom-close/.piece-zoom-body；上传预览 US-014 改 .piece-card-name→.piece-card-qty(+.disabled)；US-015 加 .tab.disabled(+hover)；US-018 加 .per-type-*/.ptype-* 族（US-004 矩阵增补样式已随 2026-08-18 矩阵回退删除：.per-type-rowhead 回 48px / .ptype-col 回 80px，.per-type-cell/.per-type-missing/--per-type 弹层 modifier 移除）；策略 PRD US-005 加 .strategy-* 族（wrapper/btn/badge/overlay+modal z-index 1100 #26282e/btn-exec/btn-apply #2ea06c/stop-btn #8a3b3b/big-density 36px/budget bar/seed-chips 四态 done#6cc79a killed#e08a8a running#2ea06c pending#666/result-head/warning #d9a05b/run-dir）；**后续删除**：矩阵化重构 US-003 删 .size-tabs/.size-chip/.piece-qty-dialog 族/.switch 族；图形预览区拆除删 .parsed-pieces-*/.piece-grid/.piece-card 系列（保留 .piece-card-label 供 PieceZoomModal 头部 g 码徽章）；矩阵行头简化删 .qty-rowname/.qty-paired-badge/.qty-reset-btn 族；裁片编号化 US-003 删 .piece-zoom-seq/.piece-zoom-name（v2 头部改 .piece-zoom-qty）；腰头成带 US-013 加 .per-type-band 族（title 左边框 #2ea06c / check accent / select / thumb 80×80；**2026-08-22 简化**：preview/ack/fillers/odd 相关类已删，仅保留该基础族）；起始端成套 US-004 加 .per-type-prefix-note（dim 11px 说明文案常驻）/.per-type-prefix-warn（#2a1c1c 底 #ff8888 红字警示，front==back / 无 2+2 资格码两态）；**2026-08-25 前缀组合预览**：加 .per-type-band-thumb--prefix（96×120 高缩略）/.per-type-band-thumb-empty--prefix（loading 骨架），删 .per-type-band-thumb:disabled（单片缩略删除后唯一使用者消失）；**编辑排料 US-002 ✅ 2026-09-04 加 .edit-layout-* 族（overlay z-index 1250 = ptype-preview 1200 之上 band-zoom 1300 之下 / modal 92vw×92vh 暗色 / canvas-tools 左上竖排 / save 绿主按钮 / svg touch-action:none）；US-003 ✅ 同日加 .edit-metrics 族 + .edit-rotate-handle；US-004 ✅ 同日加 .edit-controls 族（.export-group 同款分隔）+ button.edit-controls-btn（蓝 #2c5d8f）+ --reset 变体 + .edit-confirm-* 族（overlay z-index 1350 全栈最上 / ok 红 #8a3b3b = strategy stop 同款）**；**编辑弹窗 UI 优化 ✅ 2026-09-05：± 缩放按钮删除（滚轮唯一缩放入口）、「重置视图」改名「全览」（只复位缩放/平移不还原草稿，与主面板「重置」= editStore.reset() 语义区分；testid edit-zoom-reset 不变）、形态 select 自 footer 移入 .edit-layout-canvas-tools（state 仍在 EditLayoutModal、经 onModeChange 受控；footer 单保存按钮 → justify-content:flex-end 恒右）、新增 .edit-guide（画布右下操作指南卡 = 保存按钮上方，.edit-metrics 同款 pointer-events:none 悬浮卡 + .edit-guide-row 左对齐自然换行行式）**；**edit-polish US-005 ✅ 2026-09-05 加 .edit-polish-opt（对比卡内 compact checkbox 行：inline-flex gap:5 · pointer-events:auto 卡内交互点 · input accent #3f8f5f 绿 · 11px #aab 次要字色）**；**编辑弹窗 UI 二轮迭代 ✅ 2026-09-05：.edit-layout-canvas-tools 竖排→横排（row + align-items:center + gap 8）；微调对比卡迁画布左下独立锚点（.edit-polish-card 自带 absolute bottom/left 10）、.edit-br-stack 删除、.edit-guide 锚点下放回自身（absolute bottom/right 10）—— 四角分布 = 左上工具区/右上指标面板/左下对比卡/右下指南卡**；**同日三轮 ✅ 删 .edit-polish-foot（对比卡口径脚注收进「智能微调」按钮 title 悬浮，卡内不再占可见行）**；**同日四轮 ✅ 形态 select「形态」标题文案删除（label 容器/testid/DOM containment 不变、.edit-layout-mode 原 label 文案的 gap/color/font-size 死亡样式随删）+ .edit-guide「形态」「保存」两行删（七行减五 —— 只留画布手势，形态即眼前 select 自明、保存是 footer 按钮非画布交互）**
├── vite-env.d.ts          # vite/client 类型
├── types/                 # US-002 ✅：ws.ts / piece.ts / v03.ts；策略 PRD US-005 ✅ strategy.ts（/api/strategy/* 四路由响应 TS 镜像：七态 Phase / Status 除 state 全可选 / Result manifest 嵌套键 / race·se summary 判别联合）；上传预览 US-005 ✅ parsed.ts（US-001 v2 响应契约；裁片编号化 US-003 ✅ 删 name/ptype/paired —— label 即 g 码全链路主键）；上传预览 US-011 ✅ qty.ts（PieceQuantity/PieceQuantityMap）；US-027 ✅ solvePhase.ts（SolvePhase 五态）；腰头成带 US-012 ✅ ws.ts 扩 BandConfig{enabled,label}（2026-08-22 简化后恰两键）+ StageMsg + StartPayload.band?: BandConfig|null + ServerMsg 联合加 StageMsg；起始端成套 US-004 ✅ PrefixConfig{enabled,front,back}（无 size 键——资格码后端选定，2026-09-02 起几何搜索确定性）+ StartPayload.prefix? + StageMsg 扩 stage:'prefix'（size/holes 专属键回显；**起始端成套补片 US-004 ✅ 2026-09-02 additive** extra_label?/extra_size?/residual_mm? —— 补片 g 码/尺码/gate−组合片高，null 与键缺席同判=兜底 4 片，旧后端安全）+ band.ts PrefixPreviewResponse 同日扩 extra?{label,size}|null（只两键，PS_ 永不出端点）/residual_mm?/gate_mm?/fallback?，n_members 4（兜底）或 5（补片）；**edit-keyboard US-001 ✅（2026-09-05）piece.ts PlacedItem 扩 additive 可选 `mirror?: boolean`**（局部 x 翻转 `world=R(rot)·diag(−1,1)·p+t`；**omit-when-false**：不带镜像不出现该键，消费方统一 `it.mirror === true` 判定，详见文末「edit-keyboard US-001 关键约定」）
├── lib/                   # US-002 ✅ ws.ts；US-003 ✅ geometry.ts；US-004 ✅ params.ts（策略 PRD US-005 ✅ 加 collectStartContext：handleStart 与策略 start 载荷同源构造器，sizes/gate_mm/seed/params/per_type/quantities 逐字段同一实现；腰头成带 US-012 ✅ FormState 加 band_enabled/band_label + collectBand 三态解析（^g\d+$ 镜像后端，2026-08-22 简化后恰返回 {enabled,label}）+ bandMemberCount（missing→1/显式 0/未选码过滤三态，后端 demand 口径对齐）+ StartContext.band；US-006 seek.ts（**已删 2026-09-12** 随回放功能移除）；US-007 ✅ download.ts；BAND_LABEL_RE export（弹窗共用）；起始端成套 US-004 ✅ FormState 加 prefix_enabled/prefix_front/prefix_back + collectPrefix 三态（关/开未选或 front==back 或非 ^g\d+$ → null；开且有效 → {enabled:true,front,back}）+ prefixEligibleSizes（2+2 资格码本地预检，与后端 _parse_prefix 同口径：missing→0 非 1、'null' 通用码跳过、sizes 过滤）+ defaultPrefixLabels（决策⑤默认预选 parse doc 每 label 最大码 polygon shoelace 面积前二；5336 = g02/g03）+ StartContext.prefix；**多会话 US-005 ✅ session.ts（sid get-or-create：localStorage 键 ms_sid，uuid4 hex 32 位（RFC4122 version/variant 位置位，Math.random 降级），模块缓存 + 非法落盘值重铸；clearPersistedSessionId 墓碑出口）+ api.ts（apiFetch 全站统一 HTTP 出口 = 唯一裸 fetch 点：注入 X-Session-Id + 会话先行门 ensureSession once-promise（防 mount 竞态 401）+ 401/429 code 拦截 → triggerSessionBlock；阻断期间抛 SessionBlockedError 请求不发出；mergeSessionHeaders 归一普通对象；markSessionProbedForTest/resetSessionForTest 测试钩子；**会话过期自动恢复 US-003 ✅ 2026-09-13：探测 401 三分支（钩子在 → 恢复 + 裸 fetch 重探 / 无 code → 放行 / 无钩子 → 落回老路径阻断）+ registerStartupSessionRecovery 钩子槽（模块级依赖倒位，api.ts 保持 store-free）+ triggerSessionBlock 不再清 sid**）**；**会话过期自动恢复 US-003 ✅ sessionRecovery.ts（启动期恢复编排唯一所在：peek 旧 sid → clear → 铸新 → 裸 fetch POST /api/state-recover {from_sid} → 200 applyRestorePayload + toast「工作状态已恢复」/ 404·网络错 → toast「上次会话已过期，已开启新会话」/ 429 → session_limit 阻断；sessionRecovering 轻加载态；模块加载尾 registerStartupSessionRecovery 自注册，详见文末「会话过期自动恢复 US-003 关键约定」）**；**会话过期自动恢复 US-004 ✅ sessionCheckpoint.ts（自动 checkpoint 调度唯一所在：form/qty 去抖 3s + runRegistry markRunDone 求解完成立即 + editStore.save 立即 + hidden flush + pagehide keepalive → apiFetch POST /api/state-checkpoint（buildSavePayload 不带 save_as / 无 doc 跳过 / 异常全吞不重试 / sendChain 串行合并）+ clearCheckpointAfterProbe 启动清理，详见文末「会话过期自动恢复 US-004 关键约定」）**；ws.ts 多会话 US-005 ✅ solveWsUrl 拼 ?sid=（浏览器 WS 不能自定义 Header）；**编辑排料 US-001 ✅（2026-09-04）editGeometry.ts / overlap.ts**（几何纯函数 + 重合计算器，详见文末「编辑排料 US-001 关键约定」）；**编辑排料 US-003 ✅（2026-09-04）overlap.ts 扩**：`precomputeEditPiecesFromItems`（PlacedItem[] 直入口，编辑池源 = working 非 lastFrame）+ `applyEditPlacement`（池内单片原地增量）+ **polygon-clipping 双产物互操作**（ESM dist 只导出 default / CJS 具名 —— namespace 导入 + default 优先回退；具名 import 在 vite build 下报「"intersection" is not exported」）；**编辑排料会话钉住 ✅（2026-09-04）editHold.ts**（refreshEditHold POST /api/edit-hold 失败静默 —— 401 拦截已触发全局阻断 fail-fast 优于白编）；**edit-polish US-003 ✅（2026-09-05）editPolish.ts**（智能微调接线库：parsePrefixMemberPids 组合片 pid→成员 pid 集合（**front 段是裸 label 无 @size**，`PS_{front}+{back}@{size}` 权威式，size 取 stats.size 补全，畸形段静默跳过）/ buildExclude best-effort（band→labels / final.prefix→pids / 皆无省略键）/ buildPolishPayload（placed+gate_mm+exclude + **US-005 compact 档：第三参缺省 false 省略键 additive，true 才发 compact:true**）/ postEditPolish（apiFetch POST /api/edit-polish，失败抛 Error 中文直显对比卡），详见文末「edit-polish US-003 关键约定」）；**edit-keyboard US-001 ✅（2026-09-05）mirror 第 4 参贯穿**：`geometry.ts pointsStr(poly,rot,tr,mirror=false)` / `editGeometry.ts transformPolygon` 同款（旋转前一行式 `x = mirror ? -p[0] : p[0]`，缺省 false 输出与旧实现逐字节一致 —— 零回归红线）+ `overlap.ts` EditPiece 扩必填 `mirror: boolean`、`precomputeEditPieces(FromItems)` 读 `it.mirror === true`、`applyEditPlacement(ep,rot,tr,mirror=false)` 增量覆写（调用方须显式传标志防镜像片变换帧丢镜像），详见文末「edit-keyboard US-001 关键约定」）；**edit-keyboard US-003 ✅（2026-09-05）editPolish.ts buildPolishPayload placed 项 mirror omit-when-false 透传**（`...(it.mirror === true ? { mirror: true } : {})` —— 恒发 mirror:false 会红掉 EditLayoutModal.polish 精确锁键集用例；PolishPayload.placed 类型加 mirror?: boolean）；**edit-drag-snap US-001 ✅（2026-09-06）editGeometry.ts 扩吸附几何算子**：`contactT(v,dir,a,b)` 顶点沿单位方向平移到线段的参数 t 解（叉积线性解；近平行/零长边/延长线命中 → null 不抛异常）+ `firstContactDistance(moved,obstacle,dir)` 两多边形沿方向首触距离（顶点×边两向枚举取最小正 t，解析零迭代）—— 贴附拖动松手贴合的解析地基（2026-09-17 起触发 = Alt+左键），US-002 snap.ts 消费（详见文末「edit-drag-snap US-001 关键约定」节）；**edit-drag-snap US-002 ✅（2026-09-06）snap.ts 松手吸附引擎纯函数**：`computeSnapCorrection(dragged, rawTr, sess, others, opts)` → `{tr, kind:'free'|'retreat'|'attract', partnerKey}` —— P0 retreat（落点违谓词时沿 lastSafeTr→落点 20mm 粗扫括段 + 二分 ≤14 轮 + 1nm 微退防窄障碍隧道穿越）+ P1 attract（末帧位移方向、零位移帧退化质心连线，firstContactDistance ≤10mm 吸到触点 −1nm，多邻居 t 最小平手 key 小）+ 谓词（computeOverlap 总面积 ≤ 起手基线 + 1e-9：存量重叠不恶化不误清）+ clamp 违反 / 内部异常 fail-open 回落 raw；同 pid 多副本按 EditPiece.key 逐一寻址绝不去重（详见文末「edit-drag-snap US-002 关键约定」节）；**edit-drag-snap US-003 ✅（2026-09-06）snap.ts 导出 COLLIDE_AREA_EPS_MM2**（1e-9 原值不新增，仅改 export）—— EditCanvas lastSafeTr 跟踪谓词同口径消费（两处判定必须同 eps，否则跟踪位可能落在引擎谓词的违例侧 → 引擎防御性 fail-open 白白退化吸附效果）（详见文末「edit-drag-snap US-003 关键约定」节）
├── store/                 # US-002 ✅ runRegistry.ts（腰头成带 US-012 ✅ RunRecord 加 stage: StageMsg|null 信息记录，不影响 phase/done；**会话过期自动恢复 US-004 ✅ 2026-09-13 加 done 观察者：subscribeRunDone/markRunDone（run 结束单一出口，done 置位+同步通知，观察者异常吞）—— useSolveRun.finish 与 applySyntheticRun 已接，未来新 run 来源必须走它**）；US-003 ✅ appStore.ts；US-001 ✅ uiStore.ts（US-015 ✅ 扩 nestingEnabled + setNestingEnabled + setTab guard；会话过期自动恢复 US-003 ✅ 2026-09-13 扩 sessionRecovering 轻加载旗 + setSessionRecovering）；上传预览 US-005 ✅ uploadStore.ts（US-012 扩 qtyDialog 已随矩阵化重构 US-003 删除；US-013 扩 zoom + open/close；US-021 扩 commitStatus/commitError/commitSummary + reset 同步清）；上传预览 US-011 ✅ qtyStore.ts（矩阵化重构 US-001 简化：perSize+baseValue 单模式 + setPiecePerSize/setRowAll/resetQuantities/hydrate + clampQty/getPieceDisplay 纯函数，hydrate 双入口已合并）；US-018 ✅ controlPanelStore.ts（modal + previewLabel 双显隐字段；US-003 起 previewPtype→previewLabel 键 = g 码；策略 PRD US-005 ✅ ControlPanelModalId 加 'strategy_run'；**编辑排料 US-002 ✅ 2026-09-04 加 'edit_layout' —— 有意偏离 ESC/遮罩关闭惯例（唯一关闭 = ✕/保存）**）；**编辑排料 US-001 ✅ 2026-09-04 editStore.ts（open 快照基线/save 原地保序写回/reset/invalidate/setWorkingItem + computeLayoutStats 单一真相源，详见文末「编辑排料 US-001/002 关键约定」）；**编辑排料 US-004 ✅ 同日扩：itemsEqual 导出（dirty 判定 ε=1e-9 单一真相源）+ 基线锚定口径（同 run 重复 open 不换锚 —— 基线恒 = 会话首次 open 的算法布局，重置「回初始布局」即指它；invalidate/换 run 唯一换锚时机；working 恒从当前 lastFrame 深拷贝承接已保存编辑；savedDirty 重算 = lastFrame 偏离基线）****；策略 PRD US-005 ✅ strategyStore.ts（phase 七态 + status/result/errorMessage/lastStart；start/stop/refresh/reset；refresh 唯一真相入口 + isStrategyState 守卫 + done/stopped 拉 result 每 run 恰一次；**代际号 guard**：start/reset bump，在飞 refresh 过期响应丢弃）；US-029 ✅ tourStore.ts（activeTour/stepIndex/seen + localStorage 持久化 + TOUR_VERSION 版本号强制重看）；**2026-08-31 ✅ toastStore.ts（全局轻提示队列：pushToast 追加（**同文案去重**）+ dismissToast ✕ 手动关 = **唯一出口，不自动消失**（同日修订：数据异常告知不能被错过；去重防重复触发叠条）；`__resetToastsForTest` 测试隔离）**；腰头成带 US-013 的 bandStore.ts 已随 2026-08-22 简化删除（band 状态只在 ControlPanel form，无跨页镜像）；**多会话 US-005 ✅ ptypeStore/strategyStore 的 apiFetch → 统一出口（fetch → apiFetch，其余逻辑零改动）**；**edit-polish US-003 ✅（2026-09-05）runRegistry.ts 扩 RunRecord `band: BandConfig|null`（useSolveRun.start 自 cfg 记录——WS 载荷本就带 band，此前只发不存）+ `prefix: FinalPrefixStats|null`（applyFinal 自 final 消息 additive prefix 段记录；策略合成 run 无 WS final 自然 null）—— exclude best-effort 两个数据源；editStore.ts 扩 `replaceWorking(items)`（条数+逐下标 id 全等守恒闸 → 深拷贝整体替换 working，false=形态异常按失败处理；**不 save 不 bump 不动 lastFrame** = 微调结果只是编辑草稿，一级撤销共用入口）**；**edit-keyboard US-002 ✅（2026-09-05）editStore.ts 五处显式字段 map（deepCopyItems/itemsEqual/applyToRun/setWorkingItem/computeLayoutStats）全补 mirror omit-when-false 透传 + 新增 `resetItem(index)` 片级重置（只写 working 草稿不写 run；baseline 在案+下标双侧在界+id 对齐三守卫，命中 = 基线项深拷贝、miss = false 草稿原样；US-006 R 键消费，详见文末「edit-keyboard US-002 关键约定」）**；**edit-keyboard US-005 ✅（2026-09-05）setWorkingItem patch 扩可选 `mirror?: boolean` 写入口**（键盘 O/I 翻转镜像落地 AGENTS.md US-002 节预告的「届时扩 patch」：显式 true 带键 / 显式 false 键消失（omit-when-false）/ 缺省保持现值 —— 指针拖动会话不传 = 会话内恒定既有口径零回归；详见文末「edit-keyboard US-005 关键约定」）**
...
└── components/
    ...
    ├── nests/             # US-003 ✅ NestSVG / NestCard / NestLabel；US-005 ✅ NestsGrid；US-006 ✅ NestSVG hover（seek 回放分支已随回放功能删除 2026-09-12，恒渲染 lastFrame）；**编辑排料 US-002 ✅（2026-09-04）pieceDom.ts（5 层节点构建 SVGNS/PieceEntry/createPieceEntry 自 NestSVG 机械提取共用 —— NestSVG 与 edit/EditCanvas 同构同观感地基）**；**edit-keyboard US-003 ✅（2026-09-05）NestSVG frame 渲染路径透传 mirror**（`it.mirror === true` → pointsStr/transformPt 第 4 参 x 取负，编辑保存写回 lastFrame 的镜像副本在主视图所见即所得；solver 原生帧永无此键 → 缺省 false 路径逐字节不变）
    ├── edit/              # **编辑排料 US-002 ✅（2026-09-04）**：EditLayoutModal.tsx（受控 Portal 订阅 controlPanelStore 'edit_layout' + mount 自 bestRun() open 快照 + 状态条 computeLayoutStats 单一真相源（Δ 基线同 ceil 口径）+ **禁 ESC/遮罩关闭**（唯一关闭 = 右上 ✕ / 右下保存）+ 空态）；EditCanvas.tsx（命令式画布：working 下标 → 每下标一份 5 层节点（出现序）/ 指针锚缩放（world→user 换算）+ 空白平移 + **「全览」复位按钮（2026-09-05：± 按钮删除、原「重置视图」改名，只复位缩放/平移）+ 形态 select 同日自 footer 移入左上工具区（onModeChange 受控）+ 右下 .edit-guide 操作指南卡** / 毛板 4 层隐藏可逆 / bg 跟随 viewBox + fab 世界锚定）；**编辑排料 US-003 ✅（2026-09-04）EditCanvas 扩指针三分发**（旋转手柄 `data-edit-role="rotate"` 绕质心 / 毛版 polygon 选中+提层+拖片 / 空白平移 <3px=取消选中）+ rAF 帧节流（pointerup 同步 flushFrame）+ clampPlacement 钳制 + overlap 池增量 applyEditPlacement + computeOverlap 三指标面板（红色交集高亮 / 深度 ≤10 琥珀 >10 红 / 角度 >45° 红 / 布尔交异常 bbox 降级估算不阻塞）+ 签名跳过渲染（详见文末「编辑排料 US-003 关键约定」）；EditLayoutModal 零改动即实时状态条（working 响应式）；**US-004 ✅ 同日：EditConfirmLayer.tsx（新）自定义暗色小确认层（纯受控 message/双回调，Portal body z-index 1350，无 ESC/遮罩关闭 —— 主面板重置 confirm 与弹窗 ✕ dirty 确认同组件复用）+ EditLayoutModal ✕ 接 dirty 二次确认（itemsEqual(working, lastFrame.placed_items) 判定，非 dirty 直关）**；**edit-polish US-003 ✅（2026-09-05）**：EditLayoutModal 扩智能微调 state 机（busy/report/error/prePolish，生命周期=modal state 关窗自然清零；成功 replaceWorking **不自动保存** + 守恒闸通过才落快照（失败不留孤儿撤销）+ catch 统一补「微调失败」前缀保卡内可读；撤销 = replaceWorking(快照)+清三态），经可选 `polish` prop 受控下发；EditCanvas 工具区（形态 select 之下）「智能微调」按钮（testid edit-polish-btn，busy 禁用+「微调中…」）+ 右下 `.edit-br-stack` 卡栈（锚点自 .edit-guide 上提，指南卡零位移）内对比卡（六指标前→后 + 口径脚注 + 撤销按钮 + **US-005「回收空隙缩短料长」checkbox（.edit-polish-opt，testid edit-polish-compact，默认不勾；撤销按钮与它 = 栈内仅有的 pointer-events:auto 交互点）**，整栈透点不挡画布拖动）；配套 `__tests__/EditLayoutModal.polish.test` 13 + `lib/__tests__/editPolish.test` 13（详见文末「edit-polish US-003 关键约定」）；**编辑弹窗 UI 二轮迭代 ✅ 2026-09-05：左上工具区竖排改横排（全览/形态/智能微调一行）；对比卡自右下 .edit-br-stack 卡栈迁画布左下独立锚点（卡栈删除、.edit-guide 锚点下放回自身右下、与指南卡水平对称 —— 原右下双卡叠置挡排料尾部主视图），上文 US-003 fragment 的「右下卡栈」描述即日失效；testid/DOM containment 不变，单测与冒烟零改动**；**同日三轮 ✅ 对比卡口径脚注自卡内可见行移除（用户定案太占空间）—— 解释锚点收进「智能微调」按钮 title 悬浮（卡体整体 pointer-events:none，native title 在其上不触发、必须放可悬停的按钮上），.edit-polish-foot 删除，单测/冒烟改锁按钮 title 文案**；**同日四轮 ✅ 形态 select「形态」标题文案删除（select 裸留）+ 操作指南删「形态」「保存」两行（七减五，只留画布手势），单测反向锁两行不回潮 + label 无文本节点**；**edit-keyboard US-003 ✅（2026-09-05）mirror 渲染贯穿**：EditCanvas applyPlacement/transformPt/placementSig/clampPlacement 全链 mirror 参（working 项 `it.mirror === true` 判定 → 5 层 points/端点 x 取负；签名加 mirror 段防 replaceWorking 翻转镜像漏重写；钳制按镜像 bbox），拖动/旋转会话起手快照 mirror0（setWorkingItem 不改 mirror ⇒ 会话内恒定）帧内 commitDragPlacement 透传 DOM 5 层 + applyEditPlacement 池增量（覆写语义必须显式传标志，缺省覆回 false → 重合指标按错几何算）；EditLayoutModal prePolish 快照显式字段 map 补 mirror（撤销微调不丢镜像标志）；配套 `__tests__/EditCanvas.test` 镜像 describe 5 项 + `EditLayoutModal.polish.test` 快照含 mirror 1 项（详见文末「edit-keyboard US-003 关键约定」）；**edit-keyboard US-005 ✅（2026-09-05）键盘变换**：EditCanvas 指针交互同一 effect 兼挂 window keydown（守卫链：`interactionEnabled=false` 总闸 → 表单控件聚焦 INPUT/SELECT/TEXTAREA/BUTTON/contentEditable → 无选中片，任一命中零变换；单字符键统一小写，Shift+L 的 'L' 同键命中）—— 六键 L=+1°/K=−1°/Shift±10°/空格=180°（preventDefault 防 body 滚动）/O=水平镜像 toggle/I=垂直镜像 toggle+rot+180（diag(1,−1)=R(180°)·diag(−1,1) 复合律共用单标志），L/K 放行 e.repeat 连发、幂等键忽略 repeat，R 分支留 US-006；`applyKeyTransform` 质心锚定（t' = c_world − R(rot')·M(m')·c_local，片原地变换不漂移）+ clampPlacement 同拖动口径 + commitDragPlacement 可选 mirrorPatch（缺省不改 mirror 零回归）；新 prop `interactionEnabled`（EditLayoutModal 传 !confirmDiscard 联动 ✕ 确认层、缺省 true、`interactionRef` 渲染期回写监听器不重挂）；指南卡新增键盘行（「形态」「保存」反向锁不破）；配套 `__tests__/EditCanvas.test` 键盘 describe 10 项 + editStore patch.mirror 1 项（详见文末「edit-keyboard US-005 关键约定」）；**edit-keyboard US-006 ✅（2026-09-05）R 键片级重置**：R 分支接线（移至幂等键 repeat 闸之后）—— `editStore.resetItem(selRef 下标)`（同 pid 多副本按下标寻址绝不按 pid；返回 false = store 守卫命中 → 静默不动）+ 成功后重读草稿项按基线值走 `refreshPieceView` 单点同帧刷新（自 commitDragPlacement 拆出的「DOM 5 层 + 池增量 + 指标/手柄」视图段 —— store 写入已由 resetItem 承担不重复 setWorkingItem；commitDragPlacement = setWorkingItem + refreshPieceView 重构，指针路径行为不变）；配套 `__tests__/EditCanvas.test` R describe 5 项（详见文末「edit-keyboard US-006 关键约定」）；**edit-keyboard US-007 ✅（2026-09-05）冒烟扩展与文档同步**：smoke_edit_polish.mjs 新增 S8 键盘/镜像段（29→**46** 检查 —— O 镜像→保存→导出 placed mirror:true + PLT/DXF 正文几何镜像坐标对拍 + 微调 mirror 逐位透传 + R 重置回基线，既有 29 项复跑全绿 = mirror=false 零回归门；详见文末「编辑键盘/镜像关键约定」节）；**edit-drag-snap US-003 ✅（2026-09-06；2026-09-17 触发手势迁 Alt+左键 —— 原右键与 Edge 内置鼠标手势冲突（浏览器层拦截右键拖动、网页无法 JS 压制、右键左拖触发「后退」丢编辑态），用户定案整体切换，纯左键零回归红线不动、右键全域失效）**：EditCanvas onPointerDown 按键门控（仅左键 `button===0` 进门，毛版 polygon 分支内 `e.altKey` 分流 → Alt+左键 MoveDrag 带 `snap:true` 贴附会话 + snapSessRef 起手、纯左键既有拖片/平移/转柄路径零改动；其余按键（右键/中键/后退/前进）不起任何会话）+ svg `contextmenu` preventDefault（迁出后维持吞除，防误弹「后退」类菜单）+ pointerup `applySnapOnRelease`（再 clamp → snap.ts computeSnapCorrection → kind≠free 经 commitDragPlacement **唯一落笔出口**写入，只改 translation、rot/mirror 原值透传 + flashPartner 伙伴高亮）+ lastSafeTr 由 refreshMetrics 既有 computeOverlap 结果顺带续写（会话首帧落起手基线；拖动帧零新增计算、rAF 合帧管线不变；degraded 帧保守跳过）+ 伙伴片高亮 = ensureUiLayers 第三子层 partnerG（绿色虚线 #2ea06c 与旋转手柄同主题辅助色、pointer-events:none、900ms 常驻 + 300ms 淡出后移除，deselect/骨架重建/卸载取消计时）+ 指南卡贴附行（2026-09-17 起文案「贴附：Alt+左键拖动松手贴附」）；配套 `__tests__/EditCanvas.test` 贴附 describe 12 项（attract/retreat/free/两帧锚点/纯左键回归红线/rot+mirror 透传/非左键门控（含右键全域失效）/contextmenu 吞除/高亮消退/Alt+左键选中/指南文案，1022→1072 全量）（详见文末「edit-drag-snap US-003 关键约定」节）
    ├── ControlPanel/      # US-004 ✅ 8 子组件；US-005 ✅ MultiSeedControls（**2026-08-22 seed UI 隐藏：组件删除、ControlPanel 不再渲染；FormState.seed/multi_seed/seed_count 保留恒默认 → 载荷恒 seed=0/seed_count=1，底层多 run 能力不动**）；US-007 ✅ ExportButtons；US-018 ✅ PerTypeOverrides 改按钮触发 + PerTypeOverridesModal（高级配置弹窗；**2026-08-18 回退 US-004 矩阵化：列=g 码 × 2 行 d/tol，per_type 单级 {g 码:{d,tol}}**；**2026-08-22 标题改「高级配置：设置算法参数」+ 裁片表格上方加「裁片设置」分区标题（.per-type-band-title 同款）**）+ PtypePreviewModal（片型放大预览）；US-031 ✅ 加 data-tour 锚点（doc-banner / start-btn 父容器 / param-form 在 SizePicker / export-group 在 ExportButtons）；策略 PRD US-005 ✅ StrategyRunButton（高级运行入口 + 运行中徽标 + 单例弹窗）+ StrategyRunModal（三态：配置四档时长+双模式 / 进度五件套+seed chips+终止 / 结果最优+用布+模式汇总+run_dir 复制+应用按钮（策略 PRD US-006 ✅ 已接线 —— NestingPage 经 ControlPanel 透传 onApplyStrategy；未传回调（测试挂载）才 disabled）+error 重试+orphan 清理；ESC/遮罩/✕ 关闭均不 stop；__tests__/StrategyRunModal 13 项（三态+五件套+SE 延长+载荷同源+关闭不终止）+ StrategyRunButton 4 项）；腰头成带 US-013 ✅ PerTypeOverridesModal「布局设置」分区（band {enabled,label} 二元组草稿+下拉/缩略图）+ ControlPanel band 启动闸门（未选编号/数量全 0 置灰+StatusLine 文案）+ band×策略互斥（StrategyRunButton disabled+title；**2026-08-22 简化**：预演/ack/填料/bandStore 镜像已删，BandFormValue 恰 {enabled,label}）；起始端成套 US-004 ✅ PerTypeOverridesModal「布局设置」第二行「起始端成套前后幅」（PrefixFormValue{enabled,front,back} 草稿+confirm 同 band 通道；勾上且两码均空默认预选 defaultPrefixLabels 面积最大两片；前/后幅下拉+**组合形态预览缩略**（2026-08-25：POST /api/prefix-preview 三态 + prefix-zoom 放大层，BandPreviewSVG 复用（member.tag 覆盖标注 = 前/后幅 g 码），替换两张单片 80×80 缩略）；说明文案「满足 2+2 的尺码将自动选取」；front==back / 无资格码两态警示——prefixEligibleSizes 本地预检不阻塞确定，权威拦截在后端）+ ControlPanel prefix 启动闸门（未选前/后幅 / front==back → #start 置灰+StatusLine 文案+handleStart 兜底；无资格码**不置灰**）+ prefix×策略互斥（strategy-btn disabled+title；与 band 本身可同开）；**起始端成套补片 US-004 ✅ 2026-09-02**——prefix-zoom 放大层 hint 双形态：preview.extra 在案追加「＋ 顶部 {label}@{size} 异码片 · 余 {residual_mm}mm 近满幅」（residual 缺席显 —），无键/兜底=现行 hint；5 片缩略自动渲染（BandPreviewSVG 泛型零改动，异码片 size_color(B) 同码同色跨片型；PerTypeOverridesModal.test.tsx +2）；**编辑排料 US-002 ✅ 2026-09-04 ControlPanel 挂 `<EditLayoutModal />` 单例（ExportInfoModal 旁）**；**编辑排料 US-004 ✅ 2026-09-04 EditLayoutControls.tsx（新）=「编辑排料」标题区块（.edit-controls，StatusLine 与 ExportButtons 之间）：编辑/重置两按钮、激活口径与导出一致（some(lastFrame) && !running，含 stopped best-so-far 与策略/极限合成 record）、三层 disabled 防御（native → onClick guard → store 自守卫）、重置走 EditConfirmLayer 确认后 editStore.reset() 回算法基线**
    ├── curve/             # 【已删 2026-09-12】US-005 ConvergenceCurve 随收敛曲线功能整体移除
    ├── playback/          # 【已删 2026-09-12】US-006 PlaybackBar/Seekbar/SeekReadout 随回放功能整体移除
    ├── SessionExpiredModal.tsx  # 多会话 US-005 ✅ 阻断式全屏模态（useSyncExternalStore 订阅 lib/api 阻断态；不可点遮罩/ESC/✕ 关闭，唯一出口 = 「刷新页面」按钮 location.reload()；COPY 双码文案与后端 PRD 逐字一致；z-index 3000 盖过 tour 2000；**会话过期自动恢复 US-003 ✅ 2026-09-13 session_expired 文案改版 →「会话已过期，刷新页面后将恢复工作状态」**（停留期不清 sid，刷新 = 启动期恢复入口；session_limit 文案不变））
    ├── SessionRecoveryNotice.tsx  # 会话过期自动恢复 US-003 ✅（2026-09-13）恢复期轻提示（idle → null 零 DOM；sessionRecovering=true → role=status aria-live=polite 顶部居中「正在恢复工作状态…」绿左边框卡，pointer-events:none 不挡交互；订阅 uiStore.sessionRecovering）
...
- **双页面常驻 DOM，display:none 切换**：`.page.hidden { display: none }`（不是条件渲染）。切回排料页时 NestingPage 内 useState/useRef/runRegistry 全部保真，进行中求解 / WS 不中断。改策略需同步 6 项 App.test.tsx。
- **uiStore 双字段（US-015 扩，US-016 联动）**：`activeTab: 'nesting' | 'preview'`（默认 `'preview'`）+ `nestingEnabled: boolean`（默认 `false`，**US-016 由 PreviewPage subscribe uploadStore 联动 setNestingEnabled**）。求解/WS 等业务状态由 NestingPage 自治，不混入 uiStore。**关键不变量**：`setTab('nesting')` 在 `nestingEnabled===false` 时静默不切（见 US-015 关键约定）；setNestingEnabled(b) 不强制切 Tab（见 US-016 关键约定）。
...
## US-006 关键约定（Tooltip 调用方必读）

> **2026-09-12 回放功能移除**：appStore.seekTime / lib/seek.ts（maxElapsed/frameAtTime）/ playback/ 三组件（PlaybackBar/Seekbar/SeekReadout）/ NestSVG seek 分支 / constants/colors.ts 的 PHASE_COLORS·SEED_COLORS 已整体删除 —— NestSVG 恒渲染 `run.lastFrame`（live 口径），`.bottom` 底部区（250px）竖向空间全归排料卡。本节仅存 hover 相关约定。
...
- **面积换算 mm² → cm² 用 `÷100`**：`parseFloat(dataset.area)/100` 与旧 vanilla 实现 一致；`.toFixed(1)`。改单位 / 精度需同步 `Tooltip.test.tsx` / `NestSVG.test.tsx` 的相关断言。

## US-005 关键约定（多 seed 调用方必读）

> **2026-09-12 收敛曲线移除**：components/curve/（ConvergenceCurve + sampleFrames/renderCurveInto）已整体删除（PHASE_COLORS/SEED_COLORS 配色随删）。本节仅存多 seed run 编排约定。
...
- **useRafThrottle(seeds.length>0) 不在 solving=false 时停**：求解结束后 NestLabel 仍需 bump 重绘最终态；下次 start() 才会 runRegistry.clear + setSeeds([]) 间接停掉。
...
- **5 步序列：doc-banner / params / solve / result / export**（与 PRD 字面一致）。前 3 步告知型（无 ready，点下一步直接推进）；后 2 步联动型（ready 读 runRegistry 帧快照，ready()===false 切等待态 + 200ms 轮询，true 时自动推进；最后一步 export 自动完成 = markSeen('nesting') + close）。收敛曲线/回放已删（2026-09-12）—— result 步气泡文案只描述排料卡片实时刷新。
...
- **测试基线**：`src/components/__tests__/NestingPage.test.tsx` 5 项（registry 恰 1 条字段齐全 / phase done+状态行 / ExportButtons 解禁+多副本渲染 / 导出载荷=合成帧 / 不点 apply registry 不变）。改 applyStrategyResult 或 RunRecord 字段需同步该文件。
...
- **唯一裸 fetch 出口 = `lib/api.ts` 的 `apiFetch`**：全站 HTTP 请求（useParseDxf / useCommitToNesting / useExport / ptypeStore / strategyStore×3 / PerTypeOverridesModal×2）一律走它。新增网络请求**必须** apiFetch（结构性带 `X-Session-Id`，不落 default 会话）；裸 fetch 会绕过会话先行门与阻断拦截。**会话过期自动恢复 US-003（2026-09-13）起恰两处特权裸 fetch**：api.ts ensureSession 探测 401 后的重探 + lib/sessionRecovery.ts 的 POST /api/state-recover（均恢复链专用、手工拼 sid 头；业务请求仍零裸 fetch —— `grep 'fetch('` src/ 应仅命中 lib/api.ts 与 lib/sessionRecovery.ts）。
...
- **~~session_expired 弃 sid~~（会话过期自动恢复 US-003 起已废止）/ session_limit 保 sid**：US-005 原实现 triggerSessionBlock('session_expired') 顺手 clearPersistedSessionId()（刷新铸造新 sid 获干净会话但**丢工作状态**）；**US-003 起停留期过期不再清 sid** —— 旧 sid 留作刷新后启动期恢复的 from_sid（checkpoint 恢复锚点），SessionExpiredModal 文案改「会话已过期，刷新页面后将恢复工作状态」；clearPersistedSessionId 唯一调用方收敛到 lib/sessionRecovery.ts 启动期恢复链（peek→clear→铸新时序红线内）。session_limit 保 sid 语义不变（会话仍有效，稍后重试原会话续用）。
...
- **save/reset 防御**：写回前校验 `runRegistry.list().includes(run)`（重解/策略应用 clear 后旧引用拒绝）+ `lastFrame`/`manifest` 非空；`viewBoxMaxW = 新料长` 双向伸缩（NestSVG `W=max(viewBoxMaxW, f.width_mm)` 随之收缩）；**density_sparrow 恒不动**（solver erode 参考值）。
...
- **视图变更必须回写 vbRef**：滚轮/平移/「全览」复位全以 EditCanvas `vbRef` 为当前视图 SSOT —— `zoomBy` 漏回写 ref 则第二档起锚在陈旧视图（本项目已修）。`writeViewBox` 显示值 r6 截断（CTM 反变换 float 噪声不进 attr；数学连续性走 ref 全精度）。滚轮用 **native listener + passive:false**（React 合成 wheel passive，preventDefault 告警失效）。
- **事件分层**：毛版 polygon pointerdown 归 US-003 拖动（当前 no-op 预留，`closest('polygon')` 早退不起平移）；4 层工艺节点 pointer-events:none；空白（svg/bg/fab）pointerdown = 平移（比尺 = min(宽比,高比)，letterbox 偏移在差分中抵消，无需 CTM）。**视图工具（2026-09-05 起）= 左上「全览」按钮 + 形态 select（± 缩放按钮已删、select 自 footer 移入，onModeChange 受控、state 属 EditLayoutModal）；右上 US-003 指标面板、右下 .edit-guide 操作指南卡 —— 两卡均 pointer-events:none 不挡画布命中。**
...
## 编辑排料 US-003 关键约定（拖动/旋转/重合指标 调用方必读）

交互层落地（2026-09-04）：EditCanvas 指针三分发 + rAF 拖动帧 + 三指标面板；EditLayoutModal 零改动（状态条本就 working 响应式）。零后端改动。

- **指针三分发**（单一 effect，状态全走 refs + `useEditStore.getState()` 防陈旧闭包）：pointerdown target 带 `data-edit-role="rotate"` → 旋转拖柄（绕选中片**质心**，角度必须经 clientToWorld CTM，无 CTM 防御 no-op）；`closest('polygon')` → selectPiece（选中 + 懒建 UI 覆盖层 + raiseEntry 提层到覆盖层之下）+ 平移拖片；空白 → 平移（<3px down-up = 取消选中）。
- **拖动帧 rAF 节流**：pointermove 只记最新坐标 + scheduleFrame（rAF 缺失同步回退）；帧内 = clampPlacement → 只 setAttribute 被拖片 5 层 → `setWorkingItem`（单下标；状态条经 zustand 响应式同帧刷新，**非每 pointermove**）→ 池内 applyEditPlacement → refreshMetrics + updateHandle；pointerup 先同步 flushFrame（尾帧不丢 = jsdom 测试确定性路径）。
- **位移/角度两条坐标通路不可互换**：平移增量 = 屏幕位移 / viewScale（meet min 比，letterbox 在差分中抵消，无需 CTM）；旋转角度 = clientToWorld 方位角差（letterbox 对 dx 抵消但**角度不抵消**）+ pivot 公式 `t' = C + R(dθ)(t−C)`（translation 随动、质心不动；wrapDeg180 归一）。
- **钳制口径**：Y ∈ [0, gate] 按**被拖片 bbox**（minY<0 上抬 / maxY>gate 下压优先，超高片贴底锚）；x<0 钳 0；**右界永不钳**（料长双向伸缩是编辑特性）。gate = manifest.gate_mm。
- **旋转自由角**：无 0/90/180 吸附（2026-09-04 决策）；偏离角 `min(|r|,|r−180|,|r−360|)` 只如实显示（>MAX_ROTATION_TOL_DEG=45 红）不拦截不报警。手柄 = 质心 + (0, 3r)（r=clamp(vb.w×0.015, 4, 30)），UI 覆盖层（overlapG pointer-events:none + handleG）**懒建**——首次选中才建（无选中 = 无覆盖层，US-002 childNodes 快照依赖此形态）。
- **重合指标**：refreshMetrics = computeOverlap（manifest erode 几何 = solver 碰撞口径，面板脚注注明）→ 交集环红色半透明高亮 + 三值（面积 mm² ÷100 cm² / 穿透 >MAX_OVERLAP_MM=10 红 >0 琥珀 / 旋转偏离 >45° 红）；**布尔交异常降级**：catch → bbox 交估算（矩形环高亮 + 「· bbox 估算」标注 + data-degraded），穿透深度独立纯函数照常，拖动照常 —— 纯函数 overlap.ts 本身不吞错上抛。
- **提层置顶后文档序 ≠ working 序**：raiseEntry 把被拖片 5 层移到全部裁片之上 → `querySelectorAll` 毛版数组下标与 working 下标错位；跨渲染寻址用 **points 字符串**当片身份（浏览器验收实测教训）。
- **渲染签名跳过**：`mode|rot|tx|ty` 逐片签名相同则零 setAttribute（拖动帧已即时应用的片在 working→effect 中跳过；reset 等全量变化自然全应用）。
- **测试套路**：拖动 = mockRect 定比尺 + firePointer 三连（down/move/up，jsdom PointerEvent polyfill）；旋转 = mockCTM 复合矩阵（editGeometry.test mockMat）；rAF 实时性 = `await act(async () => setTimeout 30ms)` 等帧落（jsdom rAF 真定时器可落）；降级 = vi.mock('polygon-clipping') 双形态（具名 + default 都给，mock 代理缺 default 会炸 default 探测）。

## 编辑排料 US-004 关键约定（保存/✕关闭/重置与主界面集成 + 导出闭环 调用方必读）

集成层落地（2026-09-04）：主面板「编辑排料」区块 + ✕ dirty 二次确认 + 重解/策略应用失效挂点 + 导出闭环验证。零后端改动；useExport / ExportInfoModal **零改动**（均读 bestRun().lastFrame，save 写回后 placed/density 自动继承）。

- **dirty 判定 = working vs lastFrame（非基线）**：`editStore.itemsEqual`（长度 + id + rot/translation ε=1e-9，与 computeLayoutStats 同 FLOAT_EPS —— 拖回原位 ~1e-13 残差不算 dirty）是唯一判定点。save 写回是精确拷贝 ⇒ 保存后必非 dirty（✕ 直接关不误弹）；open 快照深拷贝 ⇒ 未编辑恒非 dirty。改 dirty 语义须同步 EditLayoutModal ✕ 三态测试（未编辑直关 / dirty 确认 / 保存后直关）。
- **基线锚定不换锚**：同一 run 重复 open 基线**保持**（恒 = 本会话首次 open 的算法布局快照；invalidate（handleStart / applyStrategyResult 挂点）与换 run 是唯一换锚时机）。「重置回初始布局」的「初始」= 算法原始结果 —— 两轮 拖→存 后重置仍全量回滚到算法布局（working 每次 open 从当前 lastFrame 深拷贝承接已保存编辑，Δ 显示对基线的累计效果）。
- **编辑区块激活口径 = 导出口径**：`runRegistry.list().some(r => r.lastFrame !== null) && phase !== 'running'`（stopped best-so-far 与策略/极限合成 record 天然命中）；**不得**引入 savedDirty 等额外条件。三层 disabled 防御同 TabBar：native disabled → onClick guard → store 自守卫（open 无 lastFrame / reset 无 baseline 或陈旧 run 均返回 false 幂等）。
- **确认层只有按钮一条路**：EditConfirmLayer（自定义暗色 confirm，不用 window.confirm）无 ESC / 遮罩点击关闭，Portal 到 body + z-index 1350 恒在全栈之上（edit-layout 1250 / band-zoom 1300）；重置文案 = PRD 原文「确认将当前更新后的排料布局重置回初始布局」，✕ dirty 文案 =「放弃未保存的修改？」。
...
## edit-polish US-003 关键约定（智能微调：按钮/对比卡/一级撤销 调用方必读）

「智能微调」= 编辑弹窗把当前 working placements 发 POST /api/edit-polish（US-002 端点），
引擎侧诊断+去旋转+去重叠（守恒：料长不增/密度不降，物理毛版口径），前端只承接结果。
改以下任何一条须先改 `components/edit/__tests__/EditLayoutModal.polish.test.tsx`（13 项）与
`lib/__tests__/editPolish.test.ts`（13 项）：

- **微调不自动保存**：成功只 `replaceWorking` 写 working 草稿（不 save/bump/动 lastFrame）——
  ✕ 关窗即弃、保存才写回（dirty 判定 itemsEqual 自然覆盖微调改动）；任何「顺手 save」违约。
- **pid 逐位守恒闸**：replaceWorking 条数 + 逐下标 id 全等才替换；响应 placed 错位 =
  形态异常按失败处理（错误文案进卡 working 不变），不得部分应用。快照（prePolish）只在
  闸通过、working 已替换后才落 —— 失败路径不得出现孤儿撤销按钮。
- **一级撤销语义**：快照 = 点击微调前的 working（含此前手动拖动中间态）；再次微调**覆盖**
  快照（撤销恒回最近一次微调前，非最初基线）；撤销/关窗清空（Inner 卸载自然清零）。
- **exclude 是 best-effort 不阻断**：run 带 band（enabled+label）→ exclude.labels；final 带
  prefix 统计段 → exclude.pids = parsePrefixMemberPids 成员集合（**组合片权威式
  `PS_{front}+{back}@{size}(+{extra}@{extra_size})`，front 段是裸 label 无 @size**，size 取
  stats.size 补全；畸形段静默跳过）；皆无（含策略合成 run）→ 载荷省略 exclude 键
...
- **US-005 compact 档 additive**：对比卡内「回收空隙缩短料长」checkbox（testid
  edit-polish-compact，.edit-polish-opt pointer-events:auto）默认不勾；勾选态属
  EditLayoutModal（polishCompact state）随下次微调请求发 `compact:true`（buildPolishPayload
  第三参），缺省省略键 = 服务端行为零变化；勾选态跨多次微调保持、撤销/关窗随卡清零
  （重开回默认不勾 —— 有意不复位跨会话）。
- **错误文案统一前缀**：postEditPolish 抛错已带「微调失败：」直显；其余（fetch 网络层
...
- **S7（US-005）** compact 压缩回收档：checkbox 默认不勾 → 勾选 → 再微调请求
  `compact:true` + placed 守恒 + width ≤ 非 compact 档（S6 结果）+ 本轮守恒不等式
  （width 不增/密度不降/重合对不增）+ 截图 `s7_compact.png`。
- **S8（edit-keyboard US-007，2026-09-05）** 键盘/镜像段（17 检查，详见文末
  「编辑键盘/镜像关键约定」）：S7 compact 结果保存 → 导出镜像前基线 DXF → 重开 →
  合成 pointerdown 选中一片（按 points 内容寻址）→ O 键镜像（DOM 恰改一片 +
  质心锚定反射对拍）→ 保存 → 导出 PLT/DXF：POST placed 恰一项 `mirror:true`
  （其余 29 项与镜像前逐位全等）+ **正文几何镜像坐标对拍**（镜像前 DXF layer1
  轮廓经 R(θ₁)·M·R(−θ₀) 复合 = 镜像后正文轮廓，PLT ≤2 HPGL unit / DXF ≤0.05mm，
  且非镜像反事实远离 ≥40unit/≥1mm = mirror 键真实驱动几何）→ 重开承接镜像片 →
  微调 mirror 逐位透传（请求/响应恰一项同下标同 pid）→ R 键重置该片 points
  逐字节回算法基线 + 截图 `s8_o_mirrored.png`/`s8_r_reset.png`。
...
## edit-keyboard US-001 关键约定（mirror 数据模型与纯函数层 调用方必读；2026-09-05）

prd-edit-keyboard-transform（键盘变换 + R 片级重置）地基故事：镜像（det=−1 反射，
`{id,rotation,translation}` 表达不了）进入数据模型与纯函数层。US-002~007 将沿
「store 贯穿 → 画布/主视图渲染 → 后端导出/微调引擎 → 键盘交互 → 冒烟」逐层接线，
接线前必读以下口径（违反任何一条 = 回归或契约破裂）：

- **mirror 语义 = 局部坐标系 x 翻转**：`world = R(rot)·diag(−1,1)·p + t`，展开
  `x'=−c·x−s·y+tx`、`y'=−s·x+c·y+ty`（c=cos(rot)、s=sin(rot)）。实现统一为
  「旋转前一行式取负」（`const x = mirror ? -p[0] : p[0]`）—— 恒等式
  `R·diag(−1,1) = 先 x 翻转再旋转`，与「base 顶点 x 预取负再走无镜像变换」逐点
  等价（单测用该恒等式做独立实现互证）。**垂直镜像不设第二标志**（US-005 落地：
  I 键 = toggle mirror + rot+180，`diag(1,−1)=R(180°)·diag(−1,1)` 复合律）。
- **omit-when-false（硬约束）**：`PlacedItem.mirror` 序列化只在 true 时带键 ——
  store 写回 / 微调载荷 / 导出 placed 一律「有镜像才带键」；恒发 `mirror:false`
  会红掉 `EditLayoutModal.polish.test` 的 `expect(body.placed).toEqual([...])`
  精确锁键集用例。solver 原生帧永无此键 → 缺省路径零回归。
- **消费判定 = `it.mirror === true`**：undefined/false 同义无镜像；不写 `it.mirror`
  真值短路以外的花式判定。
- **缺省 false 逐字节一致（零回归红线）**：`pointsStr`/`transformPolygon`/
  `applyEditPlacement` 的 mirror 参数缺省（含显式 false）时输出与改动前逐字节相同
  —— 既有 lib 单测零改动全绿是本约束的验证门。
- **`EditPiece.mirror` 是内部计算字段（必填 boolean，非可选）**：omit-when-false 只
  约束 wire/store 键集，不约束 overlap 计算池；源头 `precomputeEditPieces(FromItems)`
  读 `it.mirror === true` 初始化。`applyEditPlacement(ep, rot, tr, mirror=false)` 是
  **覆写语义**：镜像片走增量更新必须显式传当前标志，缺省参数会把 mirror 覆回 false
  （防丢镜像由调用方负责，US-003 拖动帧 / US-005 键盘帧接线点）。
- **公式三方锁步**：前端 `pointsStr`（r2 截断渲染出口）/ `transformPolygon`（全精度
  计算出口）/ 后端 `apply_transform`（US-004 落地）同输入逐点相等；改公式三方同步改。
- 本故事零组件/零后端改动（浏览器验证与冒烟归 US-003/US-005/US-007）；验证门 =
  vitest 全量 981 passed（966+15）+ `npm run build` 过。

## edit-keyboard US-002 关键约定（editStore mirror 贯穿与 resetItem；2026-09-05）

US-001 立数据模型后 store 层接线：**五处显式字段 map**（deepCopyItems / itemsEqual /
applyToRun / setWorkingItem / computeLayoutStats）此前逐字段重建对象，新增字段会被
静默丢弃 —— 本故事全部补 `mirror` 透传；另新增片级重置 action。调用方口径：

- **序列化 omit-when-false**（与 US-001 同款硬约束）：`deepCopyItems` /
  `applyToRun`（save/reset 落盘）/ `setWorkingItem` 重建对象一律
  `...(it.mirror === true ? { mirror: true } : {})` —— false/缺省不带键。
  applyToRun 对象整体重建 ⇒ 工作稿关镜像保存后 lastFrame 旧 mirror 键自然消失
  （不残留）。
- **setWorkingItem 是透传不是覆写**：patch 只含 rotation/translation，镜像片增量
  更新（拖动/键盘）不掉 mirror 标志 —— US-005 键盘 O/I 翻转 mirror 需另行写入口
  （届时扩 patch 或走 replaceWorking，本故事未扩）。
- **itemsEqual 按 `=== true` 归一布尔比较**：undefined 与显式 false 同义（键在与否
  不算差异），布尔差异 → dirty。open 的 savedDirty 重算自动继承该口径。
- **computeLayoutStats 传 mirror**：`transformPolygon(poly, rot, tr, it.mirror === true)`
  —— 镜像改变包络 maxX ⇒ 料长/密度统计对镜像片正确（三角形不对称夹具手算对拍）。
- **resetItem(index) 片级重置语义**：working[index] ← baseline.placedItems[index]
  深拷贝（恢复**算法基线** = 本会话首次 open 的算法原始布局，与全局 reset 同锚点 ⇒
  会抹掉该片已保存的编辑；mirror 随基线天然清零/回填）。**只写 working 草稿不写
  run**（保存才落盘）；不 bumpRenderTick（working 引用变化即重渲染，与
  setWorkingItem 同款）。守卫三态（任一命中 → false 且 working 原样）：①baseline
  缺席（未打开/invalidate 后）②下标越界（working 与 baseline **双侧**校验）③id
  错位（同 pid 多副本**绝不按 pid 寻址**，下标对齐防御）。
- 验证门 = vitest 全量 **994 passed**（981+13）+ `npm run build` 过；本故事零组件/
  零后端改动（浏览器验证归 US-003/US-005/US-007）。

## edit-keyboard US-003 关键约定（画布与主视图镜像渲染贯穿；2026-09-05）

US-001 纯函数层 + US-002 store 之后的**渲染/载荷接线**：镜像片所见即所得。改
EditCanvas 渲染链 / NestSVG frame 路径 / EditLayoutModal 微调 state 机 / editPolish
载荷前必读：

- **EditCanvas 全链 mirror 参**：主渲染 effect 读 working 项 `it.mirror === true` →
  `applyPlacement(entry, rot, tr, mode, mirror)`（5 层 pointsStr/transformPt 第 4 参
  x 取负）。applyPlacement/applyEntryPlacement/commitDragPlacement 的 mirror 形参是
  **必填**（内部链路，漏传编译红）；pointsStr/transformPolygon/transformPt/
  clampPlacement 家族保持**缺省 false**（外部零回归红线）。
- **placementSig 含 mirror 段**（`${mode}|${rot}|${tx}|${ty}|${m?1:0}`）：签名跳过是
  「同 rot/tr 即同放置」假设 —— mirror 翻转改变世界几何，replaceWorking 翻转镜像
  （同 rot/tr）必须重写 5 层；签名漏段 = 画布陈旧（单测双向翻转锁死）。
- **拖动/旋转会话快照 mirror0**：pointerdown 起 `mirror0: it.mirror === true`，
  会话内恒定（setWorkingItem patch 不改 mirror）。帧内三处透传：clampPlacement
  （钳制按**镜像 bbox** —— 镜像片 x 取负后 minX/maxX 与非镜像不同）、DOM 5 层、
  `applyEditPlacement(ep, rot, tr, mirror)` 池增量（US-001 覆写语义：缺省覆回
  false → 重合指标按错几何算；精确面积断言锁死）。US-005 键盘帧同款复用
  commitDragPlacement。
- **NestSVG frame 路径透传**：`const mirror = it.mirror === true` → 5 层同参。
  solver 原生帧永无 mirror 键 → 缺省路径**逐字节不变**（回归锁）；编辑保存写回
  lastFrame 的镜像副本在主视图所见即所得（浏览器对拍：恰该副本 points 变化且与
  页面内 pointsStr(mirror) 直算逐字节一致）。
- **prePolish 快照显式字段 map 补 mirror**（omit-when-false）：逐字段重建漏键 =
  撤销微调静默丢镜像标志；快照语义不变（覆盖/关窗清空）。
- **buildPolishPayload placed 项 mirror omit-when-false**：恒发 mirror:false 会红掉
  EditLayoutModal.polish 精确锁键集用例（`toEqual([{id,rotation,translation}])`
  零改动保持绿是验收项）。
- 验证门 = vitest 全量 **1005 passed**（994+11：EditCanvas 镜像 describe 5 /
  NestSVG mirror 帧 3 / editPolish mirror 2 / EditLayoutModal.polish 快照 1）+
  `npm run build` 过、static/ 已重建；浏览器 dev 模式 15/15（out/usk003_mirror_verify：
  基线逐字节回归 / store 翻转 → 画布镜像渲染 / 其余 29 片不动 / 镜像片拖动不炸 /
  保存 → lastFrame.mirror=true + 主视图恰一片镜像 / 重开保持 / 微调载荷恰一项
  mirror:true）。
...
改以下任何一条须先改 `components/edit/__tests__/EditCanvas.test.tsx`（键盘
describe 10 项）与 `store/__tests__/editStore.test.ts`（patch.mirror 1 项）：

- **keydown 挂指针交互同一 effect**：键盘落笔复用闭包内
  commitDragPlacement/refreshMetrics/updateHandle（监听器仍只挂一次
  `[]` deps）；**不得**为键盘另起 effect 重复实现落笔链。
- **守卫链顺序固定**：①`interactionRef.current` false（总闸）→ ②target ∈
  INPUT/SELECT/TEXTAREA/BUTTON 或 isContentEditable（键盘归表单控件 —— 含画布
  左上形态 select）→ ③无选中片；任一命中零变换。守卫②用 tagName 大写比较
  （SVG 元素 tagName 可为小写）。
- **interactionRef 渲染期回写**（modeRef 先例）：`interactionRef.current =
  interactionEnabled !== false` 在 render 体执行 —— keydown 监听器不重挂、
  确认层开关即时生效。EditLayoutModal 传 `interactionEnabled={!confirmDiscard}`；
  直挂 EditCanvas 的单测缺省 true 零改动兼容。指针交互**不受**该闸影响
  （确认层 overlay 已挡 pointer 热区）。
- **e.repeat 分界**：L/K 放行（auto-repeat 每击一档 = 按住连转）；幂等键
  （空格/O/I）在 repeat 检查**之后**分发前忽略（一次 keydown 一次变换）；
  空格必 preventDefault（防 body 滚动；聚焦按钮激活已被守卫②排除）。
- **单字符键统一小写**：`e.key.length === 1 ? e.key.toLowerCase() : e.key`
  —— Shift+L 的 e.key='L' 同键命中（CapsLock 布局同样）。
- **质心锚定是唯一变换出口**：applyKeyTransform —— `t' = c_world −
  R(rot')·M(m')·c_local`（c_local/c_world = base/当前世界多边形**顶点均值**；
  M=diag(−1,1) when mirror）→ clampPlacement（带 mirror 参）→
  commitDragPlacement 泛化路径（DOM 5 层 + setWorkingItem + 池增量 +
  指标/手柄，O(单片)）。不得绕开 store 直改 DOM。
...
- 验证门 = vitest 全量 **1016 passed**（1005+11）+ `npm run build` 过、
  static/ 已重建；浏览器 dev 模式 **26/26**（out/usk005_keyboard_verify/
  verify2.mjs + report2.json：真实 trusted 按键 L/K/Shift/空格/O/I 逐键
  DOM↔纯函数逐字节对拍 + 连发 20 次 78ms 不卡顿 + repeat 防抖 + 守卫链五条
  （含确认层取消后 L 恢复生效 = interactionRef 回写锁）+ 无 pageerror）。

## edit-keyboard US-006 关键约定（R 键片级重置；2026-09-05）

R = 片级重置交互入口（已定案 2026-09-05：R 键非右键菜单；重置语义 = 恢复
**算法基线**（会话首次 open 的算法原始布局，与全局 reset 同锚点）⇒ **R 会连上次
保存的编辑一起抹掉该片**（定案接受，安全网 = ✕ dirty 确认 + 全局重置 + 片级
重置）；本期不建撤销栈）。复用 US-002 的 `editStore.resetItem`，零后端、零渲染
公式改动。改以下任何一条须先改 `components/edit/__tests__/EditCanvas.test.tsx`
（R describe 5 项）：

- **R 分支位置在幂等键 repeat 闸之后**：R 与空格/O/I 同为幂等键 ——
  `if (e.repeat) return` 之后分发（一次 keydown 一次重置，按住不抖动）；守卫链
  与 US-005 六键同链（总闸 → 表单控件 → 无选中，任一命中无效）。
- **下标寻址绝不按 pid**：R 调 `resetItem(selRef 下标)` —— 同 pid 多副本按
  working 数组下标寻址；id 错位等 store 层守卫命中（返回 false）→ **静默不动
  不炸**（不得 fallback 按 pid 找同名项）。
- **store 写入唯一出口 = resetItem**：R 成功后重读 `working[index]` 按基线值走
  `refreshPieceView`（DOM 5 层 + 签名登记 + applyEditPlacement 池增量 +
  指标/手柄同帧刷新）—— **不重复 setWorkingItem**（resetItem 已写草稿）。
- **refreshPieceView 是视图段单一真相源**：自 commitDragPlacement 拆出的
  「不含 store 写入」视图段；commitDragPlacement = setWorkingItem +
  refreshPieceView（指针拖动/键盘 L-K-空格-O-I 路径行为不变，顺序调整无观察
  差异 —— 同步帧内 store 写与 DOM 写无依赖）。
- 验证门 = vitest 全量 **1021 passed**（1016+5）+ `npm run build` 过、
  static/ 已重建；浏览器 dev 模式 **26/26**（out/usk006_reset_verify/
  verify.mjs + report.json：L→保存→重开承接已存编辑→O→R 回**基线而非已存值**
  （「连已保存编辑一起抹掉」语义锁 + lastFrame 不动）+ 全 30 片 points 多重集回
  基线 + 指标同帧回基线 + 指针拖片+Shift+L+O 复合编辑后 R（镜像片复位）+
  repeat×3 忽略 + 守卫链（无选中 / ✕ dirty 确认层全键禁用 / 取消后恢复）+
  无 pageerror）。

## 编辑键盘/镜像关键约定（edit-keyboard US-007 汇总；2026-09-05）

六键 + mirror 全链的**跨故事汇总口径**（数据模型 US-001~003 / 键盘 US-005~006 /
...
- **六键语义（EditCanvas window keydown，选中片分发）**：L=顺时针 +1°、K=逆时针
  ...
  **O=水平镜像 = toggle
  ...
  （INPUT/SELECT/TEXTAREA/BUTTON/contentEditable）→ 无选中片；单字符键统一
  小写（Shift+L 的 'L' 同键命中）。
- **质心锚定唯一出口**：键盘变换一律 `applyKeyTransform(index, rot', mirror')`
  —— `t' = c_world − R(rot')·M(mirror')·c_local`（c_local/c_world = base/当前
  世界多边形**顶点均值**，M=diag(−1,1) when mirror；质心是仿射量 ⇒ 片原地变换
  不漂移），再 clampPlacement（Y∈[0,gate]/minX≥0，右界永不钳）。O 键锚的是
  **eroded 质心**（画布渲染 eroded 轮廓）；导出正文是**原始毛版轮廓** ——
  对拍时反射预测须用 eroded 世界质心当锚、几何源用 raw polygon（两口径并存是
  设计非 bug，锚定等价把差吸收进 translation）。
- **omit-when-false 线规**：`mirror === true` 才带键（undefined/false 同义无
  镜像、键不存在）—— 前端 PlacedItem / /api/edit-polish 请求与响应 placed /
  /export POST placed 三处同口径；**恒发 mirror:false 会红掉精确锁键集用例**
  （响应 placed 无改进时逐位返回输入对象，亦含该键）。改判定一律 `it.mirror === true`。
- **resetItem 下标守卫**：R 键调 `editStore.resetItem(selRef 下标)` —— 同 pid
  多副本按 working 数组下标寻址**绝不按 pid**；store 层 baseline/越界/id 错位三
  守卫命中（返回 false）静默不动不炸，不得 fallback 按 pid 找同名项。重置恢复
  **算法基线**（连已保存编辑一起抹掉，定案语义）。
- **端到端门 = smoke_edit_polish.mjs 46 检查**：S8 段（17 项）锁 O 镜像→保存→
  导出 placed 恰一项 mirror:true + PLT/DXF **正文几何**镜像坐标对拍（镜像前
  DXF layer1 轮廓经 R(θ₁)·M·R(−θ₀) 复合公式复算，PLT ≤2 unit / DXF ≤0.05mm，
...
- **两算子职责**（`src/lib/editGeometry.ts` 中段「edit-drag-snap US-001」分节，纯函数零 DOM）：`contactT(v, dir, a, b)` = 顶点 v 沿单位方向 dir 平移到线段 ab 的参数 t 解（叉积线性解 t = −cross(v−a, d)/cross(dir, d) + 命中点须落线段本体 [a,b] 含端点）；`firstContactDistance(moved, obstacle, dir)` = 两多边形沿 dir 的首触距离（moved 全部顶点×obstacle 全边 + obstacle 全部顶点×moved 全边（contactT 传 **−dir**，相对运动视角）两向取**最小正 t**）。全部解析零迭代零 RNG —— US-002 snap.ts 的 retreat/attract 只消费该单一标量。
- **dir 单位向量契约**：t = mm 的前提是调用方传单位方向（contactT/firstContactDistance 不归一化）；零向量 dir 对一切边近平行 → 恒 null 安全不炸。
- **近平行返回 null 不抛异常**（|cross(dir, d)| ≤ EDGE_PARALLEL_EPS·|d|，即 |sinθ| ≤ 1e-9）：牛仔裤弧片密集顶点实况 —— 近共线擦过的解是数十米外的浮点噪声，且 cross 恰 0 时除零产 ±Infinity/NaN 会污染最小值；真实触点在线段**端点**上，由相邻边（与 dir 不近平行）的端点命中覆盖。零长退化边同 null（防御）。
- **负 t 语义**：contactT 返回原始解（可为负 = 触点在身后，如已交叠回看）；过滤正 t 是 firstContactDistance 内部职责 —— 返回值恒为「沿 dir 前方的下一次接触」。
- **口径无关性（红线）**：吸附数学只发生在世界坐标 worldPolygon 顶点数组上，与 rot/mirror 无关 —— 镜像+旋转片 vs 直接构造的同物理形态片同 t（单测锁死，含顶点序无关性）。调用方（US-002 起）负责把 EditPiece.worldPolygon 喂进来，**不得**在算子里重算变换。
- **平手裁决**：多对同 t 时严格 `<` 保留首遇 —— 值相等故结果与枚举序无关；多**邻居**间平手取数组下标小是 US-002 引擎层职责，与本算子无关。
- **改这两个函数必须同步** `src/lib/__tests__/editGeometry.test.ts` 的 edit-drag-snap describe（16 项：矩形/斜向/凹形 L 手算精确值、近平行弧段 null 不抛、无交点 null、空多边形防御、镜像口径无关性、确定性双跑）。

## edit-drag-snap US-002 关键约定（松手吸附引擎纯函数 snap.ts；2026-09-06）

- **职责边界（红线）**：`src/lib/snap.ts` 的 `computeSnapCorrection(dragged, rawTr, sess, others, opts)` 只做「给定落点 → 纠正后 translation」的**单次纯函数求解**；触发与否（贴附 snap 会话 vs 自由拖动 —— 2026-09-17 起触发手势 = Alt+左键，原右键因 Edge 内置鼠标手势冲突整体迁出，见 US-003 节沿革）是调用方（US-003 EditCanvas）职责，引擎不感知按键/DOM/事件。结果 `{tr, kind, partnerKey}` **只改 translation**，永不动 rot/mirror；`kind`：free = 未施加纠正（含一切 fail-open）/ retreat = 回退到首个安全界 / attract = 吸拢到触点 −1nm。
- **P0 retreat 机器**（后端 polish.py `_slide_west_touch` 同款「粗扫定界 + 二分收敛」）：落点违谓词时以 lo = `sess.lastSafeTr`（会话内最后安全位）/ hi = rawTr 线段参数化（t 单位 mm），**20mm 粗扫步括出首个穿越小段**——凹形使自由区间不连通，朴素二分 [lo,hi] 会跳过窄障碍隧道穿越（单测夹具：窄障碍 150-155 中途 + 远墙 400-500 终点，朴素二分收敛 300、粗扫正确回 50）——再二分 ≤14 轮（BISECT_ITERS）到边界，自由侧回退 `SEP_NUDGE_MM`（1nm）保证终态布尔交面积精确 0。
- **P1 attract 机器**：落点合谓词时沿**末帧拖动位移方向**（rawTr − lastSafeTr；位移 <1nm 的零位移帧**退化质心连线**，逐邻居取该邻居质心方向）用 US-001 `firstContactDistance` 解析取首触距离 t，`t ≤ ATTRACT_MAX_GAP_MM(10)` 才吸，吸附位 = 落点 + (t − 1nm)·dir；多邻居取 t 最小者、**平手取 EditPiece.key 小者**（与 others 数组顺序无关的确定性）。背离方向（首触在身后）无候选不动。
- **谓词与基线**：`安全 = computeOverlap(pieceAt(拖片, 位), others).areaMm2 ≤ sess.startOverlapMm2 + COLLIDE_AREA_EPS_MM2(1e-9)` —— 复用 overlap.ts 作 oracle；起手存量重叠（solver 全局容差 MAX_OVERLAP_MM=10 构造的布局）**不恶化、不误清**（恶化位 retreat 回基线等价位、改善/持平位不动，单测锁死）。
- **clamp 与 fail-open**：候选（retreat/attract 终位）须满足 `minX ≥ −1e-6 && minY ≥ −1e-6 && maxY ≤ gate+1e-6`（CLAMP_EPS_MM 同后端 GATE_EPS_MM 浮点容差；x 右界不钳 = US-003 clampPlacement 同口径），违反即弃用回落 raw；引擎**不重钳** raw（调用方 pointerup 已 clamp）；`lastSafeTr` 为 null / 路径零长 / 锚点自身违谓词（陈旧会话防御）/ 粗扫不可达 / 任何内部异常 → 一律 fail-open 返回 raw —— 调用方拿到的最坏结果 = 与不吸附一致，绝不卡死拖动出口。
- **多副本红线**：邻居寻址全程 EditPiece.key（placed_items 数组下标）逐个枚举，**绝不按 pid 建 Map 去重** —— 同 pid 多副本是独立邻居：retreat 谓词按副本求和（5 副本各 50 ≤ 基线 200、合计 250 > 200 → retreat，去重会误判 free）、attract 按 key 取 t 最小副本，均有单测锁。
- **常量口径**：`SEP_NUDGE_MM`/`COLLIDE_AREA_EPS_MM2` 与后端 polish.py 同名常量一致；`ATTRACT_MAX_GAP_MM = 10` 为 2026-09-06 定案（测试 import 锁值）；测试分离断言容 1e-12 浮点余量 —— ~50mm 量级 double 的 ulp ~7e-15，「大数−大数」消去把 1nm 量出来带 ~1e-14 级噪声（物理不变量 = 严格分离 + ~1nm）。
- **改本引擎必须同步** `src/lib/snap.test.ts`（22 项：retreat 解析对拍 0.01mm / 隧道穿越 / 基线不恶化不误清 / 5 副本求和锁 / attract 6mm·15mm·零位移·平手 key·同 pid 寻址 / clamp 三变体 + retreat 候选违 clamp / getter 注入异常 fail-open / 确定性双跑逐位全等 / 空邻居·无锚点·自身 key 跳过防御）。

## edit-drag-snap US-003 关键约定（EditCanvas 贴附会话接线；2026-09-06，2026-09-17 触发手势迁 Alt+左键）

- **触发手势沿革（2026-09-17 用户定案）**：US-003 原触发 = 右键（`button===2`），实测与 Edge 内置「鼠标手势」冲突 —— 手势在浏览器层拦截右键按住拖动（画轨迹执行后退/关标签页等动作），官方确认网页无法用 JS 压制（`preventDefault`/`contextmenu` 拦不住，chess.com 右键画箭头同款冲突），且右键向左拖会被识别为「后退」直接导航离开页面丢编辑态。定案 = **整体切换 Alt+左键**（纯左键零回归红线不动 —— 修饰键+左键同样不占左键自由拖动；右键全域失效无任何画布会话，Edge 手势用户误右拖画布零副作用）。
- **按键门控（红线）**：`onPointerDown` 顶部 `if (e.button !== 0) return;` —— 仅左键进门；毛版 polygon 分支内 `const snap = e.altKey` 分流（Alt+左键 = `MoveDrag{snap:true}` 贴附会话；纯左键 = 既有拖片/平移/转柄路径**逐字节零改动**，回归红线）；修饰键仅对 polygon 贴附有意义（旋转柄/空白分支不分流，Alt+左键照常转柄/平移）；其余按键（右键 2/中键 1/后退 3/前进 4）不起任何会话。`snapSessRef`（`SnapSession | null`）Alt+左键 pointerdown 起 `{lastSafeTr:null, startOverlapMm2:Infinity}`（紧随 selectPiece → refreshMetrics 首帧落真实基线；首帧 degraded 则 Infinity = attract-only fail-open）、纯左键显式置 null（防御清残值）、pointercancel/骨架重建/卸载一律作废不求解。snap 标志在 pointerdown 锁存于会话（拖动中松开 Alt 不影响松手求解）。
- **contextmenu 吞除（迁出后维持）**：svg 上 native listener `preventDefault()` —— 工具型画布无自定义右键菜单需求；右键虽已无画布功能仍不弹系统菜单（防拖动/点击失误弹出含「后退/刷新」的菜单误操作丢编辑态，Edge 手势用户误右拖时不叠加菜单干扰；单测 + 浏览器 trusted click 双锁）。
- **松手单次求解（applySnapOnRelease）**：endDrag 在 flushFrame 之后调用（working/池/DOM 已是末帧 clamp 落点位）→ 会话态**先取后清**（落笔触发的 refreshMetrics 不再续写 lastSafeTr）→ 防御链（manifest/entry/pool/working 项/dragged 逐个 null 检查）→ `rawTr = clampPlacement(...)`（幂等防御）→ `computeSnapCorrection(dragged, rawTr, sess, pool, {gate})` → `kind === 'free'` 直接 return（无吸附/fail-open = 末帧落点原样保留）→ 否则 `commitDragPlacement(index, entry, it.rotation, res.tr, mirror)` **唯一落笔出口**（只改 translation；rot/mirror 读 working 现值原样透传永不触碰）+ `flashPartner(res.partnerKey)`。引擎**只被 snap:true 会话的 pointerup 调用** —— 左键拖动/键盘变换（L/K/空格/O/I/R）/旋转拖柄路径零吸附，applyKeyTransform/applyRotateFrame 与左键路径不动。
- **lastSafeTr 免费跟踪**：refreshMetrics try 分支内 sess 非空时 —— 首帧（lastSafeTr===null）`startOverlapMm2 = res.areaMm2` 落基线，谓词 `res.areaMm2 ≤ 基线 + COLLIDE_AREA_EPS_MM2`（与 snap.ts 引擎**同口径同 eps**，故常量自 US-003 起 export）成立时续写 `lastSafeTr = [dragged.tr[0], dragged.tr[1]]`；catch 分支（布尔交 degraded）保守跳过（锚点停在末个好帧，会话全程 degraded 则无锚点 → retreat fail-open 回 raw 与不吸附一致）。**拖动帧零新增计算**（复用本次 computeOverlap）、rAF 合帧管线不变；仅 snap:true 会话消费，左键/键盘会话本 ref 恒 null。
- **会话语义推论（单跳拖动的零位移帧）**：pointerup 先 flushFrame 落末帧 → refreshMetrics 已把 lastSafeTr 更新为 raw（若安全）→ attract 按引擎设计的零位移退化走**质心连线方向**（逐邻居）；retreat 落点违例时 lastSafeTr 停在会话起手位（或末安全帧）= 回退锚点。
...
- **指南卡**：贴附行文案 = `贴附：Alt+左键拖动松手贴附`（2026-09-17 随手势迁改；文案不含「形态」「保存」—— 反向锁与其余行一致）。
- **改本接线必须同步** `src/components/edit/__tests__/EditCanvas.test.tsx` 贴附 describe（12 项，2026-09-17 起 `firePointer` 加 alt 参 —— `(el, type, x, y, button=0, alt=false)`，贴附用例 `0, true` 派发 altKey、非左键门控用例保留裸 button 1/2）与 UI 覆盖层子层数断言 `childNodes.length === 3`。

## edit-drag-snap US-004 关键约定（吸附语义单一真相 + 端到端冒烟 调用方必读；2026-09-06）

贴附拖动松手贴合收官故事（2026-09-17 起触发 = Alt+左键，原右键迁出见 US-003 节
沿革）：吸附语义文档化 + `scripts/smoke_drag_snap.mjs` 双护栏
冒烟（吸附功能护栏 + 纯左键零回归红线护栏）。改吸附引擎/接线/导出链前先读本节。

### 吸附语义（六条单一真相，与 US-002/003 节互为索引）

- **触发 = 仅贴附会话（Alt+左键）**：`EditCanvas.onPointerDown` 毛版 polygon 分支
  `e.altKey`（左键进门后）才起 `MoveDrag{snap:true}` + `snapSessRef` 会话；其余按键
  门控见 US-003 节（纯左键 = 既有拖片/平移/转柄路径逐字节零改动，非左键不起会话）。
...
- **永不吸附路径（回归红线）**：纯左键拖动 / 键盘变换（L/K/Shift/空格/O/I/R）/
...
- **相位**：S1 上传 5336 → 20s 求解 30 片 → 基线 PLT 导出（placed 与 WS 末帧
  ...
  retreat/final-attract 五路全预测通过才采用 —— 全流程确定性不靠碰运气）；B Alt+左键
  pointer 序列行为段（attract DOM points == 引擎 tr / 伙伴高亮轮廓 == partnerKey
  世界轮廓 / 指标 0.0 / 高亮淡出 / trusted 右键 contextmenu 被吞 + 无位移（右键
  全域失效回归）/ trusted Alt+左键无位移（真实输入链 altKey 生效）/ 拖动中合成
  contextmenu 被吞 / 拖离远端 free / 深叠 retreat + 独立布尔交复核）；
  R 纯左键红线两档；C 末次 attract 回贴 → 保存 → 导出 placed 全精度贯通
...
- **两层拆分**：`applySyntheticRun(manifest, frameLike, seed, origin?, note?)` 模块级
  落笔（清场 runRegistry.clear + editStore.invalidate → 深拷贝 placed →
...
## 会话过期自动恢复 US-003 关键约定（启动期恢复与停留期引导 调用方必读；2026-09-13）

prd-session-expiry-auto-recovery 前端收官故事：启动期探测 401 自动换 sid 调
`/api/state-recover`（消费 US-001 内存 checkpoint + US-002 端点）恢复工作状态；
停留期 401 **不自动恢复**，阻断弹窗引导刷新（刷新 = 启动期恢复入口）。改恢复链 /
api.ts 探测分支 / sid 生命周期 / 弹窗文案前先读本节。

### 架构（依赖倒置钩子槽）

- **api.ts 保持 store-free，恢复逻辑全在 `lib/sessionRecovery.ts`**：api.ts 暴露
  `registerStartupSessionRecovery(fn)` 模块级钩子槽（null = 未注册）；sessionRecovery.ts
  模块加载尾自注册 `recoverExpiredSession`。App.tsx **静态** `import './lib/sessionRecovery'`
  —— 静态 import 在任何 effect 驱动的 apiFetch 前执行，注册必先于首次探测。lib→store
  import（stateFile.applyRestorePayload 等五 store 落笔）以 stateFile.ts 为先例。
- **单飞（single-flight）在 ensureSession 的 once-promise 内**：探测 401 →
  `await startupRecovery()`（sid 换新 + recover fetch + 重探全在 once 内）→ 并发
  apiFetch 等同一终态；业务请求在探测门前未发出，**无需请求重放**。恢复后 blocked
  （429）→ 吞掉；否则裸 fetch 重探（特权点 ①）再走 inspectSessionError。
- **探测 401 三分支**：`code==='session_expired'` 且钩子在 → 恢复链；401 **无 code**
  （非结构化体）→ 不恢复不阻断，探测落定放行（防网关/代理 401 误触发）；**无钩子**
  （装载序缺口防御）→ 落回老路径阻断弹窗。

### 时序红线与 sid 生命周期

- **peek 必须先于 clear**：`peekPersistedSessionId()`（session.ts 新增）非铸造只读
  （缓存→localStorage 双查，空/损坏 → null）；`getSessionId()` 是 lazy-mint —— 先
  get 会铸造新 sid 覆写 localStorage，旧 sid（from_sid）永久丢失。恢复链固定序：
  `peek → clearPersistedSessionId() → getSessionId()`。clearPersistedSessionId
  **唯一调用方 = sessionRecovery.ts**（US-005 的 triggerSessionBlock 顺手清已废止）。
- **停留期（探测已落定后的 401：triggerSessionBlock / useSolveRun WS error）不清 sid**：
  旧 sid = 刷新后启动期恢复的 from_sid；SessionExpiredModal session_expired 文案 =
  「会话已过期，刷新页面后将恢复工作状态」（刷新按钮 = 唯一出口，点击 reload 触发
  启动期恢复链）。
- **恢复响应三分支**（sessionRecovery.ts）：200 → `applyRestorePayload`（状态文件
  US-004 同函数零改动复用：五 store 落笔 + run 合成 + 自动切超排 Tab）+ toast
  「工作状态已恢复」；404（checkpoint 不在/已消费/超 TTL）或网络错 → 静默 + toast
  「上次会话已过期，已开启新会话」（干净新会话）；429 → triggerSessionBlock
  ('session_limit') 走既有满员弹窗。`recoverExpiredSession` 外层 try/catch 兜底
  兜到新会话 toast，finally 复位 sessionRecovering。

### 轻加载态与文案

- **uiStore.sessionRecovering + SessionRecoveryNotice**：恢复期顶部居中「正在恢复
  工作状态…」role=status aria-live=polite，pointer-events:none 不挡交互；idle → null
  零 DOM。旗标在 sessionRecovery 内 set/finally 复位（唯一写入方）。
- **文案两 toast 一弹窗**（字符串常量导出自 sessionRecovery.ts，测试逐字锁）：恢复
  成功「工作状态已恢复」/ 兜底「上次会话已过期，已开启新会话」/ 停留期弹窗
  「会话已过期，刷新页面后将恢复工作状态」。session_limit 文案不变。

### 后端语义（US-002 既定，测试必读）

- **恢复成功即消费 checkpoint（single-use）**：旧 sid 已入墓碑永不复用，防多 Tab
  反复恢复放大名额；第二次恢复 404 落兜底新会话。
- **恢复后会话被 edit_hold 钉住 2h（生产语义「恢复后 2h 不再过期」）**：
  `/api/state-recover` → rebuild_session_from_document 内含 `edit_hold.refresh(新sid)`
  —— **测「恢复后再过期」（停留期弹窗相位）必须 `MS_EDIT_HOLD_SEC=5` 短值起服**，
  否则会话死活不过期（浏览器验证实锤踩坑）。

### 测试与验证

- **单测**：`lib/__tests__/sessionRecovery.test.ts` 10 项（mockRoute 按 X-Session-Id
  路由：200 全链 URL 序 `/api/session→/api/state-recover→/api/session→/api/ptypes` +
  from_sid + localStorage + store 断言 + 自动切 Tab / 404 兜底 / 429 阻断恰 2 请求 /
  网络错 / 3 并发单飞 / sessionRecovering 生命周期 / 健康探测零 recover / 停留期
  markSessionProbedForTest → 401 → blocked + sid 保留）；session.test.ts peek 5 项
  （含时序链）；api.test.ts AC8 改写 + 探测 401 两分支；SessionRecoveryNotice 3 项；
  uiStore 2 项。
...
- **__netLog 经 addInitScript 每次导航重置**：跨 reload 断言须按新日志口径计数
  （B2 相位 = 恰 1 recover（404）+ 2 probes）。
- **恢复后回显断言口径**：applyRestorePayload 置 commitStatus='idle'、保存按钮
  disabled—— DOM 断言用 `.qty-cell-input` 实值 / `#restart`（done 态 bestRun 在案）
  / `.nest-card polygon` 数，不用 commit-status。

## 会话过期自动恢复 US-004 关键约定（自动 checkpoint 调度与启动清理 调用方必读；2026-09-13）

- **触发面五路**（`lib/sessionCheckpoint.ts` 单一所在，App.tsx 静态 import 装载接线）：
  formStore/qtyStore 变更订阅 → 去抖 `CHECKPOINT_DEBOUNCE_MS=3000`（常量可调）；
  `runRegistry` bestRun 达 done → **立即**（价值最高时刻不等去抖；无 lastFrame 的
  error run 与非 best 晚到 run 跳过）；`editStore.save` 落定 → **立即**（识别写序 =
  `savedDirty false→true 且 working 引用未换`——open 必换 working 新数组，重开不
  误触）；`visibilitychange→hidden` → 立即 flush 清空去抖；`pagehide` → keepalive
  fetch 同步发起（不走 sendChain，卸载期微任务不可靠）。
- **发送口径**：一律 `apiFetch POST /api/state-checkpoint`（统一出口，**不新增裸
  fetch 特权点**——仍恰两处）+ body = `buildSavePayload()` 不带 save_as + 无 doc
  （uploadStore.doc null）静默跳过 + 全异常吞不重试（`{stored:false, empty/conservation}`
  后端语义零处理）。端点 peek 口径不刷活性 = 自动调度**不构成会话保活**。
- **串行 + 合并**：sendChain promise 链保 POST 依序完成；载荷**发送时刻**现取 ——
  已有排队 send 时新 flush 跳过（合并语义依赖现取，勿把 buildSavePayload 提前到
  入队时刻）。
- **启动清理门序不可换**（`clearCheckpointAfterProbe`，App 挂载
  `probeSession().then(...)` 消费）：`blocked → 跳过`（429 满员快照保留，名额腾出
  后刷新可再恢复）→ `wasStartupProbeExpired() → 跳过`（探测 401 = 快照留给启动期
  恢复消费；恢复成功后端 single-use 已删）→ 否则 `DELETE`（F5 = 干净重置，防
  「稍后再过期恢复出 F5 前旧状态」幽灵回潮）。401 恢复路径里 applySyntheticRun
  触发的重落快照依赖 DELETE 被跳过（时序红线）。
- **run 结束必经 `markRunDone`**（runRegistry 新增单一出口：done 置位 + 同步通知
  `subscribeRunDone` 订阅者，观察者异常吞）。useSolveRun.finish 与
  applySyntheticRun 已接；**未来任何新 run 来源（合成/导入/策略扩展）必须调它**，
  否则求解完成自动 checkpoint 断链。applySyntheticRun 内字段全落位后才 markRunDone
  （观察者读终态）。
- **api.ts 增 `wasStartupProbeExpired()`**（probeExpired 旗标：探测 401
  session_expired 且恢复分支接管时置位；resetSessionForTest 一并复位）—— 仅启动
  清理消费，勿作他用。
- **editStore.save 识别依赖写序形状**：save 若改 set 结构（例如同时改 working），
  须同步改 sessionCheckpoint 的识别判据。
- **测试隔离**：`resetCheckpointForTest()` 清去抖定时器 + 复位串行链；测试文件
  beforeEach 须置于 store 复位**之后**（qtyStore.resetQuantities 换引用会触发一次
  调度，不清掉会跨用例定时器污染）。jsdom 断言 hidden 用
  `Object.defineProperty(document, 'visibilityState', {value:'hidden', configurable:true})`
  + dispatchEvent。
- **`scripts/us004_checkpoint_verify.mjs`**（默认 TTL 起服，不涉过期）：上传 → 改
