...
├── types/                 # US-002 ✅：ws.ts / piece.ts / v03.ts；上传预览 US-005 ✅ parsed.ts（US-001 v2 响应契约；裁片编号化 US-003 ✅ 删 name/ptype/paired —— label 即 g 码全链路主键）；上传预览 US-011 ✅ qty.ts（PieceQuantity/PieceQuantityMap）；US-027 ✅ solvePhase.ts（SolvePhase 五态）
├── lib/                   # US-002 ✅ ws.ts；US-003 ✅ geometry.ts；US-004 ✅ params.ts；US-006 ✅ seek.ts；US-007 ✅ download.ts
├── store/                 # US-002 ✅ runRegistry.ts；US-003 ✅ appStore.ts；US-001 ✅ uiStore.ts（US-015 ✅ 扩 nestingEnabled + setNestingEnabled + setTab guard）；上传预览 US-005 ✅ uploadStore.ts（US-012 扩 qtyDialog 已随矩阵化重构 US-003 删除；US-013 扩 zoom + open/close；US-021 扩 commitStatus/commitError/commitSummary + reset 同步清）；上传预览 US-011 ✅ qtyStore.ts（矩阵化重构 US-001 简化：perSize+baseValue 单模式 + setPiecePerSize/setRowAll/resetQuantities/hydrate + clampQty/getPieceDisplay 纯函数，hydrate 双入口已合并）；US-018 ✅ controlPanelStore.ts（modal + previewLabel 双显隐字段；US-003 起 previewPtype→previewLabel 键 = g 码）；US-029 ✅ tourStore.ts（activeTour/stepIndex/seen + localStorage 持久化 + TOUR_VERSION 版本号强制重看）
...
└── components/
    ...
    │   ├── QtyMatrix.tsx       # 矩阵化重构 US-002 ✅ 数量矩阵（US-003 ✅ 接入 PreviewPage）：行=doc.sizes 全码（行头码按钮切 activeSize）× 列=label 并集[徽章+compact 缩略图+「≡」整列设值]（2026-08-16 转置）；格内 clampQty 编辑+Enter/Tab 跳格；0/.zero、缺片/—.missing、特例/.override 三态；sticky+flex 内滚；小计 US-003 起 Σ 数量口径（配对 ×2 乘数已删，一份=母版一个轮廓）+全 0 警示
    │   └── __tests__/
    ...
    │       ├── QtyMatrix.test.tsx        # 矩阵化重构 US-002 ✅ 38 项单测（US-003 起配对/×2/物理口径用例全改写为 Σ 数量口径：行列结构/切码/openZoom/格内编辑/0 特例高亮/整列设值 popover/小计与总片数 Σ 联动/全 0 警示）
    ...
    ├── ControlPanel/      # US-004 ✅ 8 子组件；US-005 ✅ MultiSeedControls；US-007 ✅ ExportButtons；US-018 ✅ PerTypeOverrides 改按钮触发 + PerTypeOverridesModal（高级配置弹窗；**矩阵化 US-004 起行=码号 × 列=g 码 矩阵 + ≡ 整列设值 + 三层独立 ESC**）+ PtypePreviewModal（片型放大预览）；US-031 ✅ 加 data-tour 锚点（doc-banner / start-btn 父容器 / param-form 在 SizePicker / export-group 在 ExportButtons）
...
- **列头即切码入口**：列头是 button，点击 `setSize(该码)`（US-003 起替代 SizeTabs 驱动下方图形预览），activeSize 列 `.active` + aria-pressed。缩略图 rep 优先 activeSize 版本（与图形预览同码视觉一致），点击 `openZoom(label, activeSize)` 复用 PieceZoomModal。
- **小计 Σdemand 口径（本 Story）**：行合计 = Σ该行各码 demand；每码小计 = Σ该码各 label demand（缺片格不计）；工具条总片数 = sizeSubtotals 之和；全 0 且有行 → 红色警示。**US-003 起 Σ 数量口径（无乘数）**——一份 = 母版一个轮廓，不合成镜像；三处小计同源只做求和。
- **CSS 布局不变量**：`.qty-matrix-scroll` max-height:45vh 内部滚动；sticky 表头/首列（.qty-rowhead）/底行（tfoot）；表格 `border-collapse: separate` + 自绘边框（collapse 会吞 sticky 格边框）；sticky z 层级 thead=3 / corner=5 / rowhead=2 / tfoot=3 / subtotal-rowhead=4；窄屏（≤1366）靠行头 220px + 格子 64px min-width 自然横滚。不引入 CSS 框架。
- **「重置为默认 1」**：逐行 `setRowAll(label, rowSizes(label), 1)`（值与 baseValue 双回 1，整行同值 → 特例高亮清空）。
...
- **三层独立 ESC（AC#10 关键不变量；US-004 扩入整列设值弹层）**：PerTypeOverridesModal 的 ESC listener 在触发前判 `useControlPanelStore.getState().previewLabel !== null` 或本组件 `fill !== null` → return（让上层优先处理）；PtypePreviewModal 的 ESC listener 仅判自身 `previewLabel !== null`；ColumnFillPopover 的 ESC listener 只关弹层。当放大预览/整列设值打开时 ESC 只关最上层，底层 modal + 草稿保真；再次 ESC 才关下一层。改任一层 ESC 逻辑需同步 PerTypeOverridesModal.test.tsx「ESC 不关底层 / 双层独立 / 只关整列设值弹层」+ PtypePreviewModal.test.tsx「ESC closes」+「stacked close preview keeps 底层」用例。
- **『D7 预填』已删（US-004 矩阵化起空串 = 继承默认）**：未配置格渲染空串 + placeholder `d≤10`/`t≤45` 提示，**不再预填 '0'/'0'**（空串 = 继承全局默认 0/0；显式填 0 = 强制 0，两者可区分，collectParams 口径一致）。
- **草稿 + 确定模式（非即时生效）**：PerTypeOverridesModalInner 用 useState 持 `draft: PerTypeFormMap`（两级嵌套字符串形状，lib/params.ts）；用户编辑/整列设值仅改草稿；点确定才 `onChange(pruneDraft(draft))` + closeModal（prune 剔除双侧全空 (label,sizeKey) 格与全空 label，与 collectParams 剔除口径一致）；点取消 / 遮罩 / ESC 仅 closeModal，草稿丢弃。**目的**：矩阵格 input 即时写 form 会让 ControlPanel 频繁重渲染 + 用户误关时无法回滚。改即时生效会破坏此体验（同 QtyMatrix 行填充同模式）。
- **`key="per-type-modal"` 强制 PerTypeOverridesModalInner 重建**：每次 modal 显隐切换时 Inner 组件重建，useState 从 form.per_type 重新深拷贝 draft（避免上次草稿残留 / StrictMode 双 mount 时旧 draft 污染）。改 key 需同步「initial draft preserves form.per_type 非空值 / 不预填 0」用例。
- **US-004 矩阵结构（行=码号 × 列=g 码，格=(g 码, 码号) d/tol 双输入）**：行集 = uploadStore.doc.sizes（后端升序、null 通用殿后）∪ form.per_type 已配置 sizeKey 追加行（旧配置不静默丢），doc=null → SIZES fallback（与 SizePicker chip 同源）；列集 = /api/ptypes representatives 键 ∪ form.per_type 已配置键（fetch 失败降级空 reps 仍可配置），`compareByLabel` 数值序（先长度再字典序：g02<g99<g100，**勿去零填充**）。sizeKey 约定 = `perTypeSizeKey(size)`：number→String(size)、null→'null'（与 serializeQuantities 同口径单一实现）。testid = `d-${label}-${sk}` / `tol-${label}-${sk}` / `per-type-fill-btn-${label}` / `ptype-thumb-${label}` / `per-type-fill-d|tol`；aria = 「裁片 {g 码} 码 {码号|通用} 重合/旋转」。
- **缺片格「—」（QtyMatrix 同口径）**：doc 中该 g 码存在（labelsInDoc 命中）但该码号没有 → `.per-type-cell.missing` 渲染 disabled「—」（title=「该码号无此裁片」）；label 完全不在 doc → 不判缺、格子可编辑（parse 与 commit 数据可能暂不同步，不阻塞配置，后端命不中为 no-op）。整列设值（≡）应用时跳过 missing 行（**不给缺片码造 phantom 键**，QtyMatrix 同原则）。
- **「≡」整列设值（ColumnFillPopover，文件尾内部组件）**：列头 `.qty-rowfill-btn` 点击 → 开居中弹层（Portal 到 body + fixed 定位于表格容器可视区中心，不锚 sticky 列头 —— QtyMatrix 2026-08-16 修复同款定位）；复用 `.qty-fill-*` 样式段 + `qty-fill-popover--per-type` modifier；d/tol 任一侧留空 = 该侧整列继承默认（清空）；应用 clamp 后写该列全部非 missing 行（draft 级）；Enter 快捷应用 / ESC/遮罩/取消只关弹层。z-index：backdrop 1150 / popover 1160（必须盖过 per-type overlay 1100，QtyMatrix 原生 1000/1010 不够）。
- **URL 分享格式（US-004）**：`perTypeToUrlParam(form)` → `label@sizeKey=d,tol;...`（仅非空格产出）；`perTypeFromUrlParam(raw)` 正则 `^(g\d{1,4})@(null|\d{1,4})=([^;,]*),([^;]*)$` 逐段校验，旧 ptype 键/中文/NaN/乱拼段**静默跳过不抛错**（lenient decode）；两函数在 lib/params.ts 单一实现，改格式需同步 params.test.ts「per_type URL 分享格式」6 项。
- **片型缩略图来自 GET /api/ptypes（US-020 契约）**：PerTypeOverridesModal mount 时 fetch `/api/ptypes` → `PtypesResponse.representatives[label]`（US-004 起键 = g 码）；rep 存在 → `<button class="ptype-thumb">` 内渲染 PiecePreviewSVG compact（64×64，无 label，pad=2）；rep 不存在 / fetch 失败 → button disabled + 显示 g 码首字占位（loading 态「…」）。**fetch 用 cancelled flag 防 StrictMode 双 mount race**（mount→cleanup→mount，第一次 fetch 的 setState 被 cancelled 跳过）。改 fetch 逻辑需同步「mount triggers fetch」+「fetch failure degrades」+「fetch success renders compact svg」3 项。
...
- **z-index 层级**：tooltip(100) < piece-qty/zoom(1000) < per-type(1100) < per-type fill backdrop/popover(1150/1160) < ptype-preview(1200)。放大预览覆盖在最上；整列设值弹层覆盖在高级配置 modal 之上；高级配置 modal 覆盖在 ControlPanel 之上。改 z-index 需同步视觉回归。
...
- **不引入 CSS 框架**：`.per-type-wrapper/.per-type-btn`（#2c5d8f 蓝）+ `.per-type-overlay/.per-type-modal`（#26282e 暗底）+ `.per-type-table-wrap`（overflow-x:auto）+ `.per-type-table`（table-layout:fixed）+ `.per-type-rowhead`（sticky left，52px）+ `.ptype-col`（92px 宽，US-004 双输入列宽）+ `.ptype-thumb`（64×64 cursor:zoom-in）+ `.qty-colhead-meta`（列头徽章+≡ 居中）+ `.per-type-cell`（d/tol 双 input 堆叠居中）+ `.per-type-missing`（「—」灰字）+ `.qty-popover-backdrop--per-type`(1150) / `.qty-fill-popover--per-type`(1160) + `.per-type-actions/.per-type-btn-cancel/.per-type-btn-confirm`（#2ea06c 绿）+ `.ptype-preview-*`（z=1200）全部沿用 style.css 命令式 className，与 ControlPanel / piece-zoom-modal 暗背景同色系；`.qty-fill-*`/`.qty-label-badge`/`.qty-rowfill-btn` 跨组件复用（QtyMatrix ↔ 高级配置弹窗）。
- **浏览器验证（US-004 已做）**：scripts/us004_verify.mjs CDP harness 15/15 PASS —— 真实母版上传 → 矩阵渲染（g01–g10 动态 10 列 × doc.sizes 11 行码号、d/tol 格、10 个 ≡）→ 双层预览 + 三层 ESC → 配置 g03@28 d=1.5 + g05 整列 d=2 → 确定 → 选码 → 开始求解 → patch WebSocket.prototype.send 捕获 WS start payload，断言 `per_type.g03["28"].d === 1.5`（端到端 AC）+ g05 全码号键。US-018 时代的「留作 US-021 核对」已由本 harness 覆盖。
...
- **表单字段全字符串存储**：`FormState`（lib/params.ts）的 number 字段（time/seed）+ per_type 两级嵌套 `PerTypeFormMap = Record<g码, Record<sizeKey, {d,tol}>>` 的 d/tol 都按 input.value 字符串持有。理由：per_type 必须「空串 = 继承默认」与「"0" = 显式 0」可区分（旧 vanilla 实现 inp.value.trim() !== '' 同口径）。sizeKey = `perTypeSizeKey(size)`（number→String / null→'null'）。
- **collectParams(form) 纯函数（US-004 起双层遍历）**：params 永远全 0（US-019 起主面板输入已删）；per_type 按 label→sizeKey 双层遍历，仅 trim() !== '' 档写入（缺档省略）、双侧全空格剔除、全空 label 剔除、整体空 → null；输出 `PerTypeOverrides = Record<g码, Record<sizeKey, {d?, tol?}>>`（WS payload 形状，与后端 build_instance (label, sizeKey) 命中一致）。任何修改必须同步 `lib/__tests__/params.test.ts`（collectParams 嵌套组 + URL 分享组）+ AC#2 默认值断言。
...
- **~~PerTypeOverrides 行序 = V03_PTYPES 顺序~~（已废）**：V03_PTYPES 固定 10 片型清单已删（裁片编号化 US-003）；高级配置矩阵列集 = /api/ptypes reps 键 ∪ per_type 已配置键动态并集，`compareByLabel` 数值序（详见 US-018 节 US-004 矩阵条目）。
...
- **ptype/paired 是 additive 可选字段**：`ParsedPiece.ptype?: string` / `paired?: boolean`。旧响应 / 测试桩 / 防御路径缺字段一律按非配对（×1）计 —— QtyMatrix `pairedOf` 用 `=== true` 判、SizePicker 用 `piece.paired ? 2 : 1`。改兜底语义须同步 QtyMatrix「缺 paired 字段 → ×1 旧口径兼容」describe 与 SizePicker「缺 paired 字段」用例。
- **物理片数口径只活在展示层**：行合计列 / 每码小计行 / 工具条总片数（QtyMatrix）+「总裁片数量」（SizePicker）= Σ demand × (paired ? 2 : 1)；WS `quantities` 线格式仍是「份」口径（serializeQuantities 零改动）。把 ×2 写进 qtyStore / start payload 属回退。
- **乘数按 (label, size) 逐格取**：`multOf(label, size)` 从 piecesByLabel 逐格查 `paired`（理论同 label 跨码不一致时按格累加）；仅行头徽章 `rowPaired` 取「任一码配对」（视觉汇总）。QtyMatrix「同 label 跨码 paired 不一致」用例锁死。
