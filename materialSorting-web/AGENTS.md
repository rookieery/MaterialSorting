...
└── components/
    ...
    │       ├── QtyMatrix.test.tsx        # 矩阵化重构 US-002 ✅ 38 项单测 8 组（转置后行=码号 × 列=g 码；裁片编号化 US-003 ✅ 配对/×2/物理口径用例全改写为 Σ 数量口径：行列结构/行头切码/缩略图 openZoom/格内编辑/0 特例高亮/整列设值 popover/小计与总片数 Σ 联动/全 0 警示）；腰头成带 US-013 ✅ +4 项不成对警告（bandCol 奇数格 .odd+title / 列头徽章 / 偶数列无 / 取消勾选清除）
    ...
    ├── ControlPanel/      # US-004 ✅ 8 子组件；US-005 ✅ MultiSeedControls；US-007 ✅ ExportButtons；US-018 ✅ PerTypeOverrides 改按钮触发 + PerTypeOverridesModal（高级配置弹窗；**2026-08-18 回退 US-004 矩阵化：列=g 码 × 2 行 d/tol，per_type 单级 {g 码:{d,tol}}**）+ PtypePreviewModal（片型放大预览）；US-031 ✅ 加 data-tour 锚点（doc-banner / start-btn 父容器 / param-form 在 SizePicker / export-group 在 ExportButtons）；策略 PRD US-005 ✅ StrategyRunButton（高级运行入口 + 运行中徽标 + 单例弹窗）+ StrategyRunModal（三态：配置四档时长+双模式 / 进度五件套+seed chips+终止 / 结果最优+用布+模式汇总+run_dir 复制+应用按钮（策略 PRD US-006 ✅ 已接线 —— NestingPage 经 ControlPanel 透传 onApplyStrategy；未传回调（测试挂载）才 disabled）+error 重试+orphan 清理；ESC/遮罩/✕ 关闭均不 stop；__tests__/StrategyRunModal 13 项（三态+五件套+SE 延长+载荷同源+关闭不终止）+ StrategyRunButton 4 项）；腰头成带 US-013 ✅ PerTypeOverridesModal「布局设置」分区（band 三字段草稿+下拉/缩略图+预演 /api/band/preview 回显+ack 二次确认）+ ControlPanel band 启动闸门/互斥/bandStore 镜像 + StrategyRunButton title prop（互斥说明）；腰头成带 US-015 ✅ 填料多选行（PerTypeOverridesModal bandFillers 草稿：候选=orderedLabels−主码、chip 48×48 compact 缩略图+qty 徽章、`.on`+aria-pressed 选中态、满 3 未选中 disabled、切主码自动剔除同名、预演 body 带非空 fillers（fillersKey join(',') 做 effect 依赖防数组引用重跑）；ControlPanel bandFillerZeroLabel 启动闸门（填料所选码数量全 0 → 置灰+StatusLine 文案）；BandFormValue 扩 fillers 透传）
...
- **null 码 chip 文案「通用」**（与上传预览矩阵行头「通用」同语义；SizeTabs 已删，无共享常量）：`sizeLabel(null)='通用'`、`sizeKey(null)='null'`（DOM id `sz_null` / value `'null'`）。改文案需同步 SizePicker.test.tsx 「null 码 chip 显示通用」+ ControlPanel.test.tsx US-017 StatusLine hint 用例。
...
## 腰头成带 US-013 关键约定（布局设置弹窗 / band 闸门·互斥·镜像 / 不成对警告 调用方必读）

- **band 草稿三元组 `{enabled, label, ack}`，confirm 是唯一回写路径**：PerTypeOverridesModal mount 从 props 读初值，确定写回 `onBandChange`（ControlPanel patch form.band_* 三字段）；取消/遮罩/ESC 连同草稿丢弃（与 per_type 同约定）。未勾选时 label 原样保留（collectBand 对 enabled=false 恒 null）。**US-015 起草稿四元组 `{enabled, label, ack, fillers}`**（BandFormValue 扩 `fillers: string[]`；mount 初值过滤同名主码残留；切主码自动剔除同名 filler —— UI 候选本就不含主码，这是防御性清洗）。
- **填料多选行（US-015，v1.1 混带）**：候选 = `orderedLabels − bandLabel`（与腰头编号下拉同源 reps 键），chip = compact PiecePreviewSVG + `.qty-label-badge`；选中 `.on`+`aria-pressed`，满 `BAND_MAX_FILLERS=3` 未选中 chip `disabled`（已选中恒可反选）；预演 effect 依赖用 `fillersKey = fillers.join(',')`（数组引用每次 render 变化会致 effect 重跑），body 仅在 fillers 非空时附键（旧 payload 形状不变）；ControlPanel 新增 `bandFillerZeroLabel` 闸门（填料码 `bandMemberCount===0` → startDisabled + StatusLine 文案 + handleStart 兜底，与 bandZeroQty 同构）。
- **US-015 测试基线**：PerTypeOverridesModal.test.tsx +6（填料行可见性+候选排除主码 / 多选→预演 body `{enabled,label,fillers}` / 满 3 置灰+反选恢复 / 切主码剔除 / confirm 回写 fillers+取消丢弃 / props 初值过滤）；params.test.ts +3（collectBand fillers 清洗去重主码剔除 ack 组合 / 空→无 fillers 键 / band 关恒 null）；既有 literal 补 `fillers: []`（renderModal 默认值/band prop/3 处 toHaveBeenCalledWith/PtypePreviewModal.test.tsx）。浏览器验证 scripts/us015_verify.mjs 17/17 PASS（Playwright 系统 Chrome：上传 5336 → P0 需求表 → g05+ack 纯腰预演 79.5% → 填 g07 多选 86.5%==A/B on 臂 → 满 3 置灰/恢复 → 求解终值 87.77%==A/B 87.766% + 末帧守恒 g05=14/g07=7 + 无 WB_ 泄漏；截图 `.docs/business/us015_*.png`）。
- **ack 只来自弹窗显式勾选（FR-1）**：预演 422 `{error, hard_warning:true}`（后端 `BandAckRequired`：成员最小边 <60mm 或长宽比 >6，如 5336 g05 长宽比 6.9）→ 预演行下方 amber 勾选框；勾选 → 带 `ack:true` 重试预演（成功回显 fill，勾选框保持可见可反勾）；**切换 g 码 / 取消成带勾选 → ack 草稿重置 false**。`collectBand` 只在 `band_ack=true` 时附 ack 键（band 关恒 null，ack 残留不生效）。
- **预演失败不阻塞确定（FR-7）**：几何失败（200 `{ok:false,error}`）/ 网络 / 4xx 全走降级提示，confirm 永不因预演失败置灰。body = `buildStartContext()`（经 ref 取，effect 依赖只锁 band 选择/ack）+ band 草稿 —— 预演所见即求解所得；break_even 参考线由响应携带（后端 `BAND_BREAK_EVEN_PCT` 单一真相源，前端不双写）。
- **band 启动闸门（AC#3）**：`bandMissingLabel`（开未选编号）/ `bandZeroQty`（`bandMemberCount===0`）→ `startDisabled` 置灰 + StatusLine band 段具体文案（` — ` 连接）+ handleStart 运行时兜底。bandMemberCount 三态与后端 `_band_demand`/`build_pid_meta` 口径对齐（US-012 不变量延续）。
...
- **bandStore 单向镜像**：ControlPanel useEffect `form.band_* → setBandMirror`；QtyMatrix 订阅读（bandCol 奇数格 `.qty-cell.odd` + title + 列头 `.qty-odd-badge`）。**绝无反向回写**；form 是 WS payload 唯一 SSOT。
...
- **测试基线**：PerTypeOverridesModal.test.tsx 38 项（US-013 +10：分区/禁用/值域降级/预演 body+回显/低于参考线/ok:false 降级/**422 hard_warning→ack→重试→confirm ack:true**/几何失败无勾选框/切码重置/丢弃/缩略图）；ControlPanel.test.tsx 41 项（US-013 +5：闸门置灰+文案 / 数量全 0 / 写回全链路 / **ack 全链路** / 互斥+恢复）；QtyMatrix.test.tsx 42 项（+4 不成对）；params.test.ts +2（DEFAULT band_ack / ack 序列化）。fetch mock：`mockPreviewStatus` 可配 422（组件只读 body 不查 r.ok）。浏览器验证 scripts/us013_verify.mjs 17/17 PASS（Playwright 系统 Chrome；上传 5336 → g05 硬警告 ack → 预演 50.6% → 求解 stage「腰头成带中」→ 不成对徽章 → 互斥恢复）。
