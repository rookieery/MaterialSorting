...
- **exclude 是 best-effort 不阻断**：run 带 band（enabled+label）→ exclude.labels；final 带
  ...
  不发请求。
- **对比卡口径脚注必须出现**：「物理毛版轮廓口径（与导出一致；画布红字为腐蚀后轮廓口径，
  数值可能偏小）」—— 报告七指标与 /export 同源（原始 polygon），画布红字告警按 erode 后
  轮廓，两套数值并存是设计非 bug，删脚注 = 口径歧义回归。
- **卡栈透点**：.edit-br-stack/.edit-polish-card 均 pointer-events:none（不遮画布拖动热区），
  撤销按钮是栈内唯一 pointer-events:auto 交互点；新增卡内交互控件必须显式开 auto。
