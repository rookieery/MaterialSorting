...
- **职责边界（红线）**：`src/lib/snap.ts` 的 `computeSnapCorrection(dragged, rawTr, sess, others, opts)` 只做「给定落点 → 纠正后 translation」的**单次纯函数求解**；触发与否（右键 snap 会话 vs 左键自由拖动）是调用方（US-003 EditCanvas）职责，引擎不感知按键/DOM/事件。结果 `{tr, kind, partnerKey}` **只改 translation**，永不动 rot/mirror；`kind`：free = 未施加纠正（含一切 fail-open）/ retreat = 回退到首个安全界 / attract = 吸拢到触点 −1nm。
...
## edit-drag-snap US-003 关键约定（EditCanvas 右键贴附会话接线；2026-09-06）

- **按键门控（红线）**：`onPointerDown` 顶部 `if (e.button !== 0 && e.button !== 2) return;` —— 0 = 左键（拖片/平移/转柄既有会话**逐字节零改动**，回归红线）、2 = 右键（仅毛版 polygon 起 `MoveDrag{snap:true}` 贴附会话；旋转柄分支内 `button !== 0` return = 右键不转柄、空白分支内同 = 右键空白不平移不取消选中）、其余非主键（中键 1/后退 3/前进 4）不起任何会话。`snapSessRef`（`SnapSession | null`）右键 pointerdown 起 `{lastSafeTr:null, startOverlapMm2:Infinity}`（紧随 selectPiece → refreshMetrics 首帧落真实基线；首帧 degraded 则 Infinity = attract-only fail-open）、左键显式置 null（防御清残值）、pointercancel/骨架重建/卸载一律作废不求解。
- **contextmenu 吞除**：svg 上 native listener `preventDefault()` —— 工具型画布无自定义右键菜单，右键已被贴附手势占用（右键拖动中/裁片上/空白处均不弹系统菜单，单测 + 浏览器 trusted click 双锁）。
