...
//
// 与旧 legacy/app.js `renderAtTime` 内 `$('seek-readout').textContent = parts.join(' | ')` 等价。
// 格式：`t=X.Xs | sN yy.yy% | sM zz.zz%`
//   - t = seekTime（>=0 时）；toFixed(1) 与旧 app.js 一致
...
export function SeekReadout(): React.JSX.Element {
  ...
  // 旧 app.js：t = parseInt($('seek').value, 10)；此处等价 —— 用 seekTime 或末尾。
