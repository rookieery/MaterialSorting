...
//
// 与旧 legacy/index.html `<div class="playback">` + 旧 vanilla 实现 checkAllDone 启用 seekbar 等价。
//   - 求解中 / 未启动：disabled，max=0
//   - 全部 run 完成：max = ceil(maxElapsed)（PlaybackBar 计算），Seekbar 默认 value=末尾
//   - SeekReadout：见组件注释
//
// 启用判定：runs.length > 0 && runs.every(done)（与旧 vanilla 实现 `if (!runs.every(r=>r.done)) return` 一致）。
//
