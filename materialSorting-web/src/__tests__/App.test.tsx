...
function makeParsedDoc(): ParsedDoc {
  return {
    doc_id: 'app-test-doc',
    filename: 'M1787.dxf',
    sizes: [{ size: 28, pieces: [{ label: 'g01', polygon: [], internal_lines: [], notches: [], net_polygon: [], grain_line: null }] }],
  };
}
...
describe('App Tab 集成（US-001）', () => {
  ...
  it('切到 preview：nesting 加 .hidden（DOM 不卸载，ControlPanel 仍在），preview 取消 .hidden', () => {
    ...
    // beforeEach 设的 done+doc 让 PreviewPage 渲染 QtyMatrix（非 .preview-empty）。
    const el = renderApp();
    act(() => {
      useUiStore.getState().setTab('preview');
    });
    ...
    // preview page 顶层容器可见（US-016 联动后已上传→显示 QtyMatrix，
