...
//     （气泡显示 readyHint + 下一步 disabled）+ 200ms 轮询；ready() 翻 true 后自动推进
//     （最后一步 export 为自动完成：markSeen('nesting') + close）。
//
...
//   - result    至少一个 run 有 lastFrame（排料结果已渲染到 NestsGrid）
//   - export    同 result（有 lastFrame 才允许导出，与 ExportButtons disabled 逻辑同源）
//
// 锚点用 data-tour 解耦 CSS 类名重构（querySelector 命中首个即可）：
//   - doc-banner  [data-tour="doc-banner"]   （ControlPanel.tsx 当前文件上下文条）
//   - params      [data-tour="param-form"]   （SizePicker.tsx 码号多选区容器）
...
//
// before 副作用：5 步均 ensureNestingTab（用户从 preview Tab 用菜单「重看超排指引」
...
export const nestingTour: TourDef = {
  ...
  steps: [
    ...
    },
    {
      ...
      title: '设置参数',
      body: '勾选要参与排料的码号（多选），下方实时显示总裁片数量。可在「幅宽 / 时长 / seed」调整求解参数，开启 multi_seed 可并行对比多个 seed 的方案。',
      placement: 'bottom',
      before: ensureNestingTab,
    },
    {
    ...
    },
    {
      ...
      readyHint: '请点击「开始求解」，产出首个排料方案后自动进入下一步',
    },
    {
      id: 'export',
