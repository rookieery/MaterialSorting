...
export const previewTour: TourDef = {
  ...
  steps: [
    ...
    },
    {
      ...
      title: '设置裁片数量',
      body: '在矩阵格子内直接输入每码排料份数（0 = 该码不排此裁片）；同一裁片 g 码要跨码统一数量时，点列头缩略图下方的「≡」按钮（悬浮提示「整列设值」）批量填入；某一尺码要整行统一数量时，点行头码号旁的「≡」按钮（悬浮提示「整行设值」）批量填入。个别格子改不同的值会高亮为特例。总片数 = 各码数量之和（每份对应母版一个轮廓，不合成镜像）。数量跨码匹配同一裁片 g 码，这是求解前的必要一步。',
      placement: 'bottom',
      before: ensurePreviewTab,
    },
    {
      ...
      title: '已应用至超排',
      body: '解析结果会自动 commit 到超排（按裁片 g 码切单裁片），完成后此处显示「已应用至超排：N 裁片，M 码」，并解锁上方「超排」Tab。',
      placement: 'bottom',
      before: ensurePreviewTab,
