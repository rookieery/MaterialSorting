...
> 再收口（2026-09-17 用户定案）：**触发手势右键 → Alt+左键整体切换**。实测 Edge 内置「鼠标手势」在浏览器层拦截右键按住拖动（画轨迹执行后退/关标签页等动作），Microsoft 官方确认网页无法用 JS 压制（`preventDefault`/`contextmenu` 均拦不住；chess.com 右键画箭头同款冲突），且右键向左拖会被识别为「后退」直接导航离开页面丢编辑态 —— FR-4/「双按钮双模式」权衡一节的「右键空闲手势可占用」「可发现性优于修饰键」论据在 Edge 实机环境不成立。切换语义：纯左键零回归红线不动（当初选右键的核心动机 = 不占左键自由拖动，修饰键+左键同样不占）；右键全域失效（无任何画布会话）；contextmenu 吞除维持（防误弹「后退」类系统菜单）；snap 标志 pointerdown 锁存（拖动中松开 Alt 不影响松手求解）；指南卡/tour 文案同步改「Alt+左键拖动松手贴附」。上方定案②（触控板等效修饰键）就此自然过期 —— 修饰键已是主触发。
