...
- **弹窗全字段预览（2026-08-31）**：导出弹窗按最终表格列序展示**全部 14 字段**——8 自动字段以只读行显示成品串（数据源 `POST /api/plt-table-preview`，列序/格式权威在后端 `_row_texts` 同一真相源、前端零公式镜像；绘图时间预览 = 请求时刻、导出时重算），手输槽位渲染本地草稿输入框；预览不可用（网络错/无解）时优雅降级回 6 手输 + 提示形态，确认导出永不被预览阻塞。
...
- **门幅框口径（2026-08-31）**：PLT 门幅框满幅 `[0, gate]` 不内缩（撤销旧 Y 双边内缩 5mm——内缩框被切割软件当布料范围、贴门幅边排的裁片视觉穿框 5mm 成「越界布料」；现框=裁剪界=求解约束带三口径合一，贴边裁片平切框线，与生产件观感一致）。需物理留边（布边不裁）按 2026-08-28 定案机制直接输更小门幅。**同日增 Y 绘制平移 `PLOT_LEAD_Y_MM=TABLE_W_MM=36`（用户定案）**：整张图纸（门幅框/裁片/工艺线/表格）在 PLT 绘制层整体 +36mm=表格宽——框线离图纸原点 36mm，框线左侧留出与表格等宽的空纸边（首版 5mm 对齐生产件观感，用户反馈太短后定案为表格宽）；纯绘制层位移，世界坐标层（求解带/裁剪界/密度）与框内相对几何零变化，切割软件仍以框线为布料范围、只裁框内内容（PLT 只是绘制图纸，框外纸边与表格不参与裁剪）。与旧内缩的本质区别：内缩改框位置致裁片穿框，平移是框+内容一起走。
- **PLT（毛版）变体（2026-08-31；当日由「净版」更名——与裁片 layer1「毛版轮廓」命名统一，协议值 `'plt-clean'` 不随更名变，ascii 后缀 `_clean` 同理保留）**：导出下拉「PLT（毛版）」（`fmt='plt-clean'`）且**为默认导出格式**（同日用户定案：毛版为现场主交付，`DEFAULT_EXPORT_FMT` 2026-08-24~08-30 曾为 `'plt'` 全量），形态对齐生产参考件 `data/PC-20250508NJIF_5028-1#_29223513.plt`：① 唛架**左右各一份同内容表格**——右表即原表格原位不动，左表 = 同一 `info_table_polylines` 以 `table_x0=-(gap+表宽)` 再画一份（key 带天然落在小 x 侧，两表生产视图阅读方向一致、无需镜像文字），左表 value 带外缘与门幅左边框共线（与右表同构）；② 正文每片**只画毛版裁剪轮廓（polygon）+ 尺码\*数量标注**——净版线（net_polygon）/内部线/刀口/布纹箭头线全部不画，标注几何与全量版完全相同（同位置/同 10mm 字高/同字库，仅去掉杆+箭羽）。带左表时 X 走纸引导扩为 `PLOT_LEAD_X_MM+TABLE_GAP_MM+TABLE_W_MM=56mm`（左表世界 x∈[−36,0] → 绘制 x≥20 仍非负），PS 纸长随 lead 扩张；不带 `table` 键时无左表、引导回 20。文件名加 `_clean`（ascii）/`_毛版`（中文）后缀防与全量版混淆；`clean=False`（`fmt='plt'`）输出与旧版**逐字节一致**（additive 红线，测试锁死）。填写弹窗/预览端点两变体共用（`variant` prop 只改文案）。
- **零回归**：不带 `table` 键（旧前端 / 直接 POST）导出的 PLT 与旧版逐字节一致（边框恒到 width_mm、纸长不含表格；2026-08-31 门幅框满幅化 + Y 平移两改后逐字节基线以当日最终版为界）；带表格时边框仍到 width_mm（表格共线外挂）、PS 纸长 =(width+66)×40 覆盖表格区；PNG/DXF 载荷忽略 `table` 键。
...
| `prefix` | dict | — | 起始端成套前后幅 `{'enabled': bool, 'front': g码, 'back': g码}`（enabled=true 时 front/back 必填、匹配 `^g\d+$` 且 front≠back）；资格码（该码 front/back demand 均 ==2）存在性由 worker 求解期 fail-fast，web 策略入口经 `_parse_prefix` start 期 400 早退；2026-08-25 起与 `--strategy` 兼容（选码确定性重放；2026-09-02 起升级 `select_prefix_plan` 近满幅几何搜索——资格码×片型×异码补片×rot 取 union bbox 高 ≤ gate 最大者，无 RNG，全无可行兜底 seeded 4 片）；套装成员序 **2026-09-03 改判 paired 同型成对 [后@0,后@180,前@0,前@180]**（用户依生产参考件改判 FR-10 interleave 定稿，同型对 180° 曲率自吻合互锁；interleave/grouped 备档 order 值保留）；on 时 `--lns` 自动 warn 跳过。端到端验收（2026-09-02 异码补片收官 + 2026-09-03 成员序 paired 定案）：UI 冒烟 `materialSorting-web/scripts/smoke_prefix_extra.mjs`（29 检查全过，含导出 PLT 无 `PS_`）+ `python -m materialsorting.web.prefix_accept` 短预算对拍（异码补片收官 on/off 0.556pt accept；成员序 A/B 同会话背靠背 on/on' 均值劣化 **−0.597pt paired 更优**、on/off −0.434pt、双开 −0.313pt 全过 ≤1.0pt 线），业务定案见 `.docs/business/起始端成套前后幅_版师确认清单.md` §9/§10 |
...
**策略双模式（US-001/002，`--strategy [se|race]`，给定总预算拿更高利用率）**：词汇表 —— **race 门杀**（方案 B，裸旗标 `--strategy` 即此档）：每 seed 按 `--race-budget`（默认 180s）预算启动，**门时刻**（预算 × `--race-gate` τ，默认 0.5 → 90s）处**严格破纪录**（当前 best 严格 > **bar** 才续跑满程，否则真 terminate + best-so-far 帧交付，省出预算再投资后续 seed；首 seed 豁免 —— 无 bar 参照即锚定交付下限）；**SE 筛延**（方案 A，`--strategy se`）：阶段 1 k 轮 `--se-screen`（默认 90s）串行筛选，阶段 2 **近并列多候选顺延**（2026-09-23 prd-se-ext-top3，此前为单冠军）：筛选密度（原面积口径）与冠军相差 ≤0.5pt（`SE_EXT_BAND=0.005` 绝对百分点，闭边界）的 seed 一并顺延，按密度降序稳定排序取前 `--se-ext-top`（默认 3；冠军恒为候选[0]，argmax 语义不变）**串行**逐个跑完整 `--se-extend`（默认 180s）延长轮，各候选边车 `best_frame_s{seed}_ext.json` 独立成对；**超支设计**——k 筛选轮数与 `--time` 规划锚不变，最坏额外 +2 轮延长（默认档 ~+6min / 极限 600 档 ~+20min / 1200 档 ~+40min）；`best`/incumbent = 全部延长轮帧级全局最优（候选轮集合 ⊇ 单冠军 ⇒ 单调不降，免 A/B 门槛）；延长轮 warm 逐轮各自加载该 seed 筛选边车（回退矩阵逐轮独立）；R0 达标即停不进延长段、Ctrl-C 交付已完成轮且后续候选不启动；**名义记账**：门杀 seed 计 92.5s（90s 门 + 2.5s 启动）、满程 seed 计 182.5s（180s + 2.5s），`--time` 在策略模式 = **总预算秒数且必填**（与两档共享口径对账，T≥275 才可启动）；**确定性重放**：同 seed + 同 time_budget 逐帧一致（求解链路确定性），因此 SE 的延长 = 冠军 180s 潜力的**零方差求值** —— 两模式期望等价是机制保证而非巧合。
...
参数旗标 `--se-screen` / `--se-extend` / `--race-budget` / `--race-gate`（(0,1) 开区间）须与 `--strategy` 同给；`--se-ext-top N`（int ≥1，默认 3）须与 `--strategy se` 同给、与 `--extreme` 显式互斥（极限 SE 臂继承默认 top-3）—— `1` = 单冠军旧行为（A/B 哨兵）；种子流无重复（config `seeds` 优先、max+1 补齐）；`--target` 共存时 R0 达标即停优先于模式继续；`--kill` 与 `--strategy` 显式同给退出 1（策略模式判据内建，不叠 kill 引擎）。产物 `run_dir/strategy.json`（`mode`/`total_budget`/`planned_seeds`/`race|se` 参数/kill 统计；se 段 2026-09-23 起 additive：计划态 `ext_top_n`/`ext_band` 开跑即写，首个延长轮启动时补写实际态 `ext_seeds`（候选全集）/`extra_rounds`(m−1)，不进延长保持计划态）；多候选可观测三面 additive —— result.json `portfolio.se.ext_seeds` + config.strategy `ext_top_n`/`ext_warm_rounds`（逐延长轮 warm 灌入态；既有 `warm`/`warm_reason` 保持冠军轮语义）、run_stats 行 config 段 `se_ext` = `{top_n, band, candidates: [{seed, screen_density, gap_pt}]}`（自包含 0.5pt 阈值校准数据面，进过延长段才落键；延长终值/是否反超由 per_seed `phase='extension'` + `incumbent.seed` join 得出）；无 `--strategy` 时行为与 result.json 与现版**逐字节一致**（零回归红线）。
...
**SE 顺延臂（2026-09-20，`--extreme-strategy se`）**：warm-start 一期落地后 SE 延长轮成为真顺延（自冠军解热启动增量搜索），极限档第二种展开形态 —— `--strategy se --se-screen <budget/2> --se-extend <budget>` + 同一份极限 `solver_opts`（600 档 = 300s×k 筛选 + 冠军 600s warm 顺延；1200 档 = 600/1200）。参数依据：筛选 = 预算一半与 race 门时刻同口径（门判别力 ρ=0.916 的 300s 截面直接复用为冠军选择器），1:2 筛延比例与默认 se 90:180 同款；名义记账两臂同界（冠军全程 602.5 + 单轮筛选 302.5 = 905s 最低总预算，web 值域通用）。warm 默认 on 不可显式关（`--se-warm` 与糖衣互斥照旧），不可用自动回退重放不炸轮；race 臂（缺省）全部产物逐字节不变，se 臂 run_stats 的 `extreme` 段 additive 加 `"strategy": "se"`。web 端弹窗加「模式」下拉（race 门杀默认 / SE 顺延，镜像高级运行结构），status 载荷 additive `strategy` 键透传实际臂。**注**：se 臂极限参数迁移性与 race vs se 极限档相对优劣未做专项长 A/B（验收口径 = 最小档端到端跑通，同 band/prefix 解禁先例）；race 臂保有完整 4h 三臂验收背书，故默认档维持 race。
...
ms-run-config data/configs/5336_coded_really.json --extreme --extreme-strategy se --time 14400   # 4h SE 顺延臂（45 筛 + 1 延）
```
