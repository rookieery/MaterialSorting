...
| `prefix` | dict | — | 起始端成套前后幅 `{'enabled': bool, 'front': g码, 'back': g码}`（enabled=true 时 front/back 必填、匹配 `^g\d+$` 且 front≠back）；资格码（该码 front/back demand 均 ==2）存在性由 worker 求解期 fail-fast，web 策略入口经 `_parse_prefix` start 期 400 早退；2026-08-25 起与 `--strategy` 兼容（选码确定性重放；2026-09-02 起升级 `select_prefix_plan` 近满幅几何搜索——资格码×片型×异码补片×rot 取 union bbox 高 ≤ gate 最大者，无 RNG，全无可行兜底 seeded 4 片）；on 时 `--lns` 自动 warn 跳过。端到端验收（2026-09-02 异码补片收官）：UI 冒烟 `materialSorting-web/scripts/smoke_prefix_extra.mjs`（29 检查全过，含导出 PLT 无 `PS_`）+ `python -m materialsorting.web.prefix_accept` 短预算 on/off 对拍 accept（0.556pt ≤1.0pt，形态/双开/确定性/导出全过），业务定案见 `.docs/business/起始端成套前后幅_版师确认清单.md` §9 |
